///|/ Copyright (c) Prusa Research 2022 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Vojtěch Bubník @bubnikv, Enrico Turri
///@enricoturri1966, Lukáš Matěna @lukasmatena
...
bool run_file(const boost::filesystem::path &path)
{
    ...
    if (!res) {
        ...
        BOOST_LOG_TRIVIAL(error)
            << full_message; // lm: maybe UI error msg?  // dk: bellow. (maybe some general show error evt would be better?)
...
bool run_file(const boost::filesystem::path &path)
{
    if (boost::filesystem::exists(path)) {
        ...
        const char *argv2[] = {"open", "/Volumes/FlsunSlicer", nullptr};
...
#else
bool run_file(const boost::filesystem::path &path) { return false; }
                                     ...
                                     std::string &                                                         error_message) const
{
    ...
    Http::get(url)
        ...
        .on_progress([&, progress_fn](Http::Progress progress, bool &cancel) {
            ...
            if (cancel) {
                // Lets keep error_message empty here - if there is need to show error dialog, the message will be probably shown by
                // whatever caused the cancel.
...
boost::filesystem::path AppUpdater::priv::download_file(const DownloadAppData &data) const
{
    ...
    bool        res = http_get_file(
        data.url, 130 * 1024 * 1024 // 2.4.0 windows installer is 65MB //lm:I don't know, but larger. The binaries will grow. // dk: changed
                                    // to 130, to have 100% more space. We should put this information into version file.
                                    // on_progress
        ,
        [&last_gui_progress, expected_size](Http::Progress progress) {
            // size check
            if (progress.dltotal > 0 && progress.dltotal > expected_size) {
                std::string message = GUI::format("Downloading new %1% has failed. The file has incorrect file size. Aborting "
                                                  "download.\nExpected size: %2%\nDownload size: %3%",
        ...
        [&file, dest_path, tmp_path, expected_size](std::string body, std::string &error_message) {
            ...
            if (body_size != expected_size) {
                error_message = GUI::format(_u8L("Downloaded file has wrong size. Expected size: %1% Downloaded size: %2%"), expected_size,
                                            body_size);
                return false;
            }
            if (file == NULL) {
...
void AppUpdater::priv::version_check(const std::string &version_check_url)
{
    ...
    if (!res) {
        std::string message = GUI::format(_L("Downloading %1% version file has failed:\n%2%"), SLIC3R_APP_NAME, error_message);
        BOOST_LOG_TRIVIAL(error) << message;
...
void AppUpdater::priv::parse_version_string(const std::string &body)
{
    ...
    //err.message(), err.line(), tree_string).c_str()); 		BOOST_LOG_TRIVIAL(error) << format("Failed reading version file property tree Error:
    //\"%1%\" at line %2%. \nTree:\n%3%", err.message(), err.line(), tree_string); 		return;
    //	}

    DownloadAppData new_data;
    new_data.version = Semver::parse(body);
    ...
    //					if (!release_version) {
    //						BOOST_LOG_TRIVIAL(error) << format("Received invalid contents from version file: Not a correct semver: `%1%`",
    //version); 						return;
    //					}
    ...
    //version); 					wxCommandEvent* evt = new wxCommandEvent(EVT_SLIC3R_VERSION_ONLINE); 					evt->SetString(GUI::from_u8(version));
    //					GUI::wxGetApp().QueueEvent(evt);
    //					*/
    ...
    //			if (recent_version) {
    //				BOOST_LOG_TRIVIAL(info) << format("Got %1% online version: `%2%`. Sending to GUI thread...", SLIC3R_APP_NAME,
    //version_string); 				wxCommandEvent* evt = new wxCommandEvent(EVT_SLIC3R_EXPERIMENTAL_VERSION_ONLINE);
...
#if 0 // lm:is this meant to be ressurected? //dk: it is code that parses PrusaSlicer.version2 in 2.4.0, It was deleted from
      // PresetUpdater.cpp and I would keep it here for possible reference.
...
void AppUpdater::sync_download()
{
    ...
    p->run_downloaded_file(std::move("update.exe"));
...
void AppUpdater::cancel() { p->m_cancel = true; }
...
std::string AppUpdater::get_default_dest_folder() { return p->m_default_dest_folder.string(); }
...
void AppUpdater::set_app_data(DownloadAppData data) { p->set_app_data(std::move(data)); }

DownloadAppData AppUpdater::get_app_data() { return p->get_app_data(); }

bool AppUpdater::get_triggered_by_user() const { return p->get_triggered_by_user(); }

bool AppUpdater::get_download_ongoing() const { return p->get_download_ongoing(); }
