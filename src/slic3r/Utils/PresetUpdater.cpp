...
void PresetUpdater::priv::get_missing_resource(const std::string& vendor, const std::string& filename, const std::string& url) const
{
	...
	if (!boost::starts_with(url, "http://192.168.100.190:8090/") && !boost::starts_with(url, "https://192.168.100.190:8090/"))
	{
...
void PresetUpdater::priv::sync_config(const VendorMap vendors, const std::string& index_archive_url)
{
	...
	if (!boost::starts_with(index_archive_url, "http://192.168.100.190:8090/") &&
        !boost::starts_with(index_archive_url, "https://192.168.100.190:8090/"))
	{
...
// Version of slic3r that was running the last time and which was read out from FlsunSlicer.ini is provided
// as a parameter.
Updates PresetUpdater::priv::get_config_updates(const Semver &old_slic3r_version) const
{
    ...
    for (const Index& idx : index_db) {
		...
		if (recommended->config_version < vp.config_version) {
			BOOST_LOG_TRIVIAL(warning) << format("Recommended config version for the currently running FlsunSlicer is older than the currently installed config for vendor %1%. This should not happen.", idx.vendor());
			continue;
		}
		...
		// Config bundle update situation. The recommended config bundle version for this FlsunSlicer version from the index from the cache is newer
		...
		// 1) The last config index downloaded by any previously running FlsunSlicer instance
		// 2) The last config index installed by any previously running FlsunSlicer instance (older or newer) from its resources.
		// 3) The last config index installed by the currently running FlsunSlicer instance from its resources.
			...
			if (rsrc_vp.valid()) {
				...
				if (recommended != rsrc_idx.end() && recommended->config_version == rsrc_vp.config_version && recommended->config_version > vp.config_version) {
					...
					BOOST_LOG_TRIVIAL(warning) << format("The recommended config version for vendor `%1%` in resources does not match the recommended\n"
			                                             " config version for this version of FlsunSlicer. Corrupted installation?", idx.vendor());
				}
			}
		}
