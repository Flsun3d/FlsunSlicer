...
//#include <locale>  
//#include <codecvt> 
// For starting another FlsunSlicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "FlsunSlicer.exe" : "flsun-gcodeviewer.exe";
    ...
    //for (const auto &file : paths_to_open) {
    //    // 假设file是UTF-8编码的，将其转换为wxString
    //    wxString wideCharFile = wxString::FromUTF8(file.c_str());
    //    args.emplace_back(wideCharFile.wc_str());
    //}  
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, FlsunSlicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		// ((instance_type == NewSlicerInstanceType::Slicer) ? "FlsunSlicer" : "PrusaGCodeViewer");
		// Just run FlsunSlicer and give it a --gcodeviewer parameter.
		bin_path = bin_path.parent_path() / "FlsunSlicer";
		...
		try {
		    ...
		    // boost::process::spawn() sets SIGCHLD to SIGIGN for the child process, thus if a child FlsunSlicer spawns another
		    // subprocess and the subrocess dies, the child FlsunSlicer will not receive information on end of subprocess
		    // (posix waitpid() call will always fail).
		    // https://jmmv.dev/2008/10/boostprocess-and-sigchld.html
		    // The child instance of FlsunSlicer has to reset SIGCHLD to its default, so that posix waitpid() and similar continue to work.
		    // See GH issue #5507
		}
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "FlsunSlicer" : "flsun-gcodeviewer")).string();
