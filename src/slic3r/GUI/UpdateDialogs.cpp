///|/ Copyright (c) Prusa Research 2018 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Lukáš Matěna @lukasmatena, Lukáš Hejl
///@hejllukas, Vojtěch Král @vojtechkral, Vojtěch Bubník @bubnikv
    ...
    /*if (auto* btn_ok = get_button(wxID_OK); btn_ok != NULL) {
        ...
        btn_ok->Bind(wxEVT_BUTTON, ([this, path](wxCommandEvent& e){
            ...
            if (!boost::filesystem::exists(dir, ec) || !boost::filesystem::is_directory(dir,ec) || ec) {
                ...
                show_change = false;
                MessageDialog msgdlg(nullptr, GUI::format_wxstr(_L("Directory %1% doesn't exists. Do you wish to create it?"),
    dir.string()), _L("Notice"), wxYES_NO); if (msgdlg.ShowModal() != wxID_YES) return; ec.clear(); if(!boost::filesystem::create_directory(dir,
    ec) || ec) { MessageDialog msgdlg(nullptr, _L("Failed to create directory."), _L("Notice"), wxOK); msgdlg.ShowModal(); return;
                }
            }
            if (boost::filesystem::exists(path)) {
                show_change = false;
                MessageDialog msgdlg(nullptr, GUI::format_wxstr(_L("File %1% already exists. Do you wish to overwrite it?"),
    path.string()),_L("Notice"), wxYES_NO); if (msgdlg.ShowModal() != wxID_YES) return;
            }
            if (show_change) {
                MessageDialog msgdlg(nullptr, GUI::format_wxstr(_L("Download path is %1%. Do you wish to continue?"), path.string()),
    _L("Notice"), wxYES_NO); if (msgdlg.ShowModal() != wxID_YES) return;
            }
...
MsgUpdateConfig::MsgUpdateConfig(const std::vector<Update> &updates, bool force_before_wizard /* = false*/)
    : MsgDialog(nullptr,
                ...
                force_before_wizard ?
                    _L("FlsunSlicer is not using the newest configuration available.\n"
    ...
    for (const auto &update : updates) {
        ...
        if (!update.new_printers.empty()) {
            flex->Add(new wxStaticText(this, wxID_ANY,
                                       _L_PLURAL("New printer", "New printers", update.new_printers.find(',') == std::string::npos ? 1 : 2) +
                                           ":"),
                      0, wxALIGN_RIGHT);
...
MsgUpdateForced::MsgUpdateForced(const std::vector<Update> &updates)
    : MsgDialog(nullptr,
                wxString::Format(_(L("%s incompatibility")), SLIC3R_APP_NAME),
                _(L("You must install a configuration update.")) + " ",
                wxOK | wxICON_ERROR)
{
    auto *text = new wxStaticText(
        this, wxID_ANY,
        wxString::Format(_(L("%s will now start updates. Otherwise it won't be able to start.\n\n"
                             "Note that a full configuration snapshot will be created first. It can then be restored at any time "
                             "should there be a problem with the new version.\n\n"
                             "Updated configuration bundles:")),
                         SLIC3R_APP_NAME));
    ...
    for (const auto &update : updates) {
        ...
        if (!update.comment.empty()) {
            versions->Add(new wxStaticText(this, wxID_ANY,
                                           _(L("Comment:"))) /*, 0, wxALIGN_RIGHT*/); // uncoment if align to right (might not look good if
                                                                                      // 1  vedor name is longer than other names)
            auto *update_comment = new wxStaticText(this, wxID_ANY, from_u8(update.comment));
            update_comment->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
...
MsgDataIncompatible::MsgDataIncompatible(const std::unordered_map<std::string, wxString> &incompats)
    : MsgDialog(nullptr,
                wxString::Format(_(L("%s incompatibility")), SLIC3R_APP_NAME),
                wxString::Format(_(L("%s configuration is incompatible")), SLIC3R_APP_NAME),
                wxICON_ERROR)
{
    auto *text = new wxStaticText(
        this, wxID_ANY,
...
MsgDataLegacy::MsgDataLegacy() : MsgDialog(nullptr, _(L("Configuration update")), _(L("Configuration update")))
{
    auto *text = new wxStaticText(
        this, wxID_ANY,
        format_wxstr(
            _L("%s now uses an updated configuration structure.\n\n"
    ...
    // TRN %s = PrusaSlicer
    auto *link = new wxHyperlinkCtrl(this, wxID_ANY, format_wxstr(_L("%s 1.0 configuration update"), SLIC3R_APP_NAME),
