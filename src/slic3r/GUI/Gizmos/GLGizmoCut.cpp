...
static const ColorRGBA SELECTED_PLAG_COLOR  = ColorRGBA(0.3f, 0.3f, 0.3f, 1.0f);
...
void GLGizmoCut3D::render_shortcuts()
{
    ...
    if (m_show_shortcuts)
        for (const auto&shortcut : m_shortcuts ){
            m_imgui->text_colored(ImGuiWrapper::COL_GREEN_LIGHT, shortcut.first);
...
void GLGizmoCut3D::render_connectors_input_window(CutConnectors &connectors)
{
    ...
    ImGui::AlignTextToFramePadding();
    m_imgui->text_colored(ImGuiWrapper::COL_GREEN_LIGHT, m_labels_map["Connectors"]);
...
void GLGizmoCut3D::render_build_size()
{
    ...
    ImGui::SameLine();
    m_imgui->text_colored(ImGuiWrapper::COL_GREEN_LIGHT, size);
}
...
//    if (m_mode == size_t(CutMode::cutPlanar)) {
    ...
    if (mode == CutMode::cutPlanar || mode == CutMode::cutTongueAndGroove) {
        ...
        ImGui::SameLine();
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_LIGHT,
...
//        render_flip_plane_button();
        ...
        if (mode == CutMode::cutPlanar) {
            ...
            ImGui::Separator();
            ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_LIGHT, m_labels_map["Groove"] + ": ");
...
static void check_objects_after_cut(const ModelObjectPtrs &objects)
{
    ...
    if (is_windows10() && !err_objects_idxs.empty()) {
        auto dlg = WarningDialog(plater,
                                 _L("Open edges or errors were detected after the cut.\n"
        ...
        if (dlg.ShowModal() == wxID_YES) {
            ...
            auto fix_and_update_progress = [model_names, &objects](const int obj_idx, int model_idx, wxProgressDialog &progress_dlg,
                                                                   std::vector<std::string> &                        succes_models,
