...
static void draw_mouse_offset(const std::optional<Vec2d> &offset)
{
    ...
    ImU32  color     = ImGui::GetColorU32(ImGuiWrapper::COL_GREEN_LIGHT);
    float  thickness = 3.f;
...
EmbossStyles GLGizmoEmboss::create_default_styles()
{
    ...
    if (wx_font.IsOk()) {
        ...
        std::string font_path = Slic3r::resources_dir() + "/fonts/SourceHanSansCN-Normal.ttf";
...
{    
    ...
    if (exist_change_in_font || !exist_stored_style)
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_LIGHT, font_text);
    else
...
void GLGizmoEmboss::draw_style_rename_popup() {
    ...
    if (new_name.empty()) {
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_DARK, _u8L("Name can't be empty."));
    }else if (!is_unique) { 
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_DARK, _u8L("Name has to be unique."));
    } else {
        ImGui::NewLine();
        allow_change = true;
    }
...
void GLGizmoEmboss::draw_style_list() {
    ...
    if (m_style_manager.exist_stored_style())
        ...
        ImGui::TextColored(ImGuiWrapper::COL_GREEN_LIGHT, "%s", title.c_str());
                               ...
                               Draw               draw) const
{
    ...
    if (changed || default_value == nullptr)
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_GREEN_LIGHT, name);
    else
