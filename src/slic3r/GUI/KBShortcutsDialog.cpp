///|/ Copyright (c) Prusa Research 2018 - 2023 Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Oleksandra Iushchenko @YuSanka,
///Vojtěch Bubník @bubnikv, Filip Sykala @Jony01, David Kocík @kocikdav, Tomáš Mészáros @tamasmeszaros, Vojtěch Král @vojtechkral
///|/
...
KBShortcutsDialog::KBShortcutsDialog(wxSize deaulsize, wxString language)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe),
                wxID_ANY,
                wxString(wxGetApp().is_editor() ? SLIC3R_APP_NAME : GCODEVIEWER_APP_NAME) + " - " + _L("Keyboard Shortcuts"),
                wxDefaultPosition,
                deaulsize,
                // wxDefaultSize,
...
void KBShortcutsDialog::fill_shortcuts()
{
    ...
    if (wxGetApp().is_editor()) {
        ...
        Shortcuts plater_shortcuts = {
            ...
            {(" ↑"), L("Move selection 10 mm in positive Y direction")},
            {(" ↓"), L("Move selection 10 mm in negative Y direction")},
            {(" ←"), L("Move selection 10 mm in negative X direction")},
            {(" →"), L("Move selection 10 mm in positive X direction")},
            {std::string("Shift+") + L("Any arrow"), L("Movement step set to 1 mm")},
            {ctrl + L("Any arrow"), L("Movement in camera space")},
            {("Page Up"), L("Rotate selection 45 degrees CCW")},
            {("Page Down"), L("Rotate selection 45 degrees CW")},
            {"M", L("Gizmo move")},
            {"S", L("Gizmo scale")},
            {"R", L("Gizmo rotate")},
            {"C", L("Gizmo cut")},
        ...
        m_full_shortcuts.push_back({{_L("Prepare"), ""}, plater_shortcuts});
        ...
        m_full_shortcuts.push_back(
            {{_L("Gizmos"), _L("The following shortcuts are applicable when the specified gizmo is active")}, gizmos_shortcuts});
    ...
    Shortcuts preview_shortcuts = {
        {(" ↑"), L("Vertical slider - Move active thumb Up")},
        {(" ↓"), L("Vertical slider - Move active thumb Down")},
        {(" ←"), L("Horizontal slider - Move active thumb Left")},
        {(" →"), L("Horizontal slider - Move active thumb Right")},
        ...
        {"Shift+", L("Press to speed up 5 times while moving thumb with arrow keys or mouse wheel")},
        {ctrl, L("Press to speed up 5 times while moving thumb with arrow keys or mouse wheel")},
    };

    m_full_shortcuts.push_back({{_L("Preview"), ""}, preview_shortcuts});

   // Shortcuts layers_slider_shortcuts = {
   //     {(" ↑"), L("Move active thumb Up")},
   //     {(" ↓"), L("Move active thumb Down")},
   //     {(" ←"), L("Set upper thumb as active")},
   //     {(" →"), L("Set lower thumb as active")},
   ...
   ///* m_full_shortcuts.push_back(
   //     {{_L("Slider"), _L("When the slider is active, the following shortcuts are available for G-code previews")},
   //      layers_slider_shortcuts});*/

   // Shortcuts sequential_slider_shortcuts = {
   //     {(" ←"), L("Move active thumb Left")},
   //     {(" →"), L("Move active thumb Right")},
   //     {(" ↑"), L("Set left thumb as active")},
   //     {(" ↓"), L("Set right thumb as active")},
   //     {"Shift+", L("Press to speed up 5 times while moving thumb\nwith arrow keys or mouse wheel")},
   //     {ctrl, L("Press to speed up 5 times while moving thumb\nwith arrow keys or mouse wheel")},
   // };

   // /*m_full_shortcuts.push_back(
   //     {{_L("Horizontal Slider"), _L("The following shortcuts are applicable in G-code preview when the horizontal slider is active")},
...
wxPanel *KBShortcutsDialog::create_header(wxWindow *parent, const wxFont &bold_font)
{
    ...
    // logo
    // m_header_bitmap = new wxStaticBitmap(panel, wxID_ANY, *get_bmp_bundle("flsun", 32));
