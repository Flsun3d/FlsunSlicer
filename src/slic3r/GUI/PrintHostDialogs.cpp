    ...
    , m_paths(storage_paths)
{
    ...
    auto *label_group = new wxStaticText(this, wxID_ANY, _L("Select upload printer:"));
    content_sizer->Add(label_group);
    combo_printer = new wxComboBox(this, wxID_ANY);

    content_sizer->Add(combo_printer, 0, wxEXPAND);
    ...
    const std::string path1    = wxStandardPaths::Get().GetUserDataDir().ToUTF8().data();
    //const std::string path1    = resources_dir() + "/data/";
    ConfigIni *       m_config = new ConfigIni(path1 + "/ipconfig.ini");
    m_config->GetNodeValue("printerIP", m_name_and_ip_map);
    combo_printer->SetEditable(false); // 设置为不可编辑
    if (m_name_and_ip_map.size() == 0) {
        combo_printer->AppendString(_L("No printer is currently added"));
        btn_ok->Enable(false);
    }
    for (auto it = m_name_and_ip_map.begin(); it != m_name_and_ip_map.end();) {
        // 获取当前键和值
        auto         value  = it->second;
        std::string newKey = wxGetApp().symbol_turn_dot(it->first.ToStdString());
        combo_printer->AppendString(newKey);
        it = m_name_and_ip_map.erase(it);
        m_name_and_ip_map[newKey] = value;
    }      
    if (ip.empty()) {
        combo_printer->SetSelection(0);
        select_ip                = m_name_and_ip_map[combo_printer->GetValue()].ToStdString();
    } else {
        for (auto iter : m_name_and_ip_map) {
            if (iter.second == ip) {
                combo_printer->SetValue(iter.first);
                select_ip = ip;
                break;
            }
        }
    }
    combo_printer->Bind(wxEVT_COMBOBOX, [=](wxCommandEvent &event) {
        select_ip                = m_name_and_ip_map[event.GetString()].ToStdString();
        ip        = select_ip;
    });
    ...
    if (post_actions.has(PrintHostPostUploadAction::StartPrint)) {
        ...
        btn_print->Enable(btn_ok->IsEnabled());
        btn_print->Bind(wxEVT_BUTTON, [this, validate_path](wxCommandEvent&) {
            if (validate_path(txt_filename->GetValue())) {
