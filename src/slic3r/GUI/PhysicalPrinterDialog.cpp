///|/ Copyright (c) Flsun Research 2020 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena
///|/
...
void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup* m_optgroup)
{
    ...
    if (printhost_field)
    {
        ...
        if (temp) {
            temp->Bind(wxEVT_TEXT, ([=](wxEvent& e)
            {
#ifndef __WXGTK__
                e.Skip();
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
    ...
    for (PresetForPrinter* prstft : m_presets) {
        ...
        if (Preset* preset = wxGetApp().preset_bundle->printers.find_preset(preset_name)) {
            ...
            if (preset->vendor) {
                if (preset->vendor->name == "Flsun Research") {
    ...
    for (PresetForPrinter* prstft : m_presets) {
        ...
        std::string model_id = preset->config.opt_string("printer_model");
        if (preset->vendor && preset->vendor->name != "Flsun Research") {
            connect.supported = false;
            break;
        }
        if (preset->vendor && preset->vendor->name != "Flsun Research") {
