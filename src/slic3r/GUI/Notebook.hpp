...
public:
    ...
    std::vector<TopButton *> m_pageButtons;

private:
    bool             deaful_mode = true;
    wxWindow *       m_parent;
...
public:
                ...
                bool           add_mode_buttons = false)
    {
        ...
        mainSizer->Add(m_controlSizer, flags.Border(wxALL, 0));
        SetSizer(mainSizer);
    ...
    std::vector<TopButton *> *m_buttons;

    void slicemow()
    {
        for (auto iter : dynamic_cast<ButtonsListCtrl *>(m_bookctrl)->m_pageButtons) {
            if (iter->GetLabel() == _L("Preview")) {
                iter->SetForegroundColour(Slic3r::GUI::wxGetApp().m_color_font_focus);
                iter->SetBackgroundColour(Slic3r::GUI::wxGetApp().m_color_top_bg);
            } else {
    ...
    void OnColorsChanged() { GetBtnsListCtrl()->OnColorsChanged(); }

    void UpdateModeMarkers() { GetBtnsListCtrl()->UpdateModeMarkers(); }
