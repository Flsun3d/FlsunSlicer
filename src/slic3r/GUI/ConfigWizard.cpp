///|/ Copyright (c) Prusa Research 2018 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Vojtěch Bubník @bubnikv, Lukáš Matěna
///@lukasmatena, Enrico Turri @enricoturri1966, Lukáš Hejl @hejllukas, Vojtěch Král @vojtechkral
///|/
...
bool Bundle::load(fs::path source_path, BundleLocation location, bool ais_prusa_bundle)
{
    ...
    auto [config_substitutions, presets_loaded] = preset_bundle->load_configbundle(path_string,
...
BundleMap BundleMap::load()
{
    ...
    // Should we concider case where archive has older profiles than resources (shouldnt happen)? -> YES, it happens during re-configuration
    // when running older PS after newer version
        ...
        for (const auto &dir_entry : boost::filesystem::directory_iterator(dir.first)) {
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                if (!boost::filesystem::exists(idx_path)) {
                    BOOST_LOG_TRIVIAL(error) << format("Missing index %1% when loading bundle %2%. Going to search for it in cache folder.",
                                                       idx_path.string(), id);
                ...
                if (recommended_it == index.end()) {
                    BOOST_LOG_TRIVIAL(error) << format("Could not load bundle %1% due to no recommended version in index %2%.", id,
                                                       idx_path.string());
                    continue;
                }
                ...
                try {
                    ...
                    BOOST_LOG_TRIVIAL(error) << format("Could not load bundle %1% due to corrupted profile file %2%. Message: %3%", id,
...
const Bundle &BundleMap::prusa_bundle() const { return const_cast<BundleMap *>(this)->prusa_bundle(); }
    ...
    : wxPanel(parent), vendor_id(vendor.id), width(0)
{
    ...
    for (const auto &model : models) {
        ...
        bool found = false;
        for (const fs::path &res : {rsrc_dir_path / vendor.id / model.thumbnail, vendor_dir_path / vendor.id / model.thumbnail,
        ...
        if (!found) {
            ...
            load_bitmap(wxString::FromUTF8(Slic3r::var(PRINTER_PLACEHOLDER).c_str()), bitmap, bitmap_width);
        }
        ...
        for (size_t i = 0; i < model.variants.size(); i++) {
            ...
            const auto label = model.technology == ptFFF ? format_wxstr("%1% %2% %3%", variant.name, _L("mm"), _L("nozzle")) :
                                                           from_u8(model.name);
            ...
            variants_sizer->Add(cbox, 0, /*wxBOTTOM | */wxLEFT, 40);
            //variants_sizer->Add(3, 3);
    ...
    if (titles.size() > 0) {
        ...
        for (size_t i = 0; i < titles.size() - odd_items; i += cols) {
            for (size_t j = i; j < i + cols; j++) {
                printer_grid->Add(bitmaps[j], 0, wxBOTTOM, 5);
            }
            for (size_t j = i; j < i + cols; j++) {
                printer_grid->Add(titles[j], 0, wxLEFT, 40);
            }
            for (size_t j = i; j < i + cols; j++) {
    ...
    : wxPanel(parent->p->hscroll), parent(parent), shortname(std::move(shortname)), indent(indent)
{
    ...
    if (title != "FLSUN") 
    {
        ...
        select_all_bnt = new wxButton(this, wxID_ANY, _("Select all"));
        cancel_all     = new wxButton(this, wxID_ANY, _("Cancel all"));
#ifdef WIN32
        const auto bnt_font = GetFont().Scaled(1.5);
        select_all_bnt->SetFont(bnt_font);
        cancel_all->SetFont(bnt_font);

        select_all_bnt->SetWindowStyle(wxBORDER_NONE); // 移除边框样式
        cancel_all->SetWindowStyle(wxBORDER_NONE); // 移除边框样式
        auto background_colour = this->GetBackgroundColour();
        select_all_bnt->SetBackgroundColour(background_colour);
        cancel_all->SetBackgroundColour(background_colour);
        select_all_bnt->SetForegroundColour(Slic3r::GUI::wxGetApp().m_color_font_focus); // 设置按钮的文本和颜色
        cancel_all->SetForegroundColour(Slic3r::GUI::wxGetApp().m_color_font_focus); // 设置按钮的文本和颜色
#endif                                                                                     
        auto *topsizer   = new wxBoxSizer(wxHORIZONTAL);
        topsizer->Add(text, 1, wxEXPAND, 0);
        topsizer->AddStretchSpacer();
        topsizer->Add(select_all_bnt, 0, wxEXPAND | wxRIGHT /*| wxALIGN_RIGHT*/, 0);
        topsizer->Add(cancel_all, 0, wxEXPAND | wxLEFT, 10);
        sizer->Add(topsizer, 0, wxEXPAND, 0);
    }
    //sizer->AddSpacer(10);
...
void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason)
{
    if (is_primary_printer_page && (run_reason == ConfigWizard::RR_DATA_EMPTY || run_reason == ConfigWizard::RR_DATA_LEGACY) &&
        printer_pickers.size() > 0 && printer_pickers[0]->vendor_id == PresetBundle::PRUSA_BUNDLE) {
        for (size_t i = 0; i < printer_pickers.size(); i++) {
            for (size_t j = 0; j < printer_pickers[i]->cboxes.size(); j++) {
                printer_pickers[i]->select_one(j, true);
            }
        }
    }
}
...
PageMaterials::PageMaterials(ConfigWizard *parent, Materials *materials, wxString title, wxString shortname, wxString list1name)
    ...
    , materials(materials)
    , list_printer(new StringList(this, wxLB_SINGLE))
    ...
    , list_profile(new PresetList(this))
{
    ...
    list_type->SetMinSize(wxSize(23 * em, list_h));
    ...
    grid = new wxFlexGridSizer(3, em / 2, em);
...
void PageMaterials::update_lists(int sel_type, int sel_vendor, int last_selected_printer /* = -1*/)
{
    ...
    if (!are_equal(sel_printers, sel_printers_prev)) {
        ...
        if (sel_printers_count > 1) {
            ...
            if (materials->technology == T_FFF && templates_available && (sel_printers[0] == 1 || sel_printers[1] == 1) &&
                sel_printers_count > 1) {
        ...
        if (sel_printers_count > 0 && sel_printers[0] != 0 &&
            ((materials->technology == T_FFF && templates_available && sel_printers[0] != 1) || materials->technology != T_FFF ||
             !templates_available)) {
    ...
    if (material_type_ordering) {
        ...
        std::sort(other_profiles.begin(), other_profiles.end(),
                  [](const std::pair<std::wstring, std::reference_wrapper<const std::string>> &a,
                     const std::pair<std::wstring, std::reference_wrapper<const std::string>> &b) { return a.first < b.first; });
    }
...
void PageMaterials::sort_list_data(PresetList *list, const std::vector<ProfilePrintData> &data)
{
    ...
    for (size_t i = 0; i < prusa_profiles.size(); ++i) {
        list->append(std::string(prusa_profiles[i].name) + (prusa_profiles[i].omnipresent || template_shown ? "" : ""),
    ...
    for (size_t i = 0; i < other_profiles.size(); ++i) {
        list->append(std::string(other_profiles[i].second.name) + (other_profiles[i].second.omnipresent || template_shown ? "" : ""),
...
void PageMaterials::select_material(int i)
{
    ...
    if (checked && template_shown && !notification_shown) {
        ...
        wxString message   = _L(
            "You have selected template filament. Please note that these filaments are available for all printers but are NOT certain to "
            "be compatible with your printer. Do you still wish to have this filament selected?\n(This message won't be displayed again.)");
...
PageDownloader::PageDownloader(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Downloads from URL"), _L("Downloads"))
{
    ...
    // append info line with link on printables.com
    {
        ...
        html_window->SetPage(format_wxstr("<html><body bgcolor=%1% link=%2%>"
                                          ...
                                          "</body></html>",
                                          bgr_clr_str, text_clr_str, _L("Temporarily unavailable for use")
                                          //, main_text
    ...
    append_text(wxString::Format(
        _L("On Linux systems the process of registration also creates desktop integration files for this version of application.")));
#endif
...
bool PageDownloader::on_finish_downloader() const { return m_downloader->on_finish(); }
...
bool DownloaderUtils::Worker::perform_register(const std::string &path_override /* = {}*/)
{
    ...
    // The url will always trigger opening of prusaslicer and we have to check that user has allowed it. (GUI_App::MacOpenURL is the
    // triggered method)
#else
...
{
    cb_3mf = new wxCheckBox(this, wxID_ANY, _L("Associate .3mf files to FlsunSlicer"));
    cb_stl = new wxCheckBox(this, wxID_ANY, _L("Associate .stl files to FlsunSlicer"));
    //    cb_gcode = new wxCheckBox(this, wxID_ANY, _L("Associate .gcode files to PrusaSlicer G-code Viewer"));
    cb_3mf->SetValue(true);
    cb_stl->SetValue(true);
...
{
    append_text(_L("FlsunSlicer's user interfaces comes in three variants:Simple, Advanced, and Expert.\n"
...
PageVendors::PageVendors(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Other Vendors"), _L("Other Vendors"))
{
    ...
    std::sort(vendors.begin(), vendors.end(),
              [](const std::pair<std::wstring, const VendorProfile *> &a, const std::pair<std::wstring, const VendorProfile *> &b) {
                  return a.first < b.first;
              });

    for (const std::pair<std::wstring, const VendorProfile *> &v : vendors) {
        const VendorProfile *vendor = v.second;
        if (vendor->id == PresetBundle::PRUSA_BUNDLE) {
            continue;
        }
        ...
        cbox->Bind(wxEVT_CHECKBOX, [=](wxCommandEvent &event) { wizard_p()->on_3rdparty_install(vendor, cbox->IsChecked()); });
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "flsun_2.png", 100))
...
void ConfigWizard::priv::load_pages()
{
    ...
    if (!only_sla_mode) {
        ...
        std::sort(sorted_vendors.begin(), sorted_vendors.end(),
                  [](const std::pair<std::wstring, std::string> &a, const std::pair<std::wstring, std::string> &b) {
                      return a.first < b.first;
                  });
...
void ConfigWizard::priv::init_dialog_size()
{
    ...
    const int width_hint = index->GetSize().GetWidth() +
                           std::max(90 * em(), (/*only_sla_mode ? page_msla->get_width() :*/ page_fff->get_width()) +
                                                   30 * em()); // XXX: magic constant, I found no better solution
    ...
    if (!app_config->legacy_datadir()) {
        ...
        for (auto &dir_entry : boost::filesystem::directory_iterator(printer_dir))
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                appconfig_new.set_variant("FLSun", model, variant, true);
            }
    }
                                                                     ...
                                                                     const std::set<const VendorProfile::PrinterModel *> &printer_models)
{
    ...
    /*
    auto select_default_materials_for_printer_page = [this, appconfig_section, printer_models, technology](PagePrinters *page_printers,
    Technology technology)
    {
                    ...
                    if ((only_for_model_id.empty() || only_for_model_id == printer_model->id) &&
                        ...
                        for (const auto &preset : appconfig_presets) {
                            if (preset.second == "1") {
                                ...
                                // find if preset.first is part of the templates profile (up is searching if preset.first is part of printer
                                // vendor preset)
                                if (!no_templates) {
                                    for (const auto &bp : bundles) {
                                        if (!bp.second.preset_bundle->vendors.empty() &&
                                            ...
                                            if (template_material &&
                                                is_compatible_with_printer(PresetWithVendorProfile(*template_material,
                                                                                                   &bp.second.preset_bundle->vendors
                                                                                                        .begin()
                                                                                                        ->second),
                                                                           PresetWithVendorProfile(printer, nullptr))) {
                                                has_material = true;
                                      ...
                                      bool &               apply_keeped_changes)
{
    ...
    auto get_preferred_printer_technology  = [enabled_vendors, enabled_vendors_old, suppress_sla_printer](const std::string &bundle_name,
                                                                                                         const Bundle &     bundle) {
    ...
    if (preferred_pt = get_preferred_printer_technology("FLSun", bundles.prusa_bundle());
        preferred_pt == ptAny || (preferred_pt == ptSLA && suppress_sla_printer)) {
        for (const auto &bundle : bundles) {
            if (bundle.second.is_prusa_bundle) {
                continue;
            }
    ...
    BOOST_LOG_TRIVIAL(debug) << "ConfigWizard::priv::apply_config integrate_desktop" << page_welcome->integrate_desktop()
                             << " perform_registration_linux " << page_downloader->m_downloader->get_perform_registration_linux();
    ...
    // Prusa printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model("FLSun", bundles.prusa_bundle(), preferred_variant); preferred_model.empty()) {
        for (const auto &bundle : bundles) {
            if (bundle.second.is_prusa_bundle) {
                continue;
            }
    ...
    , p(new priv(this))
{
    ...
    const auto prusa_it = p->bundles.find("FLSun");
    wxCHECK_RET(prusa_it != p->bundles.cend(), "Vendor FLSun not found");
    const VendorProfile *vendor_prusa = prusa_it->second.vendor_profile;

    p->add_page(p->page_welcome = new PageWelcome(this));

    p->page_fff      = new PagePrinters(this, _L("FLSUN"), "FLSUN", *vendor_prusa, 0, T_FFF);
    p->page_fff->select_all_bnt->Bind(wxEVT_BUTTON, [=](const wxCommandEvent &) { p->page_fff->select_all(true, false); });
    p->page_fff->cancel_all->Bind(wxEVT_BUTTON, [=](const wxCommandEvent &) { p->page_fff->select_all(false, false); });
    ...
    if (!p->only_sla_mode)
        p->add_page(
            p->page_filaments = new PageMaterials(this, &p->filaments, _L("Filament Profiles Selection"), _L("Filaments"), _L("Type:")));
...
bool ConfigWizard::run(RunReason reason, StartPage start_page, bool is_config_left)
{
