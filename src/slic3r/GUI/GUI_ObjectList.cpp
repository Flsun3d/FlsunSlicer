...
#endif /* __WXMSW__ */
#include <slic3r/GUI/GalleryDialog.cpp>
...
void ObjectList::create_objects_ctrl()
{
    ...
    wxDataViewColumn *name = new wxDataViewColumn(_L("Name"), bmp_text_renderer, colName, 20 * em, wxALIGN_LEFT, wxDATAVIEW_COL_RESIZABLE);
    name->SetMinWidth(50); // 设置最小宽度为100像素
    AppendColumn(name);

    // column PrintableProperty (Icon) of the view control:
    wxDataViewColumn *notitle = AppendBitmapColumn(" ", colPrint, wxDATAVIEW_CELL_INERT, 3 * em, wxALIGN_CENTER_HORIZONTAL);
    notitle->SetMinWidth(30); // 设置最小宽度为100像素
    ...
    wxDataViewColumn *column = AppendBitmapColumn(_L("Operation"), colEditing, wxDATAVIEW_CELL_INERT, 3 * em, wxALIGN_CENTER_HORIZONTAL);
    column->SetMinWidth(50); // 设置最小宽度为100像素
...
static TriangleMesh create_mesh(const std::string& type_name, const BoundingBoxf3& bb)
{
    ...
    else if (type_name == "Boat") {
        wxArrayString input_files;
        input_files.Add(from_u8(get_dir_path(true) + L("Boat.stl")));
        wxGetApp().obj_list()->load_shape_object_from_gallery(input_files);
...
void ObjectList::load_shape_object_from_gallery(const wxArrayString& input_files)
{
    ...
    wxString snapshot_label = (paths.size() == 1 ? _L("Add Shape from Gallery") : _L("Add Shapes from Gallery")) + ": " +
                              _L(wxString::FromUTF8(paths.front().filename().string().c_str()));
