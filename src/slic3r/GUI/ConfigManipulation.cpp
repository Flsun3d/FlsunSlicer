...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config)
{
    ...
    auto max_lh = gpreset.config.opt_float("max_layer_height", 0) > 0 ? gpreset.config.opt_float("max_layer_height", 0) :
                                                                        gpreset.config.opt_float("nozzle_diameter", 0)*0.75;
    ...
    if (layer_height < min_lh && layer_height > EPSILON)
    {
        const wxString     msg_text = wxString::Format(L("The layer height is too small.\nReset to %0.3f"), min_lh);
        MessageDialog      dialog(nullptr, msg_text, "", wxICON_WARNING | wxOK);
        DynamicPrintConfig new_conf = *config;
        is_msg_dlg_already_exist    = true;
        dialog.ShowModal();
...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig* config)
{
    ...
    for (auto el : { "extra_perimeters","extra_perimeters_on_overhangs", "thin_walls", "overhangs",
                    "seam_position","staggered_inner_seams", "external_perimeters_first", "external_perimeter_extrusion_width", "perimeter_speed",
                    "small_perimeter_speed", "external_perimeter_speed", "enable_dynamic_overhang_speeds", "enable_inner_dynamic_overhang_speeds"})
        toggle_field(el, have_perimeters);

    for (size_t i = 0; i < 4; i++) {
        toggle_field("overhang_speed_" + std::to_string(i), config->opt_bool("enable_dynamic_overhang_speeds"));
    }
    toggle_field("enable_inner_dynamic_overhang_speeds", config->opt_bool("enable_dynamic_overhang_speeds"));
    ...
    bool have_accel_to_decel_enable = config->opt_bool("accel_to_decel_enable");
     for (auto el : {
        ...
        toggle_field(el, have_accel_to_decel_enable);
    ...
    for (auto el :
         {"raft_expansion", "first_layer_acceleration_over_raft", "first_layer_deceleration_over_raft", "first_layer_speed_over_raft"})
   ...
   /* bool have_accel_to_decel_enable = config->opt_bool("accel_to_decel_enable");
    toggle_field("accel_to_decel_factor", have_accel_to_decel_enable);
    toggle_field("set_deceleration_acceleration", !have_accel_to_decel_enable);*/
