...
static const Slic3r::ColorRGBA DEFAULT_BG_DARK_COLOR  = {0.89f, 0.89f, 0.88f, 1.0f};
static const Slic3r::ColorRGBA DEFAULT_BG_LIGHT_COLOR = {0.89f, 0.89f, 0.88f, 1.0f};
...
void GLCanvas3D::LayersEditing::render_overlay(const GLCanvas3D& canvas)
{
    ...
    imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, _L("Left mouse button:"));
    ImGui::SameLine();
    imgui.text(_L("Add detail"));

    imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, _L("Right mouse button:"));
    ImGui::SameLine();
    imgui.text(_L("Remove detail"));

    imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, _L("Shift + Left mouse button:"));
    ImGui::SameLine();
    imgui.text(_L("Reset to base"));

    imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, _L("Shift + Right mouse button:"));
    ImGui::SameLine();
    imgui.text(_L("Smoothing"));

    imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, _L("Mouse wheel:"));
    ImGui::SameLine();
...
void GLCanvas3D::SLAView::render_debug_window()
{
    ...
    for (const auto& [id, type] : m_instances_cache) {
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "(" + std::to_string(id.object_id) + ", " + std::to_string(id.instance_id) + ")");
        ImGui::SameLine();
...
void display()
{
    glViewport(0, 0, (GLsizei) 500, (GLsizei) 500); // 设置视口大小
    glMatrixMode(GL_PROJECTION);                         // 选择投影矩阵
    glLoadIdentity();                                    // 重置投影矩阵为单位矩阵
    gluOrtho2D(-1.0, 1.0, -1.0 * (GLfloat) 500 / (GLfloat) 500,
               1.0 * (GLfloat) 500 / (GLfloat) 500); // 设置2D正交投影，考虑窗口的宽高比  
    //glsafe(::glColor3f(1.0, 0.0, 0.0)); // 设置颜色为红色
    ::glBegin(GL_LINES);
    ::glVertex2f(0.5, 0.0); // 线的起点
    ::glVertex2f(-0.5, 0.0);  // 线的终点
    ::glEnd();
    //glsafe(::glFlush());
}
...
void GLCanvas3D::on_char(wxKeyEvent& evt)
{
    ...
    if ((evt.GetModifiers() & ctrlMask) != 0) {
        ...
        case '0': {
            select_view("iso");
            for (auto iter : wxGetApp().plater()->get_view_toolbar().m_items) {
                iter->set_state(GLToolbarItem::Normal);
            }
            wxGetApp().plater()->get_view_toolbar().m_items[0]->set_state(GLToolbarItem::Pressed);
            break;
        }
        case '1': {
            select_view("top");
            ...
            wxGetApp().plater()->get_view_toolbar().m_items[1]->set_state(GLToolbarItem::Pressed);
            break;
        }
        case '2': {
            select_view("front");
            ...
            wxGetApp().plater()->get_view_toolbar().m_items[2]->set_state(GLToolbarItem::Pressed);
            break;
        }
        ...
        case '3': {
            ...
            wxGetApp().plater()->get_view_toolbar().m_items[3]->set_state(GLToolbarItem::Pressed);
            break;
        }
        case '4': {
            select_view("right");
            ...
            wxGetApp().plater()->get_view_toolbar().m_items[4]->set_state(GLToolbarItem::Pressed);
            break;
        }
        case wxKeyCode::WXK_F11: { // F11快捷键
            ...
            if (evt.GetEventType() == wxEVT_KEY_UP) {
                ...
                if (m_tab_down && keyCode == WXK_TAB && !evt.HasAnyModifiers()) {
                    ...
                    post_event(SimpleEvent(EVT_GLCANVAS_TAB));
                           ...
                           evt.GetModifiers() == wxMOD_SHIFT) { // Collapse side-panel with Shift+Tab
...
//    m_main_toolbar.set_layout_type(GLToolbar::Layout::Vertical);
    ...
    item.left.action_callback = [this]() {
        if (m_canvas != nullptr)
            wxPostEvent(m_canvas, SimpleEvent(EVT_GLTOOLBAR_SEARCH));
    };
    ...
    item.tooltip = _u8L("Switch to Settings") + "\n" + "[" + GUI::shortkey_ctrl_prefix() + "2] - " + _u8L("Print Settings Tab") + "\n" +
                   ...
                   (current_printer_technology() == ptFFF ?
                        _u8L("Filament Settings Tab") :
                        _u8L("Material Settings Tab") + "\n" + "[" + GUI::shortkey_ctrl_prefix() + "4] - " + _u8L("Printer Settings Tab"));
    item.sprite_id           = 9;
    item.enabling_callback   = GLToolbarItem::Default_Enabling_Callback;
    item.visibility_callback = []() {
        return wxGetApp().app_config->get_bool("new_settings_layout_mode") || wxGetApp().app_config->get_bool("dlg_settings_layout_mode");
    };
...
void GLCanvas3D::_picking_pass()
{
    ...
    if (hit.type != SceneRaycaster::EType::None) {
        if (ImGui::BeginTable("Hit", 2)) {
            add_strings_row_to_table("Object ID", ImGuiWrapper::COL_GREEN_LIGHT, std::to_string(hit.raycaster_id), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table("Type", ImGuiWrapper::COL_GREEN_LIGHT, object_type, ImGui::GetStyleColorVec4(ImGuiCol_Text));
            sprintf(buf, "%.3f, %.3f, %.3f", hit.position.x(), hit.position.y(), hit.position.z());
            add_strings_row_to_table("Position", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            sprintf(buf, "%.3f, %.3f, %.3f", hit.normal.x(), hit.normal.y(), hit.normal.z());
            add_strings_row_to_table("Normal", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            ImGui::EndTable();
        }
    }
    else
    ...
    if (ImGui::BeginTable("Raycasters", 2)) {
        ...
        add_strings_row_to_table("Beds", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        sprintf(buf, "%d (%d)", (int)m_scene_raycaster.volumes_count(), (int)m_scene_raycaster.active_volumes_count());
        add_strings_row_to_table("Volumes", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        sprintf(buf, "%d (%d)", (int)m_scene_raycaster.gizmos_count(), (int)m_scene_raycaster.active_gizmos_count());
        add_strings_row_to_table("Gizmo elements", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        sprintf(buf, "%d (%d)", (int)m_scene_raycaster.fallback_gizmos_count(), (int)m_scene_raycaster.active_fallback_gizmos_count());
        add_strings_row_to_table("Gizmo2 elements", ImGuiWrapper::COL_GREEN_LIGHT, std::string(buf), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }
    ...
    if (gizmo_raycasters != nullptr && !gizmo_raycasters->empty()) {
        ...
        if (ImGui::BeginTable("GizmoRaycasters", 3)) {
            for (size_t i = 0; i < gizmo_raycasters->size(); ++i) {
                add_strings_row_to_table(std::to_string(i), ImGuiWrapper::COL_GREEN_LIGHT,
...
m_gizmos.set_overlay_scale(0.7 * scale); //! #ys_FIXME_experiment
#else
//     m_gizmos.set_overlay_scale(m_canvas->GetContentScaleFactor());
//     m_gizmos.set_overlay_scale(wxGetApp().em_unit()*0.1f);
    const float size = int(GLGizmosManager::Default_Icons_Size * wxGetApp().toolbar_icon_scale());
m_gizmos.set_overlay_icon_size(size * 0.7); //! #ys_FIXME_experiment
#endif /* __WXMSW__ */
...
void GLCanvas3D::_render_collapse_toolbar() const
{
    GLToolbar & collapse_toolbar = wxGetApp().plater()->get_collapse_toolbar();
    const float size             = int(GLGizmosManager::Default_Icons_Size * wxGetApp().toolbar_icon_scale());
    collapse_toolbar.set_icons_size(size *0.7); //设置侧边栏大小
    const Size  cnv_size = get_canvas_size();
    const float band = m_layers_editing.is_enabled() ? (wxGetApp().imgui()->get_style_scaling() * LayersEditing::THICKNESS_BAR_WIDTH) : 0.0;
    float       top  = 0.5f * (float) cnv_size.get_height();
    const float left = 0.5f * (float) cnv_size.get_width() - collapse_toolbar.get_width() - band;
    auto        posType = collapse_toolbar.get_vertical_orientation();
    if (posType == 1u) // set Toolbar on vertical center
    {
        top = 0.5f * collapse_toolbar.get_height();
    }
...
void GLCanvas3D::_render_view_toolbar() const
{
    ...
    const float scale = m_retina_helper->get_scale_factor() * wxGetApp().toolbar_icon_scale();
#if __APPLE__
    view_toolbar.set_scale(0.7 * scale);
#else // if GTK3
...
#else
    const float size = int(GLGizmosManager::Default_Icons_Size * wxGetApp().toolbar_icon_scale());
    view_toolbar.set_icons_size(size *0.7); //设置视图大小
#endif // ENABLE_RETINA_GL
...
void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
{
    ...
    case EWarning::GCodeConflict: {
        ...
        text = _u8L("Conflicts in G-code paths have been detected at layer 0, z=0.0f mm. Please reposition the conflicting "
                    "objects (%3% <-> %4%) further apart."),
        layer, height, _L(objName1), _L(objName2);
        break;
    }
    }
...
void GLCanvas3D::show_binary_gcode_debug_window()
{
    ...
    if (ImGui::BeginTable("BinaryGCodeConfig", 2)) {
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "File metadata compression");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "Printer metadata compression");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "Print metadata compression");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "Slicer metadata compression");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "GCode compression");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "GCode encoding");
        ImGui::TableSetColumnIndex(1);
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "Metadata encoding");
        ImGui::TableSetColumnIndex(1);
        options = { "INI" };
        ...
        ImGui::TableNextRow();
        ImGui::TableSetColumnIndex(0);
        imgui.text_colored(ImGuiWrapper::COL_GREEN_LIGHT, "Checksum type");
        ImGui::TableSetColumnIndex(1);
        options = { "None", "CRC32" };
