///|/ Copyright (c) Prusa Research 2018 - 2023 Vojtěch Bubník @bubnikv, Oleksandra Iushchenko @YuSanka, Tomáš Mészáros @tamasmeszaros, David
///Kocík @kocikdav, Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Filip Sykala @Jony01, Lukáš Hejl @hejllukas, Vojtěch Král
///@vojtechkral
...
public:
    ...
    wxColour    m_color_font_focus;
    bool        is_config_left = false;
    EAppMode    get_app_mode() const { return m_app_mode; }
    bool        is_editor() const { return m_app_mode == EAppMode::Editor; }
    bool        is_gcode_viewer() const { return m_app_mode == EAppMode::GCodeViewer; }
    bool        is_recreating_gui() const { return m_is_recreating_gui; }
    std::string logo_name() const { return is_editor() ? "flsun_2" : "flsun"; }
    ...
    //将.转为特殊符号
    std::string dot_turn_symbol(std::string lable_name);

    //将特殊符号转为.
    std::string symbol_turn_dot(std::string lable_name);
    ...
    Tab *get_dialog_tab(Preset::Type type);

    ConfigOptionMode get_mode();
    wxString         get_mode_defaul();
