///|/ Copyright (c) Prusa Research 2018 - 2023 Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena, Oleksandra Iushchenko @YuSanka, Enrico
///Turri @enricoturri1966, Tomáš Mészáros @tamasmeszaros, David Kocík @kocikdav, Lukáš Hejl @hejllukas, Pavel Mikuš @Godrak, Filip Sykala
///@Jony01, Vojtěch Král @vojtechkral
///|/ Copyright (c) 2022 Michael Kirsch
///|/ Copyright (c) 2021 Boleslaw Ciesielski
///|/ Copyright (c) 2019 John Drake @foxox
///|/
///|/ ported from lib/Slic3r/GUI/Plater.pm:
///|/ Copyright (c) Prusa Research 2016 - 2019 Vojtěch Bubník @bubnikv, Vojtěch Král @vojtechkral, Enrico Turri @enricoturri1966, Oleksandra
///Iushchenko @YuSanka, Lukáš Matěna @lukasmatena, Tomáš Mészáros @tamasmeszaros
...
#include "Gizmos/GLGizmoSimplify.hpp" 
#include "Gizmos/GLGizmoSVG.hpp" // Drop SVG file// create suggestion notification
...
// BackgroundSlicingProcess updates UI with slicing progress: Status bar / progress bar has to be updated, possibly scene has to be
// refreshed, see PrintBase::SlicingStatus for the content of the message.
...
FreqChangedParams::FreqChangedParams(wxWindow *parent) : OG_Settings(parent, false)
{
    ...
    m_og->m_on_change = [config, this](t_config_option_key opt_key, boost::any value) {
        ...
        if (opt_key == "fill_density" || opt_key == "brim_type") {
            ...
            if (opt_key == "brim") {
                ...
                if (boost::any_cast<bool>(value) == true) {
                    ...
                    new_conf.set_key_value("brim_type", new ConfigOptionEnum<BrimType>(btOuterOnly));

                } else {
                    m_brim_width = brim_width * (-1);
                    new_val      = 0;
                    new_conf.set_key_value("brim_type", new ConfigOptionEnum<BrimType>(btNoBrim));
                }
    ...
    support_def.set_enum_labels(ConfigOptionDef::GUIType::select_close,
                                {L("None"), L("Support on build plate only"), L("For support enforcers only"), L("Everywhere")});
   ...
   /* option              = m_og->get_option("fill_density");
    ...
    auto empty_widget = [this](wxWindow *parent) {
        ...
        auto btn   = new ScalableButton(parent, wxID_ANY, "mirroring_transparent", wxEmptyString, wxDefaultSize, wxDefaultPosition,
   ...
   /* auto brim_type = config->opt_enum<BrimType>("brim_type");
    m_brim_width = config->opt_float("brim_width");*/
    /*auto m_brim_type = config->opt_enum<BrimType>("brim_type") != btNoBrim;
    ConfigOptionDef def;
    def.label    = L("Brim");
    def.type     = coBool;
    def.tooltip  = L("This flag enables the brim that will be printed around each object on the first layer.");
    def.gui_type = ConfigOptionDef::GUIType::undefined;
    def.set_default_value(new ConfigOptionBool{(m_brim_width > 0.0 && m_brim_type) ? true : false});
    option              = Option(def, "brim");
    option.opt.sidetext = "";
    line.append_option(option);*/

    //auto brim_type        = config->opt_enum<BrimType>("brim_type");
     option              = m_og->get_option("brim_type");
    option.opt.label    = L("Brim");
    option.opt.full_width = true;
    //option.opt.set_default_value(new ConfigOptionEnum<BrimType>(brim_type));
    option.opt.sidetext = "   ";
    line.append_option(option);
    ...
    def.type            = coStrings;
    def.tooltip         = L("This flag enables the brim that will be printed around each object on the first layer.");
    def.set_enum_labels(ConfigOptionDef::GUIType::select_close,
                                {L("No brim"), L("Outer brim only"), L("Inner brim only"), L("Outer and inner brim")});
    def.set_default_value(new ConfigOptionStrings{L("No brim")});
    option         = Option(def, "Brim");
    option.opt.full_width = true;
    option.opt.sidetext = "";
    line.append_option(option);*/
    ...
    //def.set_enum<BrimType>({{"no_brim", L("No brim")},
    //                         {"outer_only", L("Outer brim only")},
    //                         {"inner_only", L("Inner brim only")},
    //                         {"outer_and_inner", L("Outer and inner brim")}});
    //def.set_default_value(new ConfigOptionEnum<BrimType>(brim_type));
    //option = Option(def, "Brim");
    ////option              = m_og->get_option("brim_type");
    ////option.opt.label    = L("Brim type");
    //option.opt.width    = 8;
    ////option.opt.sidetext = "   ";
    //line.append_option(option);
    ...
    auto wiping_dialog_btn = [this](wxWindow *parent) {
        m_wiping_dialog_button = new wxButton(parent, wxID_ANY, _L("Purging volumes") + dots, wxDefaultPosition, wxDefaultSize,
                                              wxBU_EXACTFIT);
        ...
        m_wiping_dialog_button->Bind(
            wxEVT_BUTTON, ([parent](wxCommandEvent &e) {
                ...
                if (dlg.ShowModal() == wxID_OK) {
                    ...
                    (project_config.option<ConfigOptionFloats>("wiping_volumes_matrix"))->values    = std::vector<double>(matrix.begin(),
                                                                                                                       matrix.end());
                    (project_config.option<ConfigOptionFloats>("wiping_volumes_extruders"))->values = std::vector<double>(extruders.begin(),
                                                                                                                          extruders.end());
   ...
   /* Choice *choice = dynamic_cast<Choice *>(m_og->get_field("support"));
    ...
    choice = dynamic_cast<Choice *>(m_og->get_field("Brim"));
    choice->suppress_scroll();*/
    ...
    m_sizer->Add(m_og->sizer, 0, wxEXPAND | wxLEFT | wxBOTTOM, 10);
    //m_sizer->Add(m_og_sla->sizer, 0, wxEXPAND /* | wxALL,20*/);
}

wxSizer *FreqChangedParams::get_sizer() { return m_sizer; }
...
ConfigOptionsGroup *FreqChangedParams::get_og(const bool is_fff) { return is_fff ? m_og.get() : m_og_sla.get(); }
...
Sidebar::Sidebar(Plater *parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    //    p->scrolled->SetScrollbars(0, 100, 1, 2); // ys_DELETE_after_testing. pixelsPerUnitY = 100 from
    ...
    // const int margin_5 = 5; // 5;
    auto init_combo = [this, margin_5](PlaterPresetComboBox **combo, wxString label, Preset::Type preset_type, bool filament) {
        auto presets_and_set_sizer = new wxBoxSizer(wxHORIZONTAL);
#ifdef WIN32
        wxPanel *text_panel = new wxPanel(p->presets_panel, wxID_ANY, wxDefaultPosition, wxDefaultSize);
        auto *   text       = new wxStaticText(text_panel, wxID_ANY, label + ":");
        //text_panel->SetBackgroundColour(wxColor(242, 242, 239));
        ScalableButton *set_btn = new ScalableButton(text_panel, wxID_ANY, "addsetting");
        text_panel->SetSizer(presets_and_set_sizer);

#else
        auto *          text    = new wxStaticText(p->presets_panel, wxID_ANY, label + ":");
        ScalableButton *set_btn = new ScalableButton(p->presets_panel, wxID_ANY, "addsetting");

#endif
        wxFont font = Slic3r::GUI::wxGetApp().normal_font();
        font.SetPointSize(12);
        text->SetFont(font);
        //text->SetForegroundColour(wxColor(20, 20, 20));

        *combo         = new PlaterPresetComboBox(p->presets_panel, preset_type);
        auto sizero = (*combo)->GetSize();
        auto old_color = set_btn->GetParent()->GetBackgroundColour();
        set_btn->Bind(wxEVT_ENTER_WINDOW, [=](wxMouseEvent &event) {
            set_btn->SetBackgroundColour(wxOSX ? wxColor(255, 255, 255) : Slic3r::GUI::wxGetApp().get_color_selected_btn_bg());
        });
        set_btn->Bind(wxEVT_LEAVE_WINDOW, [=](wxMouseEvent &event) { set_btn->SetBackgroundColour(p->presets_panel->GetBackgroundColour()); });

        if ((*combo)->get_type() == Preset::TYPE_PRINTER) {
            set_btn->SetToolTip(_L("Select printer"));
            // show_edit_menu();
        } else if ((*combo)->get_type() == Preset::TYPE_FILAMENT) {
            set_btn->SetToolTip(_L("Select consumables"));
        }
        set_btn->Bind(wxEVT_BUTTON, [=](wxCommandEvent) {
            if ((*combo)->get_type() == Preset::TYPE_FILAMENT)
                run_wizard(ConfigWizard::SP_FILAMENTS);
            else
                run_wizard(ConfigWizard::SP_PRINTERS);
        });
        int  distance            = 15;
        auto combo_and_btn_sizer = new wxBoxSizer(wxHORIZONTAL);
        combo_and_btn_sizer->Add(*combo, 1, wxEXPAND | wxLEFT, distance);
        if ((*combo)->edit_btn)
            combo_and_btn_sizer->Add((*combo)->edit_btn, 0, wxLEFT | wxRIGHT, int(/*0.*/2 * wxGetApp().em_unit()));
        auto *sizer_presets   = this->p->sizer_presets;
        auto *sizer_filaments = this->p->sizer_filaments;
        // Hide controls, which will be shown/hidden in respect to the printer technology
        presets_and_set_sizer->Add(text, 1, wxALIGN_CENTER_VERTICAL | wxLEFT, distance);
        if ((*combo)->get_type() == Preset::TYPE_PRINTER || (*combo)->get_type() == Preset::TYPE_FILAMENT) {
            presets_and_set_sizer->Add(0, 0, 1, wxEXPAND, 5);
            presets_and_set_sizer->Add(set_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT | wxRIGHT, int(/*0.*/2 * wxGetApp().em_unit()));
        }
#ifdef WIN32
        auto bnm = (*combo)->edit_btn->GetSize();
        (*combo)->edit_btn->SetMaxSize(wxSize(wxGetApp().em_unit()*3, bnm.y /*- 3*/));
        set_btn->SetMaxSize(wxSize(wxGetApp().em_unit() * 3, bnm.y /*- 3*/));
        sizer_presets->Add(text_panel, 0, wxEXPAND | wxBOTTOM | wxTOP, margin_5);
#else
        sizer_presets->Add(presets_and_set_sizer, 0, wxEXPAND | wxBOTTOM | wxTOP, margin_5);
#endif //
        if (!filament) {
             //combo_and_btn_sizer->ShowItems(preset_type == Preset::TYPE_PRINT);
            sizer_presets->Add(combo_and_btn_sizer, 0,
                               wxEXPAND |
#ifdef __WXGTK3__
                                   wxRIGHT,
                               margin_5);
            ...
            sizer_filaments->Add(combo_and_btn_sizer, 0,
                                 wxEXPAND |
#ifdef __WXGTK3__
                                     wxRIGHT,
                                 margin_5);
    ...
    p->sizer_params->Add(p->frequently_changed_parameters->get_sizer(), 0,
    ...
    is_msw ? scrolled_sizer->Add(p->presets_panel, 0, wxEXPAND | size_margin, 0) :
             scrolled_sizer->Add(p->sizer_presets, 0, wxEXPAND | size_margin, 0);
    ...
    auto init_scalable_btn = [this](ScalableButton **btn, const std::string &icon_name, wxString tooltip = wxEmptyString) {
        ...
        //*btn               = new ScalableButton(this, wxID_ANY, "", wxEmptyString, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT);
    ...
    init_scalable_btn(&p->btn_export_gcode_removable, "export_to_sd",
...
//    const int scaled_height = p->btn_eject_device->GetBitmapHeight() + 4;
...
#else
    const int scaled_height = p->btn_export_gcode_removable->GetBitmapHeight() + 14;
#endif
    auto init_btn = [this](wxButton **btn, wxString label, const int button_height) {
        *btn = new wxButton(this, wxID_ANY, label, wxDefaultPosition, wxSize(-1, button_height), wxBU_EXACTFIT | wxBORDER_NONE);
        (*btn)->SetFont(wxGetApp().bold_font());
        (*btn)->SetForegroundColour(wxColour(255, 255, 255));
        // wxGetApp().UpdateDarkUI((*btn), true);
    };
    init_btn(&p->btn_send_gcode, _L("Send G-code") + dots, scaled_height);
    ...
    complect_btns_sizer->Add(p->btn_export_gcode, 1, wxEXPAND | wxRIGHT,5);
    complect_btns_sizer->Add(p->btn_send_gcode, 1, wxEXPAND  | wxLEFT, 5);
    complect_btns_sizer->Add(p->btn_export_gcode_removable, 0, wxLEFT, 5);
    //    complect_btns_sizer->Add(p->btn_eject_device);

    btns_sizer->Add(p->btn_reslice, 1, wxEXPAND | wxRIGHT, 5);
    btns_sizer->Add(complect_btns_sizer, 1, wxEXPAND | wxTOP, 0);

    auto *sizer = new wxBoxSizer(wxVERTICAL);
    sizer->Add(p->scrolled, 1, wxEXPAND);
    sizer->Add(btns_sizer, 0, wxEXPAND | wxLEFT, 5);
    SetSizer(sizer);

    // Events
    p->btn_export_gcode->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { p->plater->export_gcode(false); });
#ifdef _WIN32
    p->btn_send_gcode->Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent &event) {
        wxString tooltip = _L("Send to printer") + " " + GUI::shortkey_ctrl_prefix() + "Shift+G";
        p->show_rich_tip(tooltip, p->btn_send_gcode);
           event.Skip();
       });
    p->btn_send_gcode->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent &event) {
        p->hide_rich_tip(p->btn_send_gcode);
           event.Skip();
       });


    p->btn_export_gcode->Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent &event) {
        wxString tooltip = _L("Export G-code") + " [" + GUI::shortkey_ctrl_prefix() + "G]";
        p->show_rich_tip(tooltip, p->btn_export_gcode);
    });

    p->btn_export_gcode->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent &event) {
        p->hide_rich_tip(p->btn_export_gcode);
        /*if (p->btn_export_gcode->IsEnabled()) {
            p->btn_export_gcode->SetBackgroundColour(wxColor(242, 242, 239));
        }*/
    });
#endif
    p->btn_reslice->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) {
        p->btn_export_gcode->Enable(false);
        p->btn_send_gcode->Enable(false);
        ...
        if (export_gcode_after_slicing)
            ...
            p->plater->reslice();
        dynamic_cast<Notebook *>(Slic3r::GUI::wxGetApp().mainframe->m_tabpanel)->slicemow();
...
void Sidebar::init_filament_combo(PlaterPresetComboBox **combo, const int extr_idx)
{
    *combo = new PlaterPresetComboBox(p->presets_panel, Slic3r::Preset::TYPE_FILAMENT);
    ...
    this->p->sizer_filaments->Add(combo_and_btn_sizer, 1,
                                  wxEXPAND |
#ifdef __WXGTK3__
                                      wxRIGHT,
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_PRINT: p->combo_print->update(); break;

    case Preset::TYPE_SLA_PRINT: p->combo_sla_print->update(); break;

    case Preset::TYPE_SLA_MATERIAL: p->combo_sla_material->update(); break;
...
void Sidebar::msw_rescale()
{
    ...
    for (PlaterPresetComboBox *combo :
         std::vector<PlaterPresetComboBox *>{p->combo_print, p->combo_sla_print, p->combo_sla_material, p->combo_printer})
        combo->msw_rescale();
...
void Sidebar::sys_color_changed()
{
    ...
    for (wxWindow *win :
         std::vector<wxWindow *>{this, p->sliced_info->GetStaticBox(), p->object_info->GetStaticBox(), p->btn_reslice, p->btn_export_gcode})
...
void Sidebar::jump_to_option(size_t selected)
{
    ...
    if (opt.type == Preset::TYPE_PREFERENCES)
        ...
        wxGetApp().get_dialog_tab(opt.type)->activate_option(opt.opt_key(), boost::nowide::narrow(opt.category));
}

ObjectManipulation *Sidebar::obj_manipul() { return p->object_manipulation; }

ObjectList *Sidebar::obj_list() { return p->object_list; }

ObjectSettings *Sidebar::obj_settings() { return p->object_settings; }

ObjectLayers *Sidebar::obj_layers() { return p->object_layers; }

wxScrolledWindow *Sidebar::scrolled_panel() { return p->scrolled; }

wxPanel *Sidebar::presets_panel() { return p->presets_panel; }

ConfigOptionsGroup *Sidebar::og_freq_chng_params(const bool is_fff) { return p->frequently_changed_parameters->get_og(is_fff); }
...
void Sidebar::show_info_sizer()
{
    ...
    if (/*m_mode < comExpert ||*/ objects.empty() || obj_idx < 0 || int(objects.size()) <= obj_idx || inst_idx < 0 ||
    ...
    if (selection.is_single_volume()) {
        ...
        if (vol_idxs.size() != 1)
            ...
            // Like after try to delete last solid part in object, the object is selected in ObjectLIst when just a part is still selected
            // on Canvas see https://github.com/prusa3d/PrusaSlicer/issues/7408
            return;
...
void Sidebar::update_sliced_info_sizer()
{
    if (p->sliced_info->IsShown(size_t(0))) {
        if (p->plater->printer_technology() == ptSLA) {
            ...
            if (is_supports)
                new_label += format_wxstr("\n    - %s\n    - %s", _L_PLURAL("object", "objects", p->plater->model().objects.size()),
                                          _L("supports and pad"));

            wxString info_text = is_supports ?
                                     wxString::Format("%.2f \n%.2f \n%.2f", (ps.objects_used_material + ps.support_used_material) / 1000,
                                                      ps.objects_used_material / 1000, ps.support_used_material / 1000) :
            ...
            wxString t_est = std::isnan(ps.estimated_print_time) ? "N/A" :
                                                                   from_u8(short_time_ui(get_time_dhms(float(ps.estimated_print_time))));
            p->sliced_info->SetTextAndShow(siEstimatedTime, t_est, _L("Estimated printing time") + ":");

            p->plater->get_notification_manager()->set_slicing_complete_print_time(_u8L("Estimated printing time") + ": " +
                                                                                       boost::nowide::narrow(t_est),
                                                                                   p->plater->is_sidebar_collapsed());
            ...
            info_text = ps.total_cost == 0.0 ? "N/A" :
                        is_wipe_tower ? wxString::Format("%.2f \n%.2f \n%.2f", ps.total_cost, (ps.total_cost - ps.total_wipe_tower_cost),
            ...
            if (ps.estimated_normal_print_time == "N/A" && ps.estimated_silent_print_time == "N/A")
                ...
                if (ps.estimated_normal_print_time != "N/A") {
                    ...
                    p->plater->get_notification_manager()->set_slicing_complete_print_time(_u8L("Estimated printing time") + ": " +
                                                                                               ps.estimated_normal_print_time,
                                                                                           p->plater->is_sidebar_collapsed());
                }
            ...
            p->sliced_info->SetTextAndShow(siWTNumbetOfToolchanges,
                                           ps.total_toolchanges > 0 ? wxString::Format("%.d", ps.total_toolchanges) : "N/A"); p->sliced_info->SetTextAndShow(siWTNumbetOfToolchanges, ps.total_toolchanges > 0 ? wxString::Format("%.d", ps.total_toolchanges) : "N/A");
...
void Sidebar::enable_buttons(bool enable)
{
   ...
   p->btn_reslice->Enable(enable);
    if (enable) 
    {
       p->btn_reslice->SetBackgroundColour(wxColour(21, 169, 93));
    } else {
        p->btn_reslice->SetBackgroundColour(wxColour(176, 176, 176));
    }
}

bool Sidebar::is_enable_export() 
{ return p->btn_export_gcode->IsEnabled(); }

void Sidebar::slice_acc_buttons(bool enable)
{
    p->btn_export_gcode->Enable(enable);
    p->btn_send_gcode->Enable(enable);
    p->btn_export_gcode_removable->Enable(enable);
    if (enable) 
    {
        p->btn_export_gcode->SetBackgroundColour(wxColour(21, 169, 93));
        p->btn_send_gcode->SetBackgroundColour(wxColour(21, 169, 93));
    }
}

bool Sidebar::show_reslice(bool show) const {
    if (show) 
       p->sliced_info->Show(false);
    return p->btn_reslice->Show(show); }
bool Sidebar::show_export(bool show) const {
    return p->btn_export_gcode->Show(show); }
...
struct Plater::priv
{
                ...
                if (act.empty()) {
                    ...
                    if (res != wxID_CANCEL)
                        if (dialog.IsCheckBoxChecked()) {
                            ...
                            wxString msg              = _L("FlsunSlicer will remember your choice.") + "\n\n" +
                            ...
                            MessageDialog msg_dlg(mainframe, msg, _L("FlsunSlicer: Don't ask me again"),
    ...
    void generate_thumbnail(
        ThumbnailData &data, unsigned int w, unsigned int h, const ThumbnailsParams &thumbnail_params, Camera::EType camera_type);
...
Plater::priv::priv(Plater *q, MainFrame *main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {"bed_shape", "bed_custom_texture", "bed_custom_model", "complete_objects", "duplicate_distance", "extruder_clearance_radius",
           "skirts", "skirt_distance", "brim_width", "brim_separation", "hot_bed_zoning", "brim_type", "variable_layer_height",
           "nozzle_diameter", "single_extruder_multi_material", "wipe_tower", "wipe_tower_x", "wipe_tower_y", "wipe_tower_width",
           "wipe_tower_rotation_angle", "wipe_tower_brim_width", "wipe_tower_cone_angle", "wipe_tower_extra_spacing", "wipe_tower_extruder",
           "extruder_colour", "filament_colour", "material_colour", "max_print_height", "printer_model", /*"is_AB",*/ "printer_notes",
           "printer_technology",
           // These values are necessary to construct SlicingParameters by the Canvas3D variable layer height editor.
           "layer_height", "first_layer_height", "min_layer_height", "max_layer_height", "brim_width", "perimeters", "perimeter_extruder",
           "fill_density", "infill_extruder", "top_solid_layers", "support_material", "support_material_extruder",
           "support_material_interface_extruder", "support_material_contact_distance", "support_material_bottom_contact_distance",
           "raft_layers"}))
    ...
    , m_project_filename(wxEmptyString)
{
    ...
    if (wxGetApp().is_editor()) {
        ...
        view3D_canvas->Bind(EVT_GLTOOLBAR_SEARCH, [this](SimpleEvent &) { wxGetApp().plater()->search(false); });
    }
    ...
    if (wxGetApp().is_editor()) {
        ...
        q->Bind(EVT_GLVIEWTOOLBAR_FULL, [q](SimpleEvent &) {
            Slic3r::GUI::wxGetApp().mainframe->ShowFullScreen(!Slic3r::GUI::wxGetApp().mainframe->IsFullScreen(),
                                                              wxFULLSCREEN_NOSTATUSBAR | wxFULLSCREEN_NOBORDER | wxFULLSCREEN_NOCAPTION);
        });

        q->Bind(EVT_GLVIEWTOOLBAR_TOP, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("top"); });
        q->Bind(EVT_GLVIEWTOOLBAR_ISO, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("iso"); });
        q->Bind(EVT_GLVIEWTOOLBAR_FRONT, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("front"); });
        q->Bind(EVT_GLVIEWTOOLBAR_REAR, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("rear"); });
        q->Bind(EVT_GLVIEWTOOLBAR_LEFT, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("left"); });
        q->Bind(EVT_GLVIEWTOOLBAR_RIGHT, [q](SimpleEvent &) { Slic3r::GUI::wxGetApp().mainframe->select_view("right"); });
    }
...
void Plater::priv::update_main_toolbar_tooltips() { view3D->get_canvas3d()->update_tooltip_for_settings_item_in_main_toolbar(); }
...
bool Plater::priv::get_config_bool(const std::string &key) const { return wxGetApp().app_config->get_bool(key); }
...
void Plater::notify_about_installed_presets()
{
    ...
    if (!names.empty()) {
        std::string notif_text = into_u8(_L_PLURAL("The preset below was temporarily installed on the active instance of FlsunSlicer",
                                                   "The presets below were temporarily installed on the active instance of FlsunSlicer",
                                                   names.size())) +
                                 ":";
                                             ...
                                             bool                         imperial_units /* = false*/)
{
    ...
    for (size_t i = 0; i < input_files_size; ++i) {
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        if (progress_dlg) {
            progress_dlg->Update(static_cast<int>(100.0f * static_cast<float>(i) / static_cast<float>(input_files.size())),
        ...
        /*const bool type_3mf     = false;
        const bool type_zip_amf = false;
        const bool type_any_amf = false;
        const bool type_prusa   = false;*/
            ...
            if (type_3mf || type_zip_amf) {
#ifdef __linux__
                // On Linux Constructor of the ProgressDialog calls DisableOtherWindows() function which causes a disabling of all children
                // of the find_toplevel_parent(q) And a destructor of the ProgressDialog calls ReenableOtherWindows() function which revert
                // previously disabled children. But if printer technology will be changes during project loading, then related SLA Print
                // and Materials Settings or FFF Print and Filaments Settings will be unparent from the wxNoteBook and that is why they will
                // never be enabled after destruction of the ProgressDialog. So, distroy progress_gialog if we are loading project file
                ...
                PrinterTechnology  loaded_printer_technology{ptFFF};
                {
                    ...
                    if (load_config && !config_loaded.empty()) {
                        ...
                        config.apply(loaded_printer_technology == ptFFF ? static_cast<const ConfigBase &>(FullPrintConfig::defaults()) :
            ...
            if (!is_project_file) {
                ...
                if (imperial_units)
                    ...
                    if (answer_convert_from_meters == wxOK_DEFAULT) {
                        RichMessageDialog
                            dlg(q,
                                format_wxstr(_L_PLURAL("The dimensions of the object from file %s seem to be defined in meters.\n"
                                                       "The internal unit of FlsunSlicer is a millimeter. Do you want to recalculate the "
                        ...
                        dlg.SetYesNoLabels(_L("Yes"), _L("No"));
                    ...
                    if (answer_convert_from_imperial_units == wxOK_DEFAULT) {
                        RichMessageDialog
                            dlg(q,
                                format_wxstr(_L_PLURAL("The dimensions of the object from file %s seem to be defined in inches.\n"
                                                       "The internal unit of FlsunSlicer is a millimeter. Do you want to recalculate the "
                                                       "dimensions of the object?",
                                                     ...
                                                     bool                   call_selection_changed /*= true*/)
{
    ...
    if (scaled_down) {
        GUI::show_info(q, _L("Your object appears to be too large, so it was automatically scaled down to fit your print bed."),
                       _L("Object too large?"));
    }
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    wxFileDialog dlg(q, dlg_title,
                     out_dir == temp_dir ?
                         from_u8(wxGetApp().app_config->get("last_output_path")) :
...
void Plater::priv::object_list_changed()
{
    ...
    sidebar->slice_acc_buttons(!model.objects.empty() && !export_in_progress && model_fits);
}
        ...
        InfoDialog dialog(q, _L("Delete object which is a part of cut object"),
                          _L("You try to delete an object which is a part of a cut object.") + "\n" +
                              _L("This action will break a cut information.\n"
                                 "After that FlsunSlicer can't guarantee model consistency"),
...
void Plater::priv::update_print_volume_state() { this->q->model().update_print_volume_state(this->bed.build_volume()); }
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages)
{
    ...
    if (full_config.has("binary_gcode")) // needed for SLA
        full_config.set("binary_gcode",
                        bool(full_config.opt_bool("binary_gcode") & wxGetApp().app_config->get_bool("use_binary_gcode_when_supported")));
    ...
    if ((return_state & UPDATE_BACKGROUND_PROCESS_INVALID) != 0) {
       ...
       /* const wxString slice_string = background_process.running() && wxGetApp().get_mode() == comSimple ? _L("Slice now") :
                                                                                                           _L("Slice now");*/
        sidebar->set_btn_label(ActionButtonType::abReslice, _L("Slice now"));
...
void Plater::priv::reload_from_disk()
{
    ...
    while (!missing_input_paths.empty()) {
        ...
        if (boost::algorithm::iequals(search.filename().string(), sel_filename)) {
            ...
            wxString message = _L("The selected file") + " (" + from_u8(sel_filename) + ") " + _L("differs from the original file") + " (" +
                               from_u8(search.filename().string()) + ").\n" + _L("Do you want to replace it") + " ?";
        ...
        for (auto [obj_idx, vol_idx] : selected_volumes) {
            ...
            if (has_source || has_name) {
                ...
                new_volume->set_transformation(
                    old_volume->get_transformation().get_matrix() * old_volume->source.transform.get_matrix_no_offset() *
...
void Plater::priv::on_select_preset(wxCommandEvent &evt)
{
    ...
    std::string preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias(preset_type,
                                                                                 Preset::remove_suffix_modified(
                                                                                     into_u8(combo->GetString(selection))),
                                                                                 idx);
    ...
    if (preset_type == Preset::TYPE_FILAMENT) {
        ...
        //wxWindowUpdateLocker noUpdates(sidebar->presets_panel());用于在切换打印机打印下拉框空白的
    ...
    combo->SetFocus();   
#endif
    if (preset_type == Preset::TYPE_FILAMENT || preset_type == Preset::TYPE_PRINTER) {
        preset_name = wxGetApp().filement_name;
        string    print_name;
        TabPrint *tab = dynamic_cast<TabPrint *>(wxGetApp().get_tab(Preset::TYPE_PRINT));

        print_name = wxGetApp().print_name.front().ToStdString();
        for (auto iter : wxGetApp().print_name) {
            if (iter.find(preset_name) != wxString::npos) {
                print_name = iter.ToStdString();
                break;
            }
        }
        print_name += " @FLSUN";
        tab->select_preset(print_name);
    }
}
...
void Plater::priv::on_slicing_update(SlicingStatusEvent &evt)
{
    ...
    if (evt.status.percent >= 100 && this->printer_technology == ptFFF) {
        ...
        if (templ_cnt > 0) {
            const std::string message_notif =
                GUI::format("%1%\n%2%\n\n%3%\n\n%4% ",
                            _L_PLURAL("You are using template filament preset.", "You are using template filament presets.", templ_cnt),
                            names,
                            _u8L("Please note that template presets are not customized for specific printer and should only be used as a "
                                 "starting point for creating your own user presets."),
                            _u8L("More info at"));
            // warning dialog proccessing cuts text at first '/n' - pass the text without new lines (and without filament names).
            const std::string message_dial =
                GUI::format("%1% %2% %3%",
                            _L_PLURAL("You are using template filament preset.", "You are using template filament presets.", templ_cnt),
                            _u8L("Please note that template presets are not customized for specific printer and should only be used as a "
                                 "starting point for creating your own user presets."),
            ...
            notification_manager->push_slicing_warning_notification(message_notif, false, 0, 0, "https://help.prusa3d.com/",
                                                                    [](wxEvtHandler *evnthndlr) {
                                                                        wxGetApp().open_browser_with_warning_dialog(
                                                                            "https://help.prusa3d.com/article/template-filaments_467599");
                                                                        return false;
                                                                    });
    ...
    if (evt.status.flags & (PrintBase::SlicingStatus::RELOAD_SCENE | PrintBase::SlicingStatus::RELOAD_SLA_SUPPORT_POINTS)) {
        ...
        case ptFFF: this->update_fff_scene(); break;
        case ptSLA:
    ...
    if ((evt.status.flags & PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS) &&
        ...
        if (evt.status.flags & PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS) {
            state = this->printer_technology == ptFFF ? this->fff_print.step_state_with_warnings(static_cast<PrintStep>(warning_step)) :
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    if (wxGetApp().slice_error) {
        wxMessageBox(_L("The slicing error did not generate G-CODE"), _L("Error"), wxOK | wxICON_ERROR);
        sidebar->p->btn_export_gcode->Enable(false);
        sidebar->p->btn_send_gcode->Enable(false);
    } 
    wxGetApp().slice_error = false;
}
...
void Plater::priv::generate_thumbnail(
    ThumbnailData &data, unsigned int w, unsigned int h, const ThumbnailsParams &thumbnail_params, Camera::EType camera_type)
{
...
bool Plater::priv::init_view_toolbar()
{
    ...
    if (!view_toolbar.add_item(item))
        return false;*/
    item.name                 = "Iso View";
    item.icon_filename        = "full_view.svg";
    item.tooltip              = _u8L("Iso View") + "[0]";
    item.sprite_id            = 0;
    item.left.action_callback = [this]() {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_ISO));
    };
    if (!view_toolbar.add_item(item))
        return false;

    item.name                 = "Top View";
    item.icon_filename        = "top.svg";
    item.tooltip              = _u8L("Top View") + "[1]";
    item.sprite_id            = 1;
    item.left.action_callback = [this]() {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_TOP));
    };
    if (!view_toolbar.add_item(item))
        return false;

    item.name                 = "Front View";
    item.icon_filename        = "front.svg";
    item.tooltip              = _u8L("Front View") + "[2]";
    item.sprite_id            = 2;
    item.left.action_callback = [this]() {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_FRONT));
    };
    if (!view_toolbar.add_item(item))
        return false;

    /* item.name                 = "Rear View";
     item.icon_filename        = "rear.svg";
     item.tooltip              = _u8L("Rear View") + "[3]";
     item.sprite_id            = 3;
     item.left.action_callback = [this]() {
         if (this->q != nullptr)
             wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_REAR));
     };
     if (!view_toolbar.add_item(item))
         return false;*/

    item.name                 = "Left View";
    item.icon_filename        = "left.svg";
    item.tooltip              = _u8L("Left View") + "[3]";
    item.sprite_id            = 3;
    item.left.action_callback = [this]() {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_LEFT));
    };
    if (!view_toolbar.add_item(item))
        return false;

    item.name                 = "Right View";
    item.icon_filename        = "right.svg";
    item.tooltip              = _u8L("Right View") + "[4]";
    item.sprite_id            = 4;
    item.left.action_callback = [this]() {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_RIGHT));
    };
    if (!view_toolbar.add_item(item))
        return false;
    view_toolbar.select_item("3D");
...
void Plater::priv::set_preview_layers_slider_values_range(int bottom, int top) { preview->set_layers_slider_values_range(bottom, top); }

void Plater::priv::update_preview_moves_slider() { preview->update_moves_slider(); }

void Plater::priv::enable_preview_moves_slider(bool enable) { preview->enable_moves_slider(enable); }
...
bool Plater::priv::can_mirror() const { return !sidebar->obj_list()->has_selected_cut_object(); }
...
bool Plater::priv::can_delete_all() const { return !model.objects.empty() && !sidebar->obj_list()->is_editing(); }
...
bool Plater::priv::can_split_to_volumes() const { return q->can_split(false); }
...
void Plater::priv::show_action_buttons(const bool ready_to_slice_) const
{
    ...
    if (get_config_bool("background_processing")) {
        ...
        if (sidebar->show_reslice(ready_to_slice) | sidebar->show_export(!ready_to_slice) |
            ...
            sidebar->Layout();
        if (ready_to_slice && Slic3r::GUI::wxGetApp().mainframe != nullptr) {
            dynamic_cast<Notebook *>(Slic3r::GUI::wxGetApp().mainframe->m_tabpanel)->reday_slicemow();
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (printer_technology_changed) {
        ...
        if (!wxGetApp().check_and_save_current_preset_changes(
                ...
                //            format_wxstr(_L("%1% printer was active at the time the target Undo / Redo snapshot was taken. Switching to
                //            %1% printer requires reloading of %1% presets."), s_pt)))
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
            this->undo_redo_stack().undo(model, this->view3D->get_canvas3d()->get_selection(),
                                         this->view3D->get_canvas3d()->get_gizmos_manager(), top_snapshot_data, it_snapshot->timestamp) :
...
void Plater::priv::update_after_undo_redo(const UndoRedo::Snapshot &snapshot, bool /* temp_snapshot_was_taken */)
{
    ...
    this->view3D->get_canvas3d()->get_selection().set_deserialized(GUI::Selection::EMode(
                                                                       this->undo_redo_stack().selection_deserialized().mode),
    ...
    if (wxGetApp().get_mode() == comSimple && model_has_advanced_features(this->model)) {
        ...
        // There is a little risk of surprising the user, as he already must have had the advanced or expert mode active for such a snapshot
        // to be taken.
        if (wxGetApp().save_mode(comAdvanced))
            view3D->set_as_dirty();
    }
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "flsunslicer window going forward";
...
bool Plater::is_project_temp() const { return false; }

void Plater::new_project()
{
    if (int saved_project = p->save_project_if_dirty(_L("Creating a new project while the current project is modified."));
...
void Plater::load_gcode(const wxString &filename)
{
    ...
    try {
        ...
        MessageDialog(this, _L("The selected file") + ":\n" + filename + "\n" + _L("does not contain valid gcode."),
                      wxString(GCODEVIEWER_APP_NAME) + " - " + _L("Error while loading .gcode file"), wxOK | wxICON_WARNING | wxCENTRE)
            .ShowModal();
        set_project_filename(wxEmptyString);
        return;
    }
...
void Plater::convert_gcode_to_ascii()
{
    ...
    if (exists) {
        MessageDialog msg_dlg(this, GUI::format_wxstr(_L("File %1% already exists. Do you wish to overwrite it?"), output_file),
                              _L("Notice"), wxYES_NO);
        if (msg_dlg.ShowModal() != wxID_YES)
            return;
    }
...
public:
    enum class LoadProjectOption : unsigned char { Unknown, AllGeometry, AllNewWindow, OneProject, OneConfig };
...
LoadProjectsDialog::LoadProjectsDialog(const std::vector<fs::path> &paths)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe),
                wxID_ANY,
                format_wxstr(_L("%1% - Multiple projects file"), SLIC3R_APP_NAME),
                wxDefaultPosition,
                wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE)
{
    ...
    if (instances_allowed) {
        btn = new wxRadioButton(this, wxID_ANY, _L("Start a new instance of FlsunSlicer"), wxDefaultPosition, wxDefaultSize,
                                id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
        btn->Bind(wxEVT_RADIOBUTTON, [this, id, contains_projects](wxCommandEvent &) {
            m_action = id;
    ...
    if (contains_projects) {
        ...
        // one config
        id++;
        btn = new wxRadioButton(this, wxID_ANY, _L("Select only one file to load the configuration."), wxDefaultPosition, wxDefaultSize,
                                id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
...
public:
    enum class LoadType : unsigned char { Unknown, OpenProject, LoadGeometry, LoadConfig, OpenWindow };
...
ProjectDropDialog::ProjectDropDialog(const std::string &filename)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe),
                wxID_ANY,
                format_wxstr("%1% - %2%", SLIC3R_APP_NAME, _L("Load project file")),
                wxDefaultPosition,
                wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE)
{
    ...
    if (!single_instance_only)
        choices.Add(_L("Start new FlsunSlicer instance"));

    main_sizer->Add(new wxStaticText(this, wxID_ANY,
                                     get_wraped_wxString(_L("Select an action to apply to the file") + ": " + from_u8(filename))),
                    0, wxEXPAND | wxALL, 10);

    m_action = std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")), static_cast<int>(LoadType::OpenProject),
                          single_instance_only ? static_cast<int>(LoadType::LoadConfig) : static_cast<int>(LoadType::OpenWindow)) -
               1;
    ...
    check->Bind(wxEVT_CHECKBOX,
                [](wxCommandEvent &evt) { wxGetApp().app_config->set("show_drop_project_dialog", evt.IsChecked() ? "0" : "1"); });
    ...
    if (wxGetApp().is_gcode_viewer()) {
        ...
        if (paths.size() > 1) {
            ...
            MessageDialog(static_cast<wxWindow *>(this), _L("You can open only one .gcode file at a time."),
                          wxString(SLIC3R_APP_NAME) + " - " + _L("Drag and drop G-code file"), wxCLOSE | wxICON_WARNING | wxCENTRE)
                .ShowModal();
            return false;
...
{
    RichMessageDialog dialog(static_cast<wxWindow *>(this), _L("All objects will be removed, continue?"),
                             wxString(SLIC3R_APP_NAME) + " - " + _L("Delete all"), wxYES_NO);
    dialog.SetYesNoLabels(_L("Yes"), _L("No"));
    if (p->model.objects.empty() || dialog.ShowModal() == wxID_YES)
        reset();
}
...
void Plater::fill_bed_with_instances()
{
    ...
    if (w.is_idle()) {
        ...
        cbs.on_processed = [this](arr2::ArrangeTaskBase &t) { p->take_snapshot(_L("Fill bed")); };
...
{
    if (binary_output &&
        (boost::algorithm::contains(printer_notes, "PRINTER_MODEL_XL") || boost::algorithm::contains(printer_notes, "PRINTER_MODEL_MINI") ||
        ...
        if (app_config->get(option_key) != "1") {
            ...
            HtmlCapableRichMessageDialog dialog(parent,
                                                format_wxstr(_L("You are exporting binary G-code for a Prusa printer. "
                                                                ...
                                                                "Ensure that your printer is running firmware version 5.1.0 or newer, as "
                                                                "older versions do not support binary G-codes.\n\n"
...
void Plater::export_gcode(bool prefer_removable)
{
    ...
    fs::path output_path;
    {
        ...
        wxFileDialog dlg(this, (printer_technology() == ptFFF) ? _L("Save G-code file as:") : _L("Save SL1 / SL1S file as:"), start_dir,
        ...
        if (dlg.ShowModal() == wxID_OK) {
            ...
            if (check_for_error(output_path, error_str)) {
                ...
                alert_when_exporting_binary_gcode(supports_binary && uses_binary,
                                                  wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_string(
                                                      "printer_notes"));
            }
        }
    }
...
bool Plater::export_3mf(const boost::filesystem::path &output_path)
{
    if (p->model.objects.empty()) {
        MainFrame *       mainframe = wxGetApp().mainframe;
        RichMessageDialog dialog(mainframe, _L("The plater is empty.\nDo you want to save the project?"), _L("Save project"), wxYES_NO);
...
void Plater::reslice()
{
    ...
    if (p->background_process.running()) {
            p->sidebar->set_btn_label(ActionButtonType::abReslice, /*_L("Slicing") + dots*/ _L("Slice now"));
            p->sidebar->p->btn_reslice->Enable(false);
...
void Plater::send_gcode()
{
    ...
    DynamicPrintConfig *physical_printer_config = const_cast<Slic3r::DynamicPrintConfig *>(
        &wxGetApp().preset_bundle->physical_printers.default_config());
    ...
    if (dlg.ShowModal() == wxID_OK) {
        ...
        // dynamic_cast<Moonraker *>(upload_job.printhost)->set_host(dlg.select_ip);
        upload_job.printhost->set_host(dlg.select_ip);
...
bool Plater::update_filament_colors_in_full_config()
{
    ...
    for (const auto &extr_filaments : extruders_filaments)
        filament_colors.push_back(
            filaments.find_preset(extr_filaments.get_selected_preset_name(), true)->config.opt_string("filament_colour", (unsigned) 0));
...
void Plater::set_default_bed_shape() const { set_bed_shape({{0.0, 0.0}, {200.0, 0.0}, {200.0, 200.0}, {0.0, 200.0}}, 0.0, {}, {}, true); }
...
GLCanvas3D *Plater::canvas3D() { return p->view3D->get_canvas3d(); }

const GLCanvas3D *Plater::canvas3D() const { return p->view3D->get_canvas3d(); }

GLCanvas3D *Plater::get_current_canvas3D() { return p->get_current_canvas3D(); }
...
void Plater::arrange(Worker &w, bool selected)
{
    ...
    cbs.on_processed = [this](arr2::ArrangeTaskBase &t) { p->take_snapshot(_L("Arrange")); };
...
void Plater::clear_before_change_mesh(int obj_idx, const std::string &notification_msg)
{
    ...
    if (paint_removed) {
        ...
        get_notification_manager()->push_notification(NotificationType::CustomSupportsAndSeamRemovedAfterRepair,
                                                      NotificationManager::NotificationLevel::PrintInfoNotificationLevel, notification_msg);
...
{
    for (auto iter : wxGetApp().mainframe->m_dialog_vec) {
        iter->Close();
    }
    if (plater_is_active) {
        if (is_preview_shown())
...
void Plater::close_search() 
{ p->sidebar->get_searcher().close_dialog(); }
...
bool Plater::init_view_toolbar() { return p->init_view_toolbar(); }

void Plater::enable_view_toolbar(bool enable) { p->view_toolbar.set_enabled(enable); }
...
const GLToolbar &Plater::get_view_toolbar() const { return p->view_toolbar; }

GLToolbar &Plater::get_view_toolbar() { return p->view_toolbar; }
...
void Plater::set_preview_layers_slider_values_range(int bottom, int top) { p->set_preview_layers_slider_values_range(bottom, top); }

void Plater::update_preview_moves_slider() { p->update_preview_moves_slider(); }

void Plater::enable_preview_moves_slider(bool enable) { p->enable_preview_moves_slider(enable); }

void Plater::reset_gcode_toolpaths() { p->reset_gcode_toolpaths(); }
...
void Plater::set_keep_current_preview_type(bool value) { p->preview->set_keep_current_preview_type(value); }
