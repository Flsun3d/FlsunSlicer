...
void OptionsGroup::activate_line(Line& line)
{
    ...
    if (!custom_ctrl) {
        ...
        if (!staticbox)
            m_grid_sizer->AddSpacer(10);
        m_grid_sizer->Add(h_sizer, 0, wxEXPAND | (staticbox ? wxALL : /* wxTOP | wxBOTTOM | wxLEFT*/ wxRIGHT),
                          staticbox ? 0 : wxGetApp().em_unit() * 3.4);
    }
    ...
    for (const Option& opt : option_set) {
        ...
        if (!custom_ctrl) {
            ...
            if (option_set.size() == 1 && option_set.front().opt.full_width)
            {
                ...
                is_sizer_field(field) ? v_sizer->Add(field->getSizer(), 1, wxEXPAND | wxRIGHT, 6) :
                                        v_sizer->Add(field->getWindow(), 1, wxEXPAND | wxRIGHT, 6);
                break;
            }
...
boost::any ConfigOptionsGroup::get_config_value(const DynamicPrintConfig& config, const std::string& opt_key, int opt_index /*= -1*/)
{
    ...
    if (opt->nullable)
    {
        ...
        case coFloat:
            ...
            break;
        case coInt: ret = config.option<ConfigOptionIntNullable>(opt_key)->value; break;
        case coPercents:
        case coFloats: {
            if (config.option(opt_key)->is_nil())
                ret = _(L("N/A"));
