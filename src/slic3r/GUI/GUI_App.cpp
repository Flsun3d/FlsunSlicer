///|/ Copyright (c) Prusa Research 2018 - 2023 Vojtěch Bubník @bubnikv, Oleksandra Iushchenko @YuSanka, Lukáš Matěna @lukasmatena, Tomáš
///Mészáros @tamasmeszaros, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Lukáš Hejl @hejllukas, Filip Sykala @Jony01, Vojtěch Král
///@vojtechkral
...
bool static check_old_linux_datadir(const wxString &app_name)
{
    ...
    // (https://github.com/prusa3d/FlsunSlicer/issues/2911)
    ...
    if (file_count <= 1) { // just cache dir with an instance lock
        ...
        if (fs::is_directory(old_path)) {
            wxString msg = from_u8(
                (boost::format(_u8L("Starting with %1% 2.3, configuration "
...
std::string set_registry_version()
{
    HKEY  hKey;
    char  buffer[1024] = {0};
    ...
    // 打开注册表键
    result = RegOpenKeyExA(HKEY_LOCAL_MACHINE,                           // 预定义的键
                           "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\FlsunSlicer", // 子键路径
                           0,                                            // 保留
                           KEY_ALL_ACCESS,                                                         // 请求的访问权限
                           &hKey);                                       // 打开的键的句柄
    ...
    // 读取版本号值
    result = RegQueryValueExA(hKey,            // 已打开的键的句柄
                              "DisplayVersion",       // 值名
                              NULL,            // 保留
                              NULL,            // 返回类型
                              (LPBYTE) buffer, // 数据缓冲区
    ...
    std::wstring newVersion = L"1.0.9"; // 打开注册表键
    //std::string version = buffer;
    if (buffer != SLIC3R_VERSION) {
        std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
        newVersion = converter.from_bytes(SLIC3R_VERSION);
        RegSetValueEx(hKey, _T("DisplayVersion"), 0, REG_SZ, reinterpret_cast<const BYTE *>(newVersion.c_str()),
                      (newVersion.length() + 1) * sizeof(wchar_t));
    }
    RegCloseKey(hKey);
    return buffer;
}
...
static void generic_exception_handle()
{
    ...
    try {
        ...
        wxString errmsg = _L("FlsunSlicer has encountered a localization error. "
                             "Please report to FlsunSlicer team, what language was active and in which scenario "
...
void GUI_App::post_init()
{
    ...
    if (this->is_gcode_viewer()) {
        ...
        if (!this->init_params->input_files.empty()) {
            ...
            if (plater()->load_files(fns) && this->init_params->input_files.size() == 1) {
                ...
                if (boost::algorithm::iends_with(filename, ".amf") || boost::algorithm::iends_with(filename, ".amf.xml") ||
    ...
    // Set FlsunSlicer version and save to FlsunSlicer.ini or PrusaSlicerGcodeViewer.ini.
...
{
    // app config initializes early becasuse it is used in instance checking in FlsunSlicer.cpp
...
wxGLContext *GUI_App::init_glcontext(wxGLCanvas &canvas)
{
    ...
    return m_opengl_mgr.init_glcontext(canvas, init_params != nullptr ? init_params->opengl_version : std::make_pair(0, 0),
                                       init_params != nullptr ? init_params->opengl_compatibiity_profile : false,
...
// gets path to FlsunSlicer.ini, returns semver from first line comment
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t      start = body.find("FlsunSlicer ");
...
void removeFilesExcept(fs::path directory, const std::string &exceptFileName)
{
    ...
    for (fs::directory_iterator it(directory); it != fs::directory_iterator(); ++it) {
        if (fs::is_regular_file(*it) && it->path().filename().string() != exceptFileName) {
            try {
                fs::remove(it->path());
                std::cout << "Removed file: " << it->path().filename() << std::endl;
            } catch (const std::exception &e) {
                std::cerr << "Error removing file: " << e.what() << std::endl;
            }
        } else if (fs::is_directory(*it)) {
            removeFilesExcept(it->path(), exceptFileName); // 递归删除文件夹中的文件
        }
    }
}
...
{
    // Profiles for the alpha are stored into the FlsunSlicer-alpha directory to not mix with the current release.
    ...
    if (m_app_conf_exists) {
        ...
        //判断本地配置文件版本号低于运行版本号则删除配置文件
        if (app_config->legacy_datadir()) {
            removeFilesExcept(data_dir(), "ipconfig.ini");

            /*namespace bfs = boost::filesystem;
            bfs::path boostFilePath(data_dir());
            bfs::remove_all(boostFilePath);
            assert(!bfs::exists(boostFilePath));*/
        }
        if (!error.empty()) {
            // Error while parsing config file. We'll customize the error message and rethrow to be displayed.
            if (is_editor()) {
                throw Slic3r::RuntimeError(
                    format("Error parsing FlsunSlicer config file, it is probably corrupted. "
...
std::string GUI_App::check_older_app_config(Semver current_version, bool backup)
{
    ...
    if (SLIC3R_APP_KEY "-alpha" != GetAppName())
        candidates.emplace_back(parent_file_path / SLIC3R_APP_KEY "-alpha" / filename);
    if (SLIC3R_APP_KEY "-beta" != GetAppName())
        candidates.emplace_back(parent_file_path / SLIC3R_APP_KEY "-beta" / filename);
    if (SLIC3R_APP_KEY != GetAppName())
        candidates.emplace_back(parent_file_path / SLIC3R_APP_KEY / filename);
    ...
    InfoDialog msg(nullptr, format_wxstr(_L("You are opening %1% version %2%."), SLIC3R_APP_NAME, SLIC3R_VERSION),
                   backup ? format_wxstr(_L("The active configuration was created by <b>%1% %2%</b>,"
...
bool GUI_App::on_init_inner()
{
    ...
    if (wxPlatformInfo::Get().GetArchName().substr(0, 2) == "64") {
        RichMessageDialog dlg(nullptr,
                              _L("You are running a 32 bit build of FlsunSlicer on 64-bit Windows."
                                 "\n32 bit build of FlsunSlicer will likely not be able to utilize all the RAM available in the system."
                                 "\nPlease download and install a 64 bit build of FlsunSlicer from https://www.prusa3d.cz/prusaslicer/."
                                 "\nDo you wish to continue?"),
                              "FlsunSlicer", wxICON_QUESTION | wxYES_NO);
    ...
    if (bool new_sys_menu_enabled = app_config->get_bool("sys_menu_enabled"); init_sys_menu_enabled != new_sys_menu_enabled)
    ...
    if (app_config->get_bool("show_splash_screen")) {
        ...
        if (app_config->has("window_mainframe") && app_config->get_bool("restore_win_position")) {
            ...
            last_size    = metrics->get_rect().GetSize();
        ...
        wxBitmap bmp = wxBitmap((wxString::FromUTF8(path) + "flsun_3.png"), wxBITMAP_TYPE_ANY);
        scrn         = new SplashScreen(bmp, wxSPLASH_CENTRE_ON_SCREEN | wxSPLASH_TIMEOUT, 3000);
        if (!default_splashscreen_pos)
    ...
    if (is_editor()) {
        ...
        Bind(EVT_SLIC3R_EXPERIMENTAL_VERSION_ONLINE, [this](const wxCommandEvent &evt) {
            if (this->plater_ != nullptr && (m_app_updater->get_triggered_by_user() || app_config->get("notify_release") == "all")) {
                ...
                if (*Semver::parse(SLIC3R_VERSION) < *Semver::parse(evt_string)) {
                    auto notif_type = (evt_string.find("beta") != std::string::npos ? NotificationType::NewBetaAvailable :
                                                                                      NotificationType::NewAlphaAvailable);
                    this->plater_->get_notification_manager()
                        ->push_version_notification(notif_type, NotificationManager::NotificationLevel::ImportantNotificationLevel,
                                                    Slic3r::format(_u8L("New prerelease version %1% is available."), evt_string),
                                                    _u8L("See Releases page."), [](wxEvtHandler *evnthndlr) {
                                                        wxGetApp().open_browser_with_warning_dialog(
                                                            "https://github.com/prusa3d/FlsunSlicer/releases");
                                                        return true;
                                                    });
                }
            }
        ...
        Bind(EVT_SLIC3R_APP_DOWNLOAD_PROGRESS, [this](const wxCommandEvent &evt) {
            ...
            if (this->plater_ != nullptr)
                this->plater_->get_notification_manager()->set_download_progress_percentage((float) std::stoi(into_u8(evt.GetString())) /
                                                                                            100.f);
        });
    ...
    mainframe->SetSize(last_size);
    auto icon_name = mainframe->IsMaximized() ? "reduction" : "square";
    mainframe->m_restoreBtn->SetBitmap(*get_bmp_bundle(icon_name, 20));
#endif // WIN32
    // hide settings tabs after first Layout
    if (is_editor())
        mainframe->select_tab(size_t(0));
   // mainframe->SetSize(wxSize(1000, 800));
    ...
    if (const std::string &crash_reason = app_config->get("restore_win_position"); boost::starts_with(crash_reason, "crashed")) {
        wxString preferences_item = _L("Restore window position on start");
        InfoDialog dialog(nullptr,
            _L("FlsunSlicer started after a crash"),
            format_wxstr(_L("FlsunSlicer crashed last time when attempting to set window position.\n"
                ...
                "","",
               // "<a href=http://github.com/prusa3d/FlsunSlicer/issues/2939>#2939</a>",
               // "<a href=http://github.com/prusa3d/FlsunSlicer/issues/5573>#5573</a>",
...
std::string GUI_App::dot_turn_symbol(std::string lable_name)
{
    char        to_replace  = '.';
    std::string replacement = "*3$%";
    size_t      pos         = 0;
    while ((pos = lable_name.find(to_replace, pos)) != std::string::npos) {
        lable_name.replace(pos, 1, replacement);
        pos += replacement.length(); // 跳过已替换的部分
    }
    return lable_name;
}

std::string GUI_App::symbol_turn_dot(std::string lable_name)
{
    char        replacement = '.';
    std::string to_replace  = "*3$%";

    size_t pos = lable_name.find(to_replace);
    while (pos != std::string::npos) {
        lable_name.replace(pos, to_replace.length(), 1, replacement);
        pos = lable_name.find(to_replace, pos + 1);
    }
    return lable_name;
}
...
const wxColour GUI_App::get_label_default_clr_system() { return dark_mode() ? wxColour(115, 220, 103) : wxColour(0, 0, 0); }

const wxColour GUI_App::get_label_default_clr_modified() { return dark_mode() ? wxColour(253, 111, 40) : wxColour(252, 77, 1); }

const std::vector<std::string> GUI_App::get_mode_default_palette() { return {"#7DF028", "#FFDC00", "#E70000"}; }
void GUI_App::init_ui_colours()
{
    ...
    m_color_default_btn_label = is_dark_mode ? wxColour(255, 181, 101) : wxColour(0, 0, 0);
    m_color_selected_btn_bg   = is_dark_mode ? wxColour(95, 73, 62) : wxColour(218, 231, 223);
    m_color_tab_bg            = wxColour(74, 84, 81);
    m_color_top_bg            = wxColour(38, 48, 45);
    m_color_top_font_def      = wxColour(234, 234, 234);
    m_color_font_focus        = wxColour(0, 178, 96);
    ...
    if (wxButton *btn = dynamic_cast<wxButton *>(window)) {
        ...
        // button marking
        {
            auto mark_button = [this, btn, highlited](const bool mark) {
                if (btn->GetLabel() == _L("Simple") || btn->GetLabel() == _L("Advanced") || btn->GetLabel() == _L("Expert")) {
                    return;
                }
                if (btn->GetLabel().IsEmpty()) {
                    auto old_color = btn->GetParent()->GetBackgroundColour();
                    btn->SetBackgroundColour(mark ? m_color_selected_btn_bg : highlited ? m_color_highlight_default : old_color);
                } else
                    btn->SetForegroundColour(mark ? m_color_hovered_btn_label :
        ...
        if (text->GetBorder() != wxBORDER_SIMPLE)
            text->SetWindowStyle(text->GetWindowStyle() & ~wxFULLSCREEN_ALL);
...
void GUI_App::SetWindowVariantForButton(wxButton *btn)
{
    ...
    // But standard height is possible to change using SetWindowVariant method (see
    // https://docs.wxwidgets.org/3.0/window_8h.html#a879bccd2c987fedf06030a8abcbba8ac)
...
const std::string GUI_App::get_html_bg_color(wxWindow *html_parent)
{
    ...
    // is wrong, which leads to https://github.com/prusa3d/FlsunSlicer/issues/7603
    // and https://github.com/prusa3d/FlsunSlicer/issues/3775. wxSYS_COLOUR_WINDOW
...
void GUI_App::check_printer_presets()
{
    ...
    msg_text.RemoveLast();
    msg_text += "\n\n" +
                _L("But since this version of FlsunSlicer we don't show this information in Printer Settings anymore.\n"
...
{
    wxString language = app_config->get("translation_language");
    wxSize   kebsize;
    if (language.empty()) {
        const std::string path     = Slic3r::resources_dir() + "/data/userconfig.ini";
        ConfigIni *       m_config = new ConfigIni(path);
        std::string       select_language;
        m_config->GetValue("ChangeLanguage.Language", select_language);
        if (select_language == "cn") {
            select_language = "zh_CN";
        }
        language = select_language;
    }
    if (language != "zh_CN" && language != "en") {
        kebsize = wxSize(1150, 650);
    }
    KBShortcutsDialog *dlg = new KBShortcutsDialog(kebsize, language);
    dlg->ShowModal();
}
...
#ifdef _MSW_DARK_MODE
void GUI_App::force_menu_update() { NppDarkMode::SetSystemMenuForApp(app_config->get_bool("sys_menu_enabled")); }
#endif //_MSW_DARK_MODE
...
{
    input_file.Clear();
    wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (GCODE/GCO/G/BGCODE/BGC/NGC):"), app_config->get_last_dir(), "",
...
bool GUI_App::select_language()
{
    ...
    for (size_t i = 0; i < language_infos.size(); ++i) {
        ...
        switch (language_infos[i]->Language) {
        case 130: names.Add(wxT("中文(简体)")); break;
        case 175: names.Add(wxT("English")); break;
        case 301: names.Add(wxT("français")); break;
        case 373: names.Add(wxT("Deutsch")); break;
        case 423: names.Add(wxT("italiano")); break;
        case 735: names.Add(wxT("español")); break;
        default: break;
        }
    }
    if (init_selection == -1)
...
bool GUI_App::load_language(wxString language, bool initial)
{
    if (initial) {
        ...
        // Get the active language from FlsunSlicer.ini, or empty string if the key does not exist.
        std::string viewer_path = (boost::filesystem::path(Slic3r::data_dir()) / (SLIC3R_APP_KEY ".ini")).make_preferred().string();
        if (boost::filesystem::exists(viewer_path)) {
            auto m_config = new ConfigIni(viewer_path);
            std::string lag;
            m_config->GetValue("translation_language", lag);
            language = lag;
        }
        //language = app_config->get("translation_language");
        if (!language.empty())
            BOOST_LOG_TRIVIAL(trace) << boost::format("translation_language provided by FlsunSlicer.ini: %1%") % language;
        ...
        if (language.empty()) {
            ...
            if (select_language == "cn") {
                select_language =  "zh_CN";
                lang_system = wxLANGUAGE_CHINESE_CHINA;
            } else if (select_language == "en") {
                lang_system = wxLANGUAGE_ENGLISH;
            } else if (select_language == "de") {
                lang_system = wxLANGUAGE_GERMAN;
            } else if (select_language == "es") {
                lang_system = wxLANGUAGE_SPANISH;
            } else if (select_language == "fr") {
                lang_system = wxLANGUAGE_FRENCH;
            } else if (select_language == "it") {
                lang_system = wxLANGUAGE_ITALIAN;
            }
            app_config->set("translation_language", select_language);                        
        ...
        }
        {
            ...
            // For example in our case it's trying to load "en_CZ" and as a result FlsunSlicer catch warning message.
       ...
       // Set the current translation's language to default, otherwise GetBestTranslation() may not work (see the wxWidgets source code).
            ...
            // Let the wxFileTranslationsLoader enumerate all translation dictionaries for FlsunSlicer
    ...
    if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
        BOOST_LOG_TRIVIAL(trace)
            << boost::format("The following language code requires right to left layout, which is not supported by FlsunSlicer: %1%") %
                   language_info->CanonicalName.ToUTF8().data();
        language_info = nullptr;
    }

    if (language_info == nullptr) {
        // FlsunSlicer does not support the Right to Left languages yet.
    ...
    if (!wxLocale::IsAvailable(language_info->Language)) {
        ...
        wxString message = "Switching FlsunSlicer to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "FlsunSlicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
Tab *GUI_App::get_dialog_tab(Preset::Type type)
{
    for (auto iter : mainframe->m_dialog_vec)
        if (iter->m_dialog->type() == type)
            return iter->m_dialog->completed() ? iter->m_dialog : nullptr; // To avoid actions with no-completed Tab
    return nullptr;
}
...
wxString GUI_App::get_mode_defaul()
{
    if (!app_config->has("view_mode"))
        return _L("Simple");

    const auto mode = app_config->get("view_mode");
    return mode == "expert" ? _L("Expert") : mode == "simple" ? _L("Simple") : _L("Advanced");
}
...
wxMenu *GUI_App::add_config_menu(wxMenuBar *menu)
{
    ...
    if (is_editor()) {
        ...
        local_menu->Append(config_id_base + ConfigMenuUpdateConf, _L("Check for Configuration Updates"),
                           _L("Check for configuration updates"));*/
    ...
    if (is_editor()) {
        ...
        // mode_menu->AppendRadioItem(config_id_base + ConfigMenuModeAdvanced, _L("Advanced", "Mode"), _L("Advanced View Mode"));
        mode_menu->AppendRadioItem(config_id_base + ConfigMenuModeExpert, _L("Expert"), _L("Expert View Mode"));
        Bind(
            wxEVT_UPDATE_UI,
            [this](wxUpdateUIEvent &evt) {
                if (get_mode() == comSimple) 
                {
                    evt.Check(true);
                }
            },
            config_id_base + ConfigMenuModeSimple);
        Bind(
            wxEVT_UPDATE_UI,
            [this](wxUpdateUIEvent &evt) {
                if (get_mode() == comAdvanced)
                    evt.Check(true);
            },
        ...
        Bind(
            wxEVT_UPDATE_UI,
            [this](wxUpdateUIEvent &evt) {
                if (get_mode() == comExpert)
                    evt.Check(true);
            },
            config_id_base + ConfigMenuModeExpert);
    ...
    // if (is_editor()) {
    //    local_menu->AppendSeparator();
    //    local_menu->Append(config_id_base + ConfigMenuFlashFirmware, _L("Flash Printer &Firmware"), _L("Upload a firmware image into an
    //    Arduino based printer"));
    //    // TODO: for when we're able to flash dictionaries
    //    // local_menu->Append(config_id_base + FirmwareMenuDict,  _L("Flash Language File"),    _L("Upload a language dictionary file into
    //    a Prusa printer"));
    //}
   /* local_menu->Append(config_id_base + ConfigMenuWifiConfigFile, _L("Wi-Fi Configuration File"),
    ...
    local_menu->Bind(wxEVT_MENU, [this, config_id_base](wxEvent &event) {
        ...
        case ConfigMenuUpdateConf: check_updates(true); break;
        case ConfigMenuUpdateApp: app_version_check(true); break;
#ifdef __linux__
        case ConfigMenuDesktopIntegration: show_desktop_integration_dialog(); break;
#endif
        case ConfigMenuTakeSnapshot:
            // Take a configuration snapshot.
            if (wxString action_name = _L("Taking a configuration snapshot"); check_and_save_current_preset_changes(
                    action_name, _L("Some presets are modified and the unsaved changes will not be captured by the configuration snapshot."),
                    false, true)) {
                ...
                if (dlg.ShowModal() == wxID_OK)
                    if (const Config::Snapshot *snapshot = Config::take_config_snapshot_report_error(*app_config,
                                                                                                     Config::Snapshot::SNAPSHOT_USER,
                                                                                                     dlg.GetValue().ToUTF8().data());
                        snapshot != nullptr)
            ...
            if (check_and_save_current_preset_changes(_L("Loading a configuration snapshot"), "", false)) {
                ...
                if (!dlg.snapshot_to_activate().empty()) {
                    ...
                    try {
                        ...
                        // Enable substitutions, log both user and system substitutions. There should not be any substitutions performed
                        // when loading system presets because compatibility of profiles shall be verified using the min_slic3r_version keys
                        // in config index, but users are known to be creative and mess with the config files in various ways.
    ...
    if (mode_menu != nullptr) {
        auto modfn = [this](int mode, wxCommandEvent &) {
            if (get_mode() != mode)
                save_mode(mode);
        };
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening FlsunSlicer by drag & dropping a G-Code onto FlsunSlicer icon in Finder,
...
wxBookCtrlBase *GUI_App::tab_panel() const { return mainframe->m_tabpanel; }

NotificationManager *GUI_App::notification_manager() { return plater_->get_notification_manager(); }

GalleryDialog *GUI_App::gallery_dialog() { return mainframe->gallery_dialog(); }
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
{
    ...
    bool bnm;
    if (start_page == ConfigWizard::SP_FILAMENTS || start_page == ConfigWizard::SP_PRINTERS) {
        bnm = wizard->run(reason, start_page, false);

    } else {
        bnm = wizard->run(reason, start_page);
    }
    const bool res = bnm;
...
bool GUI_App::open_browser_with_warning_dialog(const wxString &url,
                                               ...
                                               int             flags /* = 0*/)
{
    ...
    if (force_remember_choice || app_config->get(option_key).empty()) {
        if (app_config->get(option_key).empty()) {
            RichMessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("FlsunSlicer: Open hyperlink"),
                                     wxICON_QUESTION | wxYES_NO);
            ...
            if (dialog.IsCheckBoxChecked()) {
                ...
                MessageDialog msg_dlg(parent, msg, _L("FlsunSlicer: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
    ...
    // and will be shown only when "Suppress to open hyperlink in browser" is ON.
        ...
        MessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("FlsunSlicer: Open hyperlink"),
                             wxICON_QUESTION | wxYES_NO);
...
#ifdef __WXMSW__
void GUI_App::associate_3mf_files() { associate_file_type(L".3mf", L"Prusa.Slicer.1", L"FlsunSlicer", true); }

void GUI_App::associate_stl_files() { associate_file_type(L".stl", L"Prusa.Slicer.1", L"FlsunSlicer", true); }

void GUI_App::associate_gcode_files() { associate_file_type(L".gcode", L"FlsunSlicer.GCodeViewer.1", L"PrusaSlicerGCodeViewer", true); }
void GUI_App::associate_bgcode_files() { associate_file_type(L".bgcode", L"FlsunSlicer.GCodeViewer.1", L"PrusaSlicerGCodeViewer", true); }
#endif // __WXMSW__
...
void GUI_App::on_version_read(wxCommandEvent &evt)
{
    ...
    if (*Semver::parse(SLIC3R_VERSION) >= *Semver::parse(into_u8(evt.GetString()))) {
        if (m_app_updater->get_triggered_by_user()) {
            ...
            if (*Semver::parse(SLIC3R_VERSION) > *Semver::parse(into_u8(evt.GetString())))
                text = Slic3r::format(_u8L("There are no new released versions online. The latest release version is %1%."),
                                      evt.GetString());
...
void GUI_App::app_updater(bool from_user)
{
    ...
    if (dialog_result != wxID_OK) {
        ...
        /*int exitCode = wxExecute("C:\\Program Files\\Everything\\Everything.exe");
        if (exitCode == -1) {
            BOOST_LOG_TRIVIAL(info) << "There is error.";
        }*/
        return;
    }
    app_data.target_path = dwnld_dlg.get_download_path();
    // start download
    this->plater_->get_notification_manager()->push_download_progress_notification(GUI::format(_L("Downloading %1%"),
                                                                                               app_data.target_path.filename().string()),
                                                                                   std::bind(&AppUpdater::cancel_callback,
                                                                                             this->m_app_updater.get()));
...
void GUI_App::app_version_check(bool from_user)
{
    if (from_user) {
        if (m_app_updater->get_download_ongoing()) {
            MessageDialog msgdlg(nullptr, _L("Downloading of the new version is in progress. Do you want to continue?"), _L("Notice"),
                                 wxYES_NO);
...
void GUI_App::open_wifi_config_dialog(bool forced, const wxString &drive_path /* = {}*/)
{
    ...
    if (dialog.ShowModal() == wxID_OK) {
        plater_->get_notification_manager()->push_exporting_finished_notification(file_path,
                                                                                  boost::filesystem::path(file_path).parent_path().string(),
                                                                                  true);
