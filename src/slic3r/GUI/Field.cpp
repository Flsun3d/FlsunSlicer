    ...
    case coFloatOrPercent: {
        ...
        if ((m_opt.type == coFloatOrPercent || m_opt.type == coFloatsOrPercents) && !str.IsEmpty() && str.Last() != '%') {
            ...
            if (!str.ToDouble(&val)) {
                ...
                wxString          msg_text = _L("For this parameter, select YES if you want a percentage, otherwise select NO.");
                // TRN %1% = Value, %2% = units
                /*if (sidetext == "mm") {
                    msg_text = format_wxstr(_L("Do you mean %1%%% instead of %1% mm?\n"
                                               "Select YES if you want to change this value to %1%%%, \n"
                                               "or NO if you are sure that %1% mm is a correct value."),
                                            stVal);
                } else {
                    msg_text = format_wxstr(_L("Do you mean %1%%% instead of %1% mm/s?\n"
                                               "Select YES if you want to change this value to %1%%%, \n"
                                               "or NO if you are sure that %1% mm/s is a correct value."),
                                            stVal);
                }*/
...
void TextCtrl::BUILD() {
	...
	temp->Bind(wxEVT_LEFT_DOWN, ([temp, this](wxEvent &event)
	{
...
#else
        if (m_opt.label == L("Supports") || m_opt.label == L("Brim")) {
            size           = wxSize(15 * wxGetApp().em_unit(), 2 * wxGetApp().em_unit());
        }
        temp = new choice_ctrl(m_parent, wxID_ANY, wxString(""), wxDefaultPosition, size, 0, nullptr,
...
{
    if (m_opt.type == coStrings || m_opt.type == coEnum) {
        on_change_field();
        return;
    }
...
boost::any& Choice::get_value()
{
	...
	std::vector <std::string> right_panel_options{ "support","Brim" "pad", "scale_unit" };
