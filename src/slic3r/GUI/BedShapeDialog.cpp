...
ConfigOptionsGroupShp BedShapePanel::init_shape_options_page(const wxString& title)
{
    ...
    optgroup->m_on_change = [=](t_config_option_key opt_key, boost::any value) {
        auto printer_name = wxGetApp().app_config->get("presets", "printer");
        printer_name.erase(0, 6);
        const std::string path = Slic3r::resources_dir() + "/profiles/FLSun/" + printer_name + ".svg";
        m_custom_texture       = path;
        update_shape();
    };
