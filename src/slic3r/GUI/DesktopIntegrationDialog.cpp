...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that FlsunSlicer.desktop exists
    struct stat buffer;
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon FlsunSlicer.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "icons")) {
            target_dir_icons      = target_candidates[i];
            std::string icon_path = GUI::format("%1%/icons/FlsunSlicer.png", resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%FlsunSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if (target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying FlsunSlicer icon to icons directory failed.";
    } else
        // save path to icon
        app_config->set("desktop_integration_icon_slicer_path",
                        GUI::format("%1%/icons/%2%FlsunSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            std::string path = GUI::format("%1%/applications/FlsunSlicer%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file)) {
                candidate_found = true;
                BOOST_LOG_TRIVIAL(debug) << "FlsunSlicer.desktop file installation success.";
                break;
            } else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to FlsunSlicer.desktop file installation failed. failed path: "
                                         << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path",
                    GUI::format("%1%/applications/FlsunSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        // Icon
        if (!target_dir_icons.empty()) {
            std::string icon_path = GUI::format("%1%/icons/FlsunSlicer-gcodeviewer_192px.png", resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%FlsunSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path,
                                                version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file_viewer = GUI::format("[Desktop Entry]\n"
                                                      ...
                                                      "GenericName=3D Printing Software\n"
                                                      "Icon=FlsunSlicer-gcodeviewer%2%\n"
        ...
        if (create_desktop_file(desktop_path, desktop_file_viewer))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. FlsunSlicer desktop "
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration()
{
    ...
    std::string desktop_file_downloader = GUI::format("[Desktop Entry]\n"
                                                      "Name=FlsunSlicer URL Protocol%1%\n"
                                                      "Exec=\"%2%\" --single-instance %%u\n"
                                                      "Terminal=false\n"
                                                      "Type=Application\n"
                                                      "MimeType=x-scheme-handler/FlsunSlicer;\n"
                                                      "StartupNotify=false\n"
                                                      "NoDisplay=true\n",
    ...
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default PrusaSlicerURLProtocol%1%.desktop x-scheme-handler/FlsunSlicer", version_suffix);
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            if (create_desktop_file(path, desktop_file_downloader)) {
                ...
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to PrusaSlicerURLProtocol.desktop file installation failed. failed path: "
                                         << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
