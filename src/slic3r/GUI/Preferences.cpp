...
void PreferencesDialog::build()
{
	...
	if (is_editor) {
		...
		append_bool_option(m_optgroup_general, "associate_3mf",
			L("Associate .3mf files to FlsunSlicer"),
			L("If enabled, sets FlsunSlicer as default application to open .3mf files."),
			app_config->get_bool("associate_3mf"));

		append_bool_option(m_optgroup_general, "associate_stl",
			L("Associate .stl files to FlsunSlicer"),
			L("If enabled, sets FlsunSlicer as default application to open .stl files."),
...
#if __APPLE__
			L("Allow just a single FlsunSlicer instance"),
...
#else
			L("Allow just a single FlsunSlicer instance"),
			L("If this is enabled, when starting FlsunSlicer and another instance of the same FlsunSlicer is already running, that instance will be reactivated instead."),
#endif
		...
		append_bool_option(m_optgroup_general, "default_action_on_close_application",
			...
			L("Always ask for unsaved changes in presets, when: \n"
						"- Closing FlsunSlicer while some presets are modified,\n"
		...
		append_bool_option(m_optgroup_general, "associate_gcode",
			L("Associate .gcode files to FlsunSlicer G-code Viewer"),
			L("If enabled, sets FlsunSlicer G-code Viewer as default application to open .gcode files."),
			app_config->get_bool("associate_gcode"));
		append_bool_option(m_optgroup_general, "associate_bgcode",
			L("Associate .bgcode files to FlsunSlicer G-code Viewer"),
			L("If enabled, sets FlsunSlicer G-code Viewer as default application to open .bgcode files."),
	...
	append_bool_option(m_optgroup_general, "restore_win_position",
		...
		L("If enabled, FlsunSlicer will be open at the position it was closed"),
...
/*		if (opt_key == "suppress_hyperlinks")
	...
	if (is_editor) {
		append_bool_option(m_optgroup_gui, "show_collapse_button",
			L("Show sidebar fold/expand buttons"),
...
/*
		append_bool_option(m_optgroup_gui, "suppress_hyperlinks",
			...
			L("If enabled, FlsunSlicer will not open a hyperlinks in your browser."),
...
*/
  ...
  //                  app_config->get_bool("use_binary_gcode_when_supported"));
		...
		/*append_bool_option(m_optgroup_other, "downloader_url_registered",
			...
			L("If enabled, FlsunSlicer will be allowed to download from Printables.com"),
    ...
    buttons->GetAffirmativeButton()->SetLabel(_L("OK"));
    buttons->GetCancelButton()->SetLabel(_L("Cancel"));
