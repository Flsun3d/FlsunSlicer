                                                 ...
                                                 const std::vector<std::pair<std::string, std::string>> &failed_models)
{
    ...
    if (!succes_models.empty()) {
        msg = _L_PLURAL("The following model was repaired successfully", "The following models were repaired successfully",
                        succes_models.size()) +
              ":";
...
wxMenu* MenuFactory::append_submenu_add_generic(wxMenu* menu, ModelVolumeType type)
{
    ...
    std::vector<const char *> defaulmodel1 = {L("Box"), L("Cylinder"), L("Sphere"), L("Slab"), L("Boat")};
    std::vector<const char *> defaulmodel2 = {L("Box"), L("Cylinder"), L("Sphere"), L("Slab")};
    std::vector<const char *> defaulmodel  = type == ModelVolumeType::INVALID ? defaulmodel1 : defaulmodel2;
    // if (!(type == ModelVolumeType::MODEL_PART && mode == comAdvanced))
    for (auto &item : defaulmodel)
        {
