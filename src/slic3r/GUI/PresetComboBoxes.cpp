///|/ Copyright (c) Prusa Research 2020 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Vojtěch
///Bubník @bubnikv, Filip Sykala @Jony01, Lukáš Matěna @lukasmatena, Tomáš Mészáros @tamasmeszaros
...
static std::string suffix(const Preset &preset) { return (preset.is_dirty ? Preset::suffix_modified() : ""); }

static std::string suffix(Preset *preset) { return (preset->is_dirty ? Preset::suffix_modified() : ""); }

wxString PresetComboBox::get_preset_name(const Preset &preset) { return from_u8(preset.name); }

static wxString get_preset_name_with_suffix(const Preset &preset) { return from_u8(preset.name + Preset::suffix_modified()); }
...
void PresetComboBox::update(std::string select_preset_name)
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i) {
        ...
        std::string main_icon_name = m_type == Preset::TYPE_PRINTER && preset.printer_technology() == ptSLA ? "sla_printer" :
                                                                                                              m_main_bitmap_name;

        auto bmp = get_bmp(bitmap_key, main_icon_name, "lock_closed", is_enabled, preset.is_compatible,
                                        ...
                                        const std::string &material_rgb /* = ""*/)
{
    ...
    if (bmp_bndl == nullptr) {
        ...
        if (m_type == Preset::TYPE_FILAMENT && !filament_rgb.empty()) {
            ...
            bmps.emplace_back(get_solid_bmp_bundle(wide_icon_width, icon_height, is_single_bar ? filament_rgb : extruder_rgb));
            /* if (!is_single_bar)
        ...
        bmps.emplace_back(/*is_system ? get_bmp_bundle("lock_closed") :*/ get_empty_bmp_bundle(/*norm_icon_width*/1, icon_height));//去除了右边的下拉框里的图标
...
PlaterPresetComboBox::PlaterPresetComboBox(wxWindow *parent, Preset::Type preset_type)
    : PresetComboBox(parent, preset_type, wxSize(15 * wxGetApp().em_unit(),2 * wxGetApp().em_unit()))
{
    ...
    auto old_color = parent->GetBackgroundColour();
    edit_btn = new ScalableButton(parent, wxID_ANY, "edit_prest");

    edit_btn->SetToolTip(_L("Parameter setting"));
    //edit_btn->SetBackgroundColour(old_color);
    edit_btn->Bind(wxEVT_ENTER_WINDOW, [=](wxMouseEvent &event) {
        edit_btn->SetBackgroundColour(wxOSX ? wxColor(255, 255, 255) : Slic3r::GUI::wxGetApp().get_color_selected_btn_bg());
    });
    edit_btn->Bind(wxEVT_LEAVE_WINDOW, [=](wxMouseEvent &event) { edit_btn->SetBackgroundColour(old_color); });
    edit_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent) {
        if (m_type == Preset::TYPE_PRINTER) {
            // wxGetApp().mainframe->m_printer_dialog->m_dialog->OnActivate();
            wxGetApp().mainframe->m_printer_dialog->ShowModal();

            // show_edit_menu();
        } else if (m_type == Preset::TYPE_FILAMENT) {
            // wxGetApp().mainframe->m_filament_dialog->m_dialog->OnActivate();
            wxGetApp().mainframe->m_filament_dialog->ShowModal();

        } else if (m_type == Preset::TYPE_PRINT) {
            // wxGetApp().mainframe->m_print_dialog->m_dialog->OnActivate();
            wxGetApp().mainframe->m_print_dialog->ShowModal();
        }
        // switch_to_tab();
    });
}
...
void PlaterPresetComboBox::show_add_menu()
{
    ...
    append_menu_item(
        ...
        [](wxCommandEvent &) { wxTheApp->CallAfter([]() { run_wizard(ConfigWizard::SP_PRINTERS); }); }, "edit_uni", menu,
        []() { return true; }, wxGetApp().plater());

    append_menu_item(
        menu, wxID_ANY, _L("Add physical printer"), "",
        [this](wxCommandEvent &) {
...
void PlaterPresetComboBox::show_edit_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Add/Remove presets"), "",
        [](wxCommandEvent &) { wxTheApp->CallAfter([]() { run_wizard(ConfigWizard::SP_FILAMENTS); }); }, "edit_uni", menu,
        []() { return true; }, wxGetApp().plater());
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        append_menu_item(
            menu, wxID_ANY, _L("Change extruder color"), "", [this](wxCommandEvent &) { this->change_extruder_color(); }, "funnel", menu,
            []() { return true; }, wxGetApp().plater());
#endif //__linux__
        /*append_menu_item(menu, wxID_ANY, _L("Show/Hide template presets"), "",
            [](wxCommandEvent&) { wxGetApp().open_preferences("no_templates", "General"); }, "spool", menu, []() { return true; },
           wxGetApp().plater());*/
    ...
    if (this->is_selected_physical_printer()) {
        ...
        if (!host.empty()) {
            append_menu_item(
                menu, wxID_ANY, _L("Open the physical printer URL"), "", [this](wxCommandEvent &) { this->open_physical_printer_url(); },
                "open_browser", menu, []() { return true; }, wxGetApp().plater());
        }

        append_menu_item(
            menu, wxID_ANY, _L("Delete physical printer"), "", [this](wxCommandEvent &) { this->del_physical_printer(); }, "cross", menu,
            []() { return true; }, wxGetApp().plater());
    } else
        append_menu_item(
            menu, wxID_ANY, _L("Add/Remove presets"), "",
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Add physical printer"), "", [this](wxCommandEvent &) { this->add_physical_printer(); }, "edit_uni", menu,
        []() { return true; }, wxGetApp().plater());
...
{
    std::string name = preset.alias.empty() ? preset.name :
                                              (preset.vendor && preset.vendor->templates_profile ? preset.name : preset.alias);
    return from_u8(name + suffix(preset));
}
wxString PlaterPresetComboBox::get_print_name(const Preset &preset) 
{ 
    std::string name = preset.alias.empty() ? preset.name :
                                              (preset.vendor && preset.vendor->templates_profile ? preset.name : preset.alias);
    return from_u8(name);
}
wxString select_filament = "";
...
void PlaterPresetComboBox::update()
{
    ...
    std::vector<wxString>   print_name;
    ...
    if (m_type == Preset::TYPE_PRINT) {
        ...
        {
            const Preset &preset = presets[i];
            bool is_selected = m_type == Preset::TYPE_FILAMENT ? selected_filament_preset->name == preset.name :
            ...
            if (!preset.is_visible || (!is_compatible && !is_selected))
                continue;
            print_name.push_back(get_print_name(preset));
            wxGetApp().print_name = print_name;
            //print_name_prest[get_preset_name(preset)] = &presets[i];
        }
    }
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i) {
        const Preset &preset      = presets[i];
        ...
        std::string bitmap_type_name = bitmap_key = m_type == Preset::TYPE_PRINTER && preset.printer_technology() == ptSLA ?
                                                        "sla_printer" :
                                                        m_main_bitmap_name;
        ...
        if (m_type == Preset::TYPE_FILAMENT) {
            ...
            bitmap_key += single_bar ? filament_rgb : extruder_rgb;
        } else if (m_type == Preset::TYPE_SLA_MATERIAL) {
            material_rgb = is_selected ? m_preset_bundle->sla_materials.get_edited_preset().config.opt_string("material_colour") :
        ...
        auto bmp = get_bmp(bitmap_key, wide_icons, bitmap_type_name, is_compatible, preset.is_system || preset.is_default, single_bar,
                           filament_rgb, extruder_rgb, material_rgb);
        assert(bmp);
        ...
        if (preset.is_default || preset.is_system) {
            if (preset.vendor && preset.vendor->templates_profile) {
                ...
                if (is_selected) {
                    ...
                    if (m_type == Preset::TYPE_FILAMENT) 
                    {
                        select_filament = selected_user_preset;
                        select_filament.Remove(0, 8);
                        size_t pos = select_filament.find('_');
                        if (pos != std::string::npos) {
                            select_filament.erase(pos);
                        } 
                        wxGetApp().filement_name = select_filament.ToStdString();
                    }
                }
            }
        } else {
    ...
    if (m_type == Preset::TYPE_PRINTER) {
        ...
        if (!m_preset_bundle->physical_printers.empty()) {
            ...
            std::sort(preset_data.begin(), preset_data.end(),
                      [](const PhysicalPrinterPresetData &a, const PhysicalPrinterPresetData &b) { return a.lower_name < b.lower_name; });
...
wxString TabPresetComboBox::get_preset_name(const Preset &preset) { return from_u8(preset.name + suffix(preset)); }
