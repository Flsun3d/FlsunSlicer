    ...
    if (!m_gl_initialized) {
...
#if ENABLE_OPENGL_ES
                _L("FlsunSlicer requires OpenGL ES 2.0 capable graphics driver to run correctly, \n"
...
#elif ENABLE_GL_CORE_PROFILE
                _L("FlsunSlicer requires OpenGL 3.2 capable graphics driver to run correctly,\n"
                   "while OpenGL version %s, render %s, vendor %s was detected."), s_gl_info.get_version_string(), s_gl_info.get_renderer(), s_gl_info.get_vendor());
#else
                _L("FlsunSlicer requires OpenGL 2.0 capable graphics driver to run correctly, \n"
                   "while OpenGL version %s, render %s, vendor %s was detected."), s_gl_info.get_version_string(), s_gl_info.get_renderer(), s_gl_info.get_vendor());
#endif // ENABLE_OPENGL_ES
            message += "\n";
...
#ifdef _WIN32
            message += "\n";
            message += _L("As a workaround, you may run FlsunSlicer with a software rendered 3D graphics by running prusa-slicer.exe with the --sw-renderer parameter.");
#endif
        	wxMessageBox(message, wxString("FlsunSlicer - ") + _L("Unsupported OpenGL version"), wxOK | wxICON_ERROR);
        }
        ...
        if (valid_version) {
            ...
            if (!result) {
                ...
                wxMessageBox(message, wxString("FlsunSlicer - ") + _L("Error loading shaders"), wxOK | wxICON_ERROR);
            }
#if ENABLE_GL_CORE_PROFILE
        ...
        // texture of the bed (see: https://github.com/prusa3d/FlsunSlicer/issues/8417).
...
//        std::cout << "Multisample " << (can_multisample() ? "enabled" : "disabled") << std::endl;
    ...
    {
        attribList.Reset();
        attribList.PlatformDefaults().RGBA().DoubleBuffer().MinRGBA(1, 1, 1, 1).Depth(24).EndList();
#ifdef __APPLE__
        // on MAC the method RGBA() has no effect
        attribList.SetNeedsARB(true);
#endif // __APPLE__
