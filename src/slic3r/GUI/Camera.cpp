...
void Camera::zoom_to_box(const BoundingBoxf3& box, double margin_factor)
{
    ...
    const double zoom = calc_zoom_to_bounding_box_factor(box, 1.5);
    if (zoom > 0.0) {
        m_zoom = zoom;
...
void Camera::set_default_orientation()
{
    ...
    const double phi_rad    = Geometry::deg2rad(0);
    ...
    // Camera &camera = Slic3r::GUI::wxGetApp().plater()->get_camera();
    if (!wxGetApp().app_config->get_bool("use_free_camera"))
        this->recover_from_free_camera();
    this->set_target(Vec3d(0.0, 0.0, 0.0));
    this->set_zoom(2.7);
}
