...
public:
    ...
    wxMenu *CreatePopupMenu() override
    {
        ...
        if (wxGetApp().app_config->get("single_instance") == "0") {
            ...
            append_menu_item(
                menu, wxID_ANY, _L("Open new instance"), _L("Open a new FlsunSlicer instance"),
...
public:
    ...
    wxMenu *CreatePopupMenu() override
    {
        wxMenu *menu = new wxMenu;
        append_menu_item(
            menu, wxID_ANY, _L("Open FlsunSlicer"), _L("Open a new FlsunSlicer instance"),
...
static wxIcon main_frame_icon(GUI_App::EAppMode app_mode)
{
    ...
    if (len > 0 && len < MAX_PATH) {
        ...
        if (app_mode == GUI_App::EAppMode::GCodeViewer) {
            // Only in case the slicer was started with --gcodeviewer parameter try to load the icon from flsun-gcodeviewer.exe
            // Otherwise load it from the exe.
            for (const std::wstring_view exe_name : {std::wstring_view(L"FlsunSlicer.exe"), std::wstring_view(L"flsun-slicer-console.exe")})
                if (boost::iends_with(path, exe_name)) {
                    path.erase(path.end() - exe_name.size(), path.end());
                    path += L"flsun-gcodeviewer.exe";
                    break;
                }
        }
    }
...
#else  // _WIN32
    return wxIcon(Slic3r::var(app_mode == GUI_App::EAppMode::Editor ? "FlsunSlicer_128px.png" : "FlsunSlicer_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
#define BORDERLESS_FRAME_STYLE (wxRESIZE_BORDER | wxMINIMIZE_BOX | wxMAXIMIZE_BOX | wxCLOSE_BOX)

#if _WIN32
BEGIN_EVENT_TABLE(MainFrame, DPIFrame)
EVT_LEFT_DOWN(MainFrame::OnLeftDown)
EVT_LEFT_UP(MainFrame::OnLeftUp)
EVT_MOUSE_CAPTURE_LOST(MainFrame::OnMouseLost)
EVT_MOTION(MainFrame::OnMotion)
END_EVENT_TABLE()
#endif

#define BITTONBARID 2200
...
MainFrame::MainFrame(const int font_point_size)
    : DPIFrame(NULL,
               ...
               BORDERLESS_FRAME_STYLE /* | wxFRAME_SHAPED*/,
               "mainframe",
               font_point_size)
    ...
    , diff_dialog(this)
{
    ...
    case GUI_App::EAppMode::Editor:
        ...
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("FlsunSlicer.png"), wxBITMAP_TYPE_PNG), "FlsunSlicer");
        break;
    case GUI_App::EAppMode::GCodeViewer:
        m_taskbar_icon = std::make_unique<GCodeViewerTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("FlsunSlicer.png"), wxBITMAP_TYPE_PNG), "G-code Viewer");
        break;
    }
#endif // __APPLE__
#ifdef _WIN32
    ...
    sizer->Add(m_main_sizer, 1, wxEXPAND /* | wxALL, 3*/);
    SetSizer(sizer);
    ...
    Bind(wxEVT_CLOSE_WINDOW, [this](wxCloseEvent &event) {
        ...
        if (m_plater != nullptr) {
            int saved_project = m_plater->save_project_if_dirty(_L("Closing FlsunSlicer. Current project is modified."));
            ...
            // check unsaved changes only if project wasn't saved
                     ...
                     (plater()->is_presets_dirty() &&
                      !wxGetApp().check_and_save_current_preset_changes(_L("FlsunSlicer is closing"),
                                                                        _L("Closing FlsunSlicer while some presets are modified.")))) {
                event.Veto();
                return;
            }
        }
...
// FIXME maybe this is useful for __WXGTK3__ as well?
    ...
    Bind(wxEVT_SIZE, [this](wxSizeEvent &) {
        ...
        auto      nowsize = this->GetSize();
        auto      scale   = (double) nowsize.x / (double) size.x;
        wxGetApp().plater()->get_camera().set_zoom(2.7 * scale);
        Refresh();
        Layout();
    });
#ifdef WIN32
...
static void     append_tab_menu_items_to_menubar(wxMenuBar *bar, PrinterTechnology pt, bool is_mainframe_menu)
{
    if (is_mainframe_menu)
        bar->Append(new wxMenu(), pref() + _L("Prepare") + suff());
    for (const wxString &title : {is_mainframe_menu ? _L("Print Settings") : pref() + _L("Print Settings") + suff(),
                                  pt == ptSLA ? _L("Material Settings") : _L("Filament Settings"), _L("Printer Settings")})
    ...
    bar_parent->Bind(wxEVT_MENU_OPEN, [main_frame, bar, is_mainframe_menu](wxMenuEvent &event) {
        ...
        if (!menu || menu->GetMenuItemCount() > 0) {
            ...
            event.Skip(); // event.Skip() is verry important to next processing of the wxEVT_UPDATE_UI by this menu items.
                          // If wxEVT_MENU_OPEN will not be pocessed in next event queue then MenuItems of this menu will never caught
                          // wxEVT_UPDATE_UI and, as a result, "check/radio value" will not be updated
            return;
        }
        ...
        if (title == _L("Prepare"))
            ...
            main_frame->select_tab(
                wxGetApp().get_tab(main_frame->plater()->printer_technology() == ptFFF ? Preset::TYPE_PRINT : Preset::TYPE_SLA_PRINT));
...
void MainFrame::update_layout()
{
    ...
    ESettingsLayout layout = wxGetApp().is_gcode_viewer() ?
                                 ...
                                 (wxGetApp().app_config->get_bool("old_settings_layout_mode") ?
                                      ESettingsLayout::Old :
                                  wxGetApp().app_config->get_bool("new_settings_layout_mode") ?
                                      (wxGetApp().tabs_as_menu() ? ESettingsLayout::Old : ESettingsLayout::New) :
                                  wxGetApp().app_config->get_bool("dlg_settings_layout_mode") ? ESettingsLayout::Dlg :
                                                                                                ESettingsLayout::Old);
...
#ifdef __WXMSW__
    enum class State { noUpdate, fromDlg, toDlg };
    State update_scaling_state = // m_layout == ESettingsLayout::Unknown   ? State::noUpdate   : // don't scale settings dialog from the
                                 // application start
    ...
    case ESettingsLayout::Old: {
        ...
        //#ifdef _MSW_DARK_MODE
        m_plater->Layout();
        m_preview_page         = m_plater;
        const std::string path = Slic3r::resources_dir() + "/icons/";
        if (!wxGetApp().tabs_as_menu())
            dynamic_cast<Notebook *>(m_tabpanel)->InsertPage(0, m_plater, _L("Prepare"), std::string(path + "prepare.png"), true);
        // else
        //#endif
        //  m_tabpanel->InsertPage(0, m_plater, _L("Prepare"));
...
#ifdef _MSW_DARK_MODE
        if (!wxGetApp().tabs_as_menu())
            dynamic_cast<Notebook *>(m_tabpanel)->InsertPage(0, m_plater_page, _L("Prepare"), std::string("plater"), true);
        else
#endif
            m_tabpanel->InsertPage(0, m_plater_page, _L("Prepare")); // empty panel just for Plater tab */
        m_plater->Show();
        break;
    }
...
void MainFrame::init_tabpanel()
{
    ...
    m_tabpanel->Bind(wxEVT_NOTEBOOK_PAGE_CHANGED, [this](wxBookCtrlEvent &e) {
        ...
        auto      bnm   = m_tabpanel->GetSelection();
...
void MainFrame::create_preset_tabs()
{
    ...
    m_printer_dialog     = new MyPrintDialog(_L("Printer Settings"), wxSize(screenSize.x * 0.7, screenSize.y * 0.7));
    m_filament_dialog    = new MyPrintDialog(_L("Filament Settings"), wxSize(screenSize.x * 0.7, screenSize.y * 0.7));
    m_print_dialog       = new MyPrintDialog(_L("Print Settings"), wxSize(screenSize.x * 0.7, screenSize.y * 0.7));
    m_dialog_vec.push_back(m_printer_dialog);
    m_dialog_vec.push_back(m_filament_dialog);
    m_dialog_vec.push_back(m_print_dialog);

    wxPanel *         m_priview         = new wxPanel(m_tabpanel, wxID_ANY);
    WebPage *         m_monitoring_page = new WebPage(m_tabpanel, wxID_ANY);
    const std::string path              = Slic3r::resources_dir() + "/icons/";

    dynamic_cast<Notebook *>(m_tabpanel)->AddPage(m_priview, _L("Preview"), path + "preview.png");
    dynamic_cast<Notebook *>(m_tabpanel)->AddPage(m_monitoring_page, _L("Device"), path + "device.png");
}
...
bool MainFrame::can_export_gcode() const
{
    if (m_plater == nullptr)
        return false;
    if (!m_plater->sidebar().is_enable_export()) 
    {
        return false;
    }
...
bool MainFrame::can_eject() const { return wxGetApp().removable_drive_manager()->status().has_eject; }
...
bool MainFrame::can_select() const { return (m_plater != nullptr) && !m_plater->model().objects.empty(); }

bool MainFrame::can_deselect() const { return (m_plater != nullptr) && !m_plater->is_selection_empty(); }

bool MainFrame::can_delete() const { return (m_plater != nullptr) && !m_plater->is_selection_empty(); }

bool MainFrame::can_delete_all() const { return (m_plater != nullptr) && !m_plater->model().objects.empty(); }
...
static void init_macos_application_menu(wxMenuBar *menu_bar, MainFrame *main_frame)
{
    ...
    if (apple_menu != nullptr) {
        ...
        apple_menu->Bind(
            wxEVT_MENU, [main_frame](wxCommandEvent &) { main_frame->Close(); }, wxID_EXIT);
    }
}
#endif // __APPLE__
static wxMenu *generate_help_menu()
{
    ...
    //        wxString::Format(_L("Open the %s website in your browser"), SLIC3R_APP_NAME),
    //        [](wxCommandEvent&) {
    //        wxGetApp().open_browser_with_warning_dialog("https://help.prusa3d.com/article/first-print-with-prusaslicer_1753", nullptr,
    //        false); });
    //    // TRN Item from "Help" menu
    //    append_menu_item(helpMenu, wxID_ANY, wxString::Format(_L("Sample &G-codes and Models"), SLIC3R_APP_NAME),
    //        wxString::Format(_L("Open the %s website in your browser"), SLIC3R_APP_NAME),
    //        [](wxCommandEvent&) { wxGetApp().open_browser_with_warning_dialog("https://help.prusa3d.com/article/sample-g-codes_529630",
    //        nullptr, false); });
    ...
    //        [](wxCommandEvent&) { wxGetApp().open_browser_with_warning_dialog("https://github.com/prusa3d/PrusaSlicer/releases", nullptr,
    //        false); });
    ...
    //        [](wxCommandEvent&) { wxGetApp().open_browser_with_warning_dialog("https://github.com/prusa3d/slic3r/issues/new", nullptr,
    //        false); });
    //    if (wxGetApp().is_editor())
    //#ifndef __APPLE__
...
#define MENUITEM_ID 8888
static void add_common_view_menu_items(wxMenu *view_menu, MainFrame *mainFrame, std::function<bool(void)> can_change_view)
{
    // The camera control accelerators are captured by GLCanvas3D::on_char().
    append_menu_item(
        view_menu, wxID_ANY, _L("Iso") + sep + "&0", _L("Iso View"), [mainFrame](wxCommandEvent &) { mainFrame->select_view("iso"); }, "",
        nullptr, [can_change_view]() { return can_change_view(); }, mainFrame);
    view_menu->AppendSeparator();
    // TRN Main menu: View->Top
    append_menu_item(
        view_menu, wxID_ANY, _L("Top") + sep + "&1", _L("Top View"), [mainFrame](wxCommandEvent &) { mainFrame->select_view("top"); }, "",
        nullptr, [can_change_view]() { return can_change_view(); }, mainFrame);
    // TRN Main menu: View->Bottom
    append_menu_item(
        view_menu, wxID_ANY, _L("Bottom") + sep + "&2", _L("Bottom View"),
        [mainFrame](wxCommandEvent &) { mainFrame->select_view("bottom"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Front") /* + sep + "&3"*/, _L("Front View"),
        [mainFrame](wxCommandEvent &) { mainFrame->select_view("front"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Rear") /*+ sep + "&4"*/, _L("Rear View"),
        [mainFrame](wxCommandEvent &) { mainFrame->select_view("rear"); }, "", nullptr, [can_change_view]() { return can_change_view(); },
        mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Left") + sep + "&3", _L("Left View"), [mainFrame](wxCommandEvent &) { mainFrame->select_view("left"); },
        "", nullptr, [can_change_view]() { return can_change_view(); }, mainFrame);
    append_menu_item(
        view_menu, wxID_ANY, _L("Right") + sep + "&4", _L("Right View"), [mainFrame](wxCommandEvent &) { mainFrame->select_view("right"); },
        "", nullptr, [can_change_view]() { return can_change_view(); }, mainFrame);
}
...
void MainFrame::init_menubar_as_editor()
{
    ...
    {
        append_menu_item(
            fileMenu, wxID_NEW, _L("&New Project") + "\tCtrl+N", _L("Start a new project"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        append_menu_item(
            fileMenu, wxID_OPEN, _L("&Open Project") + dots + "\tCtrl+O", _L("Open a project file"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        recent_projects_menu                = new wxMenu();
        wxMenuItem *recent_projects_submenu = append_submenu(fileMenu, recent_projects_menu, wxID_HIGHEST, _L("Recent projects"), "");
        m_recent_projects.UseMenu(recent_projects_menu);
        recent_projects_menu->Bind(
            ...
            [this](wxCommandEvent &evt) {
                ...
                if (wxFileExists(filename)) {
                    ...
                    // wxMessageDialog msg(this, _L("The selected project is no longer available.\nDo you want to remove it from the recent
                    // projects list?"), _L("Error"), wxYES_NO | wxYES_DEFAULT);
                    MessageDialog
                        msg(this,
        ...
        /* recent_projects_menu->Bind(
             ...
             [=](wxUpdateUIEvent &evt)
             {
                 evt.Enable(m_recent_projects.GetCount() > 0);
             },
             recent_projects_submenu->GetId());*/

        append_menu_item(
            fileMenu, wxID_SAVE, _L("&Save Project") + "\tCtrl+S", _L("Save current project file"),
            [this](wxCommandEvent &) { save_project(); }, "save", nullptr, [this]() { return m_plater != nullptr && can_save(); }, this);
#ifdef __APPLE__
        append_menu_item(
...
#endif // __APPLE__
            [this](wxCommandEvent &) { save_project_as(); }, "save", nullptr, [this]() { return m_plater != nullptr && can_save_as(); },
            this);
        ...
        append_menu_item(
            import_menu, wxID_ANY, _L("Import Config from &Project") + dots + "\tCtrl+Alt+L", _L("Load configuration from project file"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        append_menu_item(
            ...
            [this](wxCommandEvent &) { load_configbundle(); }, "import_config_bundle", nullptr, []() { return true; }, this);
        append_submenu(fileMenu, import_menu, wxID_ANY, _L("&Import"), "");

        export_menu                   = new wxMenu();
        wxMenuItem *item_export_gcode = append_menu_item(
            export_menu, MENUITEM_ID, _L("Export &G-code") + dots + "\tCtrl+G", _L("Export current plate as G-code"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        wxMenuItem *item_send_gcode = append_menu_item(
            export_menu, MENUITEM_ID + 1, _L("S&end G-code") + dots + "\tCtrl+Shift+G", _L("Send to print current plate as G-code"),
            [this](wxCommandEvent &) {
                if (m_plater)
                    m_plater->send_gcode();
            },
        ...
        append_menu_item(
            export_menu, MENUITEM_ID + 2, _L("Export G-code to SD Card / Flash Drive") + dots + "\tCtrl+U",
        ...
        append_menu_item(
            export_menu, MENUITEM_ID + 3, _L("Export Plate as &STL/OBJ") + dots, _L("Export current plate as STL/OBJ"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        append_menu_item(
            export_menu, MENUITEM_ID + 4, _L("Export Plate as STL/OBJ &Including Supports") + dots,
        ...
        export_menu->AppendSeparator();
        append_menu_item(
            export_menu, MENUITEM_ID + 5, _L("Export &Toolpaths as OBJ") + dots, _L("Export toolpaths as OBJ"),
            [this](wxCommandEvent &) {
                if (m_plater)
        ...
        append_menu_item(
            ...
            [this](wxCommandEvent &) { export_configbundle(); }, "export_config_bundle", nullptr, []() { return true; }, this);
        append_menu_item(
            export_menu, wxID_ANY, _L("Export Config Bundle With Physical Printers") + dots,
            _L("Export all presets including physical printers to file"), [this](wxCommandEvent &) { export_configbundle(true); },
            "export_config_bundle", nullptr, []() { return true; }, this);
        append_submenu(fileMenu, export_menu, wxID_ANY, _L("&Export"), "");

        /*wxMenu *convert_menu = new wxMenu();
       append_menu_item(
           convert_menu, wxID_ANY, _L("Convert ASCII G-code to &binary") + dots, _L("Convert a G-code file from ASCII to binary format"),
           [this](wxCommandEvent &) {
               if (m_plater != nullptr)
       ...
       append_menu_item(
           convert_menu, wxID_ANY, _L("Convert binary G-code to &ASCII") + dots, _L("Convert a G-code file from binary to ASCII format"),
           [this](wxCommandEvent &) {
               if (m_plater != nullptr)
       ...
       append_submenu(fileMenu, convert_menu, wxID_ANY, _L("&Convert"), "");*/
        ...
        /*append_menu_item(fileMenu, wxID_ANY, _L("Ejec&t SD Card / Flash Drive") + dots + "\tCtrl+T", _L("Eject SD card / Flash drive after
           the G-code was exported to it."), [this](wxCommandEvent&) { if (m_plater) m_plater->eject_drive(); }, "eject_sd", nullptr,
           [this]() {return can_eject(); }, this);*/

        fileMenu->AppendSeparator();

        m_menu_item_reslice_now = append_menu_item(
            fileMenu, wxID_MORE, _L("(Re)Slice No&w") + "\tCtrl+R", _L("Start new slicing process"),
            [this](wxCommandEvent &) { reslice_now(); }, "re_slice", nullptr, [this]() { return m_plater != nullptr && can_reslice(); },
            this);
        fileMenu->AppendSeparator();
        append_menu_item(
    ...
    if (m_plater != nullptr) {
...
#endif
        append_menu_item(
            editMenu, MENUITEM_ID + 6, _L("&Select All") + "\tCtrl+A" /*sep + GUI::shortkey_ctrl_prefix() + sep_space + "A"*/,
            _L("Selects all objects"), [this](wxCommandEvent &) { m_plater->select_all(); }, "", nullptr, [this]() { return can_select(); },
            this);
        append_menu_item(
            editMenu, MENUITEM_ID + 7, _L("D&eselect All") + "\tEsc", _L("Deselects all objects"),
            [this](wxCommandEvent &) { m_plater->deselect_all(); }, "", nullptr, [this]() { return can_deselect(); }, this);
        editMenu->AppendSeparator();
        append_menu_item(
            editMenu, MENUITEM_ID + 8, _L("&Delete Selected") + sep + hotkey_delete, _L("Deletes the current selection"),
            [this](wxCommandEvent &) { m_plater->remove_selected(); }, "remove_menu", nullptr, [this]() { return can_delete(); }, this);
        append_menu_item(
            editMenu, MENUITEM_ID + 9, _L("Delete &All") + sep + GUI::shortkey_ctrl_prefix() + sep_space + hotkey_delete,
            _L("Deletes all objects"), [this](wxCommandEvent &) { m_plater->reset_with_confirm(); }, "delete_all_menu", nullptr,
            [this]() { return can_delete_all(); }, this);

        editMenu->AppendSeparator();
        append_menu_item(
            editMenu, MENUITEM_ID + 10, _L("&Undo") + sep + GUI::shortkey_ctrl_prefix() + sep_space + "Z", _L("Undo"),
            [this](wxCommandEvent &) { m_plater->undo(); }, "undo_menu", nullptr, [this]() { return m_plater->can_undo(); }, this);
        append_menu_item(
            editMenu, MENUITEM_ID + 11, _L("&Redo") + sep + GUI::shortkey_ctrl_prefix() + sep_space + "Y", _L("Redo"),
            [this](wxCommandEvent &) { m_plater->redo(); }, "redo_menu", nullptr, [this]() { return m_plater->can_redo(); }, this);

        editMenu->AppendSeparator();
        append_menu_item(
            editMenu, MENUITEM_ID + 12, _L("&Copy") + sep + GUI::shortkey_ctrl_prefix() + sep_space + "C",
            _L("Copy selection to clipboard"), [this](wxCommandEvent &) { m_plater->copy_selection_to_clipboard(); }, "copy_menu", nullptr,
            [this]() { return m_plater->can_copy_to_clipboard(); }, this);
        append_menu_item(
            editMenu, MENUITEM_ID + 13, _L("&Paste") + sep + GUI::shortkey_ctrl_prefix() + sep_space + "V", _L("Paste clipboard"),
            [this](wxCommandEvent &) { m_plater->paste_from_clipboard(); }, "paste_menu", nullptr,
        ...
        //        editMenu->AppendSeparator();
        append_menu_item(
            editMenu, wxID_ANY, _L("Searc&h") + "\tCtrl+F", _L("Search in settings"), [this](wxCommandEvent &) { m_plater->search(false); },
    ...
    auto windowMenu = new wxMenu();
    {
        ...
        append_menu_item(
            ...
            [this /*, tab_offset*/](wxCommandEvent &) { m_print_dialog->ShowModal(); }, "cog", nullptr, []() { return true; }, this);
        wxMenuItem *item_material_tab = append_menu_item(
            ...
            [this /*, tab_offset*/](wxCommandEvent &) { select_tab(2); }, "spool", nullptr, []() { return true; }, this);
        ...
        wxMenuItem *item_printer_tab = append_menu_item(
            ...
            [this /*, tab_offset*/](wxCommandEvent &) { select_tab(3); }, "printer", nullptr, []() { return true; }, this);
        m_changeable_menu_items.push_back(item_printer_tab);
        if (m_plater) {
            windowMenu->AppendSeparator();
            append_menu_item(
                windowMenu, wxID_HIGHEST + 5, _L("3&D") + "\tCtrl+5", _L("Show the 3D editing view"),
                [this](wxCommandEvent &) { m_plater->select_view_3D("3D"); }, "preview_menu", nullptr,
        ...
        windowMenu->AppendSeparator();
        append_menu_item(
            windowMenu, wxID_ANY, _L("Print &Host Upload Queue") + "\tCtrl+J", _L("Display the Print Host Upload Queue window"),
            [this](wxCommandEvent &) { m_printhost_queue_dlg->Show(); }, "upload_queue", nullptr, []() { return true; }, this);

        windowMenu->AppendSeparator();
        append_menu_item(
            windowMenu, wxID_ANY, _L("Open New Instance") + "\tCtrl+Shift+I", _L("Open a new FlsunSlicer instance"),
            [](wxCommandEvent &) { start_new_slicer(); }, "", nullptr,
    ...
    if (m_plater) {
        ...
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show &Labels") + sep + "E", _L("Show object/instance labels in 3D scene"),
            [this](wxCommandEvent &) { m_plater->show_view3D_labels(!m_plater->are_view3D_labels_shown()); }, this,
            [this]() { return m_plater->is_view3D_shown(); }, [this]() { return m_plater->are_view3D_labels_shown(); }, this);
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show Legen&d") + sep + "L", _L("Show legend in preview"),
        ...
        append_menu_check_item(
            ...
            [this](wxCommandEvent &) { m_plater->collapse_sidebar(!m_plater->is_sidebar_collapsed()); }, this, []() { return true; },
            [this]() { return m_plater->is_sidebar_collapsed(); }, this);
#ifndef __APPLE__
    ...
    configurationMenu = wxGetApp().add_config_menu(m_menubar);
...
{
    fileMenuGcodeviewer = new wxMenu;
    {
        append_menu_item(
            fileMenuGcodeviewer, wxID_ANY, _L("&Open G-code") + dots + "\tCtrl+O", _L("Open a G-code file"),
            [this](wxCommandEvent &) {
                if (m_plater != nullptr)
                    m_plater->load_gcode();
            },
...
#ifdef __APPLE__
        append_menu_item(
            fileMenuGcodeviewer, wxID_ANY, _L("Re&load from Disk") + dots + "\tCtrl+Shift+R", _L("Reload the plater from disk"),
            [this](wxCommandEvent &) { m_plater->reload_gcode_from_disk(); }, "", nullptr,
            [this]() { return !m_plater->get_last_loaded_gcode().empty(); }, this);
#else
        append_menu_item(
            fileMenuGcodeviewer, wxID_ANY, _L("Re&load from Disk") + sep + "F5", _L("Reload the plater from disk"),
            [this](wxCommandEvent &) { m_plater->reload_gcode_from_disk(); }, "", nullptr,
            [this]() { return !m_plater->get_last_loaded_gcode().empty(); }, this);
#endif // __APPLE__
        fileMenuGcodeviewer->AppendSeparator();
        /* append_menu_item(
             fileMenu, wxID_ANY, _L("Convert ASCII G-code to &binary") + dots, _L("Convert a G-code file from ASCII to binary format"),
             [this](wxCommandEvent &) {
                 if (m_plater != nullptr)
                     m_plater->convert_gcode_to_binary();
             },
             "convert_file", nullptr, []() { return true; }, this);
         append_menu_item(
             fileMenu, wxID_ANY, _L("Convert binary G-code to &ASCII") + dots, _L("Convert a G-code file from binary to ASCII format"),
             [this](wxCommandEvent &) {
                 if (m_plater != nullptr)
                     m_plater->convert_gcode_to_ascii();
             },
         ...
         fileMenu->AppendSeparator();*/
        append_menu_item(
            fileMenuGcodeviewer, wxID_ANY, _L("Export &Toolpaths as OBJ") + dots, _L("Export toolpaths as OBJ"),
            [this](wxCommandEvent &) {
                if (m_plater != nullptr)
                    m_plater->export_toolpaths_to_obj();
            },
            "export_plater", nullptr, [this]() { return can_export_toolpaths(); }, this);
        append_menu_item(
            fileMenuGcodeviewer, wxID_ANY, _L("Open &FlsunSlicer") + dots, _L("Open FlsunSlicer"),
            [](wxCommandEvent &) { start_new_slicer(); }, "", nullptr, []() { return true; }, this);
        fileMenuGcodeviewer->AppendSeparator();
        append_menu_item(fileMenuGcodeviewer, wxID_EXIT, _L("&Quit"), wxString::Format(_L("Quit %s"), SLIC3R_APP_NAME),
    ...
    if (m_plater != nullptr) {
        ...
        viewMenu->AppendSeparator();
        append_menu_check_item(
            viewMenu, wxID_ANY, _L("Show Legen&d") + sep + "L", _L("Show legend"),
            [this](wxCommandEvent &) { m_plater->show_legend(!m_plater->is_legend_shown()); }, this,
            [this]() { return m_plater->is_preview_shown(); }, [this]() { return m_plater->is_legend_shown(); }, this);
    }

    // helpmenu
    helpMenuGcodeviewer = generate_help_menu();

    m_menubar = new wxMenuBar();
    m_menubar->Append(fileMenuGcodeviewer, _L("&File"));
    // if (viewMenu != nullptr) m_menubar->Append(viewMenu, _L("&View"));
    // Add additional menus from C++
    // wxGetApp().add_config_menu(m_menubar);
    m_menubar->Append(helpMenuGcodeviewer, _L("&Help"));
    // SetMenuBar(m_menubar);
...
void MainFrame::reslice_now()
{
    ...
    {
     m_plater->reslice();
     dynamic_cast<Notebook *>(m_tabpanel)->slicemow();
...
void MainFrame::repair_stl()
{
    wxString input_file;
    {
        wxFileDialog dlg(this, _L("Select the STL file to repair:"), wxGetApp().app_config->get_last_dir(), "", file_wildcards(FT_STL),
    ...
    {
        wxFileDialog dlg(this, _L("Save OBJ file (less prone to coordinate errors than STL) as:"), get_dir_name(output_file),
                         wxString::FromUTF8(get_base_name(output_file, ".obj")), file_wildcards(FT_OBJ), wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
        if (dlg.ShowModal() != wxID_OK)
            return;
...
void MainFrame::export_config()
{
    ...
    wxString dlg_file = from_u8(!m_last_config.IsEmpty() ? get_base_name(m_last_config) : "config.ini");
    // Ask user for the file name for the config file.
    wxFileDialog dlg(this, _L("Save configuration as:"),
                     !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(),
                     dlg_file /*!m_last_config.IsEmpty() ? get_base_name(m_last_config) : "config.ini"*/, file_wildcards(FT_INI),
...
void MainFrame::load_config_file()
{
    ...
    wxFileDialog dlg(this, _L("Select configuration to load:"),
                     !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(), "config.ini",
                     "INI files (*.ini, *.gcode/*, *.bgcode*/)|*.ini;*.INI;*.gcode;*.g", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    wxString     file;
    if (dlg.ShowModal() == wxID_OK)
        file = dlg.GetPath();
...
void MainFrame::load_configbundle(wxString file /* = wxEmptyString, const bool reset_user_profile*/)
{
    ...
    if (file.IsEmpty()) {
        wxFileDialog dlg(this, _L("Select configuration to load:"),
                         !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(), "config.ini",
                         file_wildcards(FT_INI), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
        if (dlg.ShowModal() != wxID_OK)
            return;
        file = dlg.GetPath();
    }
    ...
    try {
        ...
        std::tie(config_substitutions,
                 presets_imported) = wxGetApp().preset_bundle->load_configbundle(file.ToUTF8().data(),
                                                                                 PresetBundle::LoadConfigBundleAttribute::SaveImported,
                                                                                 ForwardCompatibilitySubstitutionRule::Enable);
    } catch (const std::exception &ex) {
        show_error(this, ex.what());
        return;
...
void MainFrame::select_tab(Tab *tab)
{
    if (!tab)
        return;
    for (auto iter : m_dialog_vec) {
        if (iter->m_dialog == tab) {
            if (iter->m_dialog)
                iter->m_dialog->OnActivate();
            iter->Show();
        }
    }
...
void MainFrame::init_title_bar()
{
    m_title_bar = new wxPanel(this, wxID_ANY);
    m_title_bar->Bind(wxEVT_LEFT_DCLICK, &MainFrame::OnDoubleClick, this);

    wxBoxSizer *      m_title_bar_sizer = new wxBoxSizer(wxHORIZONTAL);
    const std::string path              = Slic3r::resources_dir() + "/icons/";
    wxStaticBitmap *m_logo = new wxStaticBitmap(m_title_bar, wxID_ANY, wxBitmap(Slic3r::GUI::from_u8(path + "logo.png"), wxBITMAP_TYPE_PNG));
    TopButton *     file_bto          = new TopButton(m_title_bar, BITTONBARID + 1, _("File(F)"));
    TopButton *     editl_bto         = new TopButton(m_title_bar, BITTONBARID + 2, _("Edit(E)"));
    TopButton *     Configuration_bto = new TopButton(m_title_bar, BITTONBARID + 3, _("Configuration(C)"));
    TopButton *     help_bto          = new TopButton(m_title_bar, BITTONBARID + 4, _("Help(H)"));
    wxFont          font              = this->normal_font();
    // font.SetWeight(FONTWEIGHT_BLACK);
    font.SetPointSize(12);
    wxColour background_colour = Slic3r::GUI::wxGetApp().m_color_top_bg;
    wxColour font_colour       = Slic3r::GUI::wxGetApp().m_color_top_font_def;

    file_bto->SetWindowStyle(wxBORDER_NONE);          // 移除边框样式
    editl_bto->SetWindowStyle(wxBORDER_NONE);         // 移除边框样式
    Configuration_bto->SetWindowStyle(wxBORDER_NONE); // 移除边框样式
    help_bto->SetWindowStyle(wxBORDER_NONE);          // 移除边框样式
    file_bto->SetBackgroundColour(background_colour);
    editl_bto->SetBackgroundColour(background_colour);
    Configuration_bto->SetBackgroundColour(background_colour);
    help_bto->SetBackgroundColour(background_colour);

    file_bto->SetForegroundColour(font_colour);          // 设置按钮的文本和颜色
    editl_bto->SetForegroundColour(font_colour);         // 设置按钮的文本和颜色
    Configuration_bto->SetForegroundColour(font_colour); // 设置按钮的文本和颜色
    help_bto->SetForegroundColour(font_colour);          // 设置按钮的文本和颜色

    file_bto->SetFont(font);
    editl_bto->SetFont(font);
    Configuration_bto->SetFont(font);
    help_bto->SetFont(font);
    ScalableButton *m_minimizeBtn            = new ScalableButton(m_title_bar, wxID_ANY, "min");
    auto            icon_name                = this->IsMaximized() ? "reduction" : "square";
    m_restoreBtn                             = new ScalableButton(m_title_bar, wxID_ANY, icon_name);
    ScalableButton *              m_closeBtn = new ScalableButton(m_title_bar, wxID_ANY, "close", "", wxDefaultSize, wxDefaultPosition,
                                                    wxBU_EXACTFIT | wxNO_BORDER, 24);
    std::vector<ScalableButton *> min_close_vec;
    min_close_vec.push_back(m_minimizeBtn);
    min_close_vec.push_back(m_restoreBtn);
    min_close_vec.push_back(m_closeBtn);

    m_title_bar_sizer->Add(m_logo, 0, wxALL, 1);
    m_title_bar_sizer->Add(file_bto, 0, wxALL, 1);
    m_title_bar_sizer->Add(editl_bto, 0, wxALL, 1);
    m_title_bar_sizer->Add(Configuration_bto, 0, wxALL, 1);
    m_title_bar_sizer->Add(help_bto, 0, wxALL, 1);
    m_title_bar_sizer->Add(0, 0, 1, wxEXPAND, 5);
    m_title_bar_sizer->Add(m_minimizeBtn, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, 0);
    m_title_bar_sizer->Add(m_restoreBtn, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, 0);
    m_title_bar_sizer->Add(m_closeBtn, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxLEFT, 0);

    m_title_bar->Bind(wxEVT_LEFT_DOWN, &MainFrame::OnLeftDown, this);
    m_title_bar->Bind(wxEVT_LEFT_UP, &MainFrame::OnLeftUp, this);
    m_title_bar->Bind(wxEVT_MOTION, &MainFrame::OnMotion, this);

    m_minimizeBtn->Bind(wxEVT_LEFT_DOWN, &MainFrame::OnMinimizeButtonClick, this);
    m_restoreBtn->Bind(wxEVT_LEFT_DOWN, &MainFrame::OnRestoreButtonClick, this);
    for (auto iterz : min_close_vec) {
        iterz->SetBackgroundColour(background_colour);
        iterz->SetMinSize(wxSize(40, 30));

        iterz->Bind(wxEVT_ENTER_WINDOW, [=](wxMouseEvent &event) { iterz->SetBackgroundColour(wxColour(0, 178, 104)); });

        iterz->Bind(wxEVT_LEAVE_WINDOW, [=](wxMouseEvent &event) { iterz->SetBackgroundColour(background_colour); });
    }

    m_closeBtn->Bind(wxEVT_LEFT_DOWN, &MainFrame::OnCloseButtonClick, this);

    m_title_bar->SetSizer(m_title_bar_sizer);
    m_title_bar_sizer->Fit(m_title_bar);
    m_title_bar->SetBackgroundColour(background_colour);
    m_main_sizer->Add(m_title_bar, 0, wxEXPAND, 5);

    file_bto->Bind(wxEVT_LEFT_DOWN, &MainFrame::onItemLeftButton, this);
    editl_bto->Bind(wxEVT_LEFT_DOWN, &MainFrame::onItemLeftButton, this);
    Configuration_bto->Bind(wxEVT_LEFT_DOWN, &MainFrame::onItemLeftButton, this);
    help_bto->Bind(wxEVT_LEFT_DOWN, &MainFrame::onItemLeftButton, this);
}

void MainFrame::init_title_gcodeviewer_bar()
{
    m_title_bar = new wxPanel(this, wxID_ANY);
    m_title_bar->Bind(wxEVT_LEFT_DCLICK, &MainFrame::OnDoubleClick, this);
    wxBoxSizer *      m_title_bar_sizer = new wxBoxSizer(wxHORIZONTAL);
...
void MainFrame::OnCharHook(wxKeyEvent &event)
{
    ...
    if (event.ControlDown()) {
        ...
        case 'J':
            ...
            break;
        case 'U':
            if (m_plater && can_export_gcode_sd())
                m_plater->export_gcode(true);
            break;
        case 'R': reslice_now(); break;
        case 'P': wxGetApp().open_preferences();
            break;
        case 'E': export_config(); break;
        case '2':
        case 326:
            // m_print_dialog->m_dialog->OnActivate();
            m_print_dialog->ShowModal();
            break;
        case '3':
        case 327:
            // m_printer_dialog->m_dialog->OnActivate();
            m_printer_dialog->ShowModal();
            break;
        case '4':
        case 328:
            // m_filament_dialog->m_dialog->OnActivate();
            m_filament_dialog->ShowModal();
            break;
        case '5':
        case 329: m_plater->select_view_3D("3D"); break;
        case '6':
        case 330: m_plater->select_view_3D("Preview"); break;
        default: event.Skip();
        }
    } else
        event.Skip();
...
{
    fileMenu->Enable(wxID_NEW, m_plater != nullptr && can_start_new_project() ? true : false);
    fileMenu->Enable(wxID_HIGHEST, m_recent_projects.GetCount() > 0 ? true : false);
    fileMenu->Enable(wxID_MORE, m_plater != nullptr && can_reslice() ? true : false);

    export_menu->Enable(MENUITEM_ID, can_export_gcode() ? true : false);
    export_menu->Enable(MENUITEM_ID + 1, can_export_gcode() ? true : false);
    export_menu->Enable(MENUITEM_ID + 2, can_export_gcode_sd() ? true : false);
    export_menu->Enable(MENUITEM_ID + 3, can_export_model() ? true : false);
    export_menu->Enable(MENUITEM_ID + 4, can_export_supports() ? true : false);
    export_menu->Enable(MENUITEM_ID + 5, can_export_toolpaths() ? true : false);

    editMenu->Enable(MENUITEM_ID + 6, can_select() ? true : false);
    editMenu->Enable(MENUITEM_ID + 7, can_deselect() ? true : false);
    editMenu->Enable(MENUITEM_ID + 8, can_delete() ? true : false);
    editMenu->Enable(MENUITEM_ID + 9, can_delete_all() ? true : false);
    editMenu->Enable(MENUITEM_ID + 10, m_plater->can_undo() ? true : false);
    editMenu->Enable(MENUITEM_ID + 11, m_plater->can_redo() ? true : false);
    editMenu->Enable(MENUITEM_ID + 12, m_plater->can_copy_to_clipboard() ? true : false);
    editMenu->Enable(MENUITEM_ID + 13, m_plater->can_paste_from_clipboard() ? true : false);
}
...
void MainFrame::OnPaint(wxPaintEvent &event)
{
    wxDisplay display;
    wxSize    screenSize = display.GetClientArea().GetSize();
    if (GetSize().x > screenSize.x || GetSize().y > screenSize.y) {
        this->SetSize(wxSize(screenSize.x, screenSize.y));
    }

    // 在窗口边缘绘制自定义的边框（可选）
    wxPaintDC dc(this);
    wxPen     pen(wxColour(0, 0, 0), 20, wxTRANSPARENT);
    dc.SetPen(pen);                    // 设置笔的颜色和样式
    dc.SetBrush(*wxTRANSPARENT_BRUSH); // 设置填充颜色（透明）
    dc.DrawRectangle(GetSize());       // 绘制整个窗口的边框
}
...
void MainFrame::OnMotion(wxMouseEvent &event)
{
    if (event.LeftIsDown() && event.Dragging() && HasCapture()) {
        if (m_offset.x != -1) {
            ...
            int       deltaX     = newPos.x - m_startPos.x;
            int       deltaY     = newPos.y - m_startPos.y;
            wxPoint   currentPos = GetPosition();
            wxDisplay display;
            wxSize    screenSize = display.GetClientArea().GetSize();
            if ((deltaX != 0 || deltaY != 0) && currentPos.y + deltaY < screenSize.y) {
                SetPosition(wxPoint(currentPos.x + deltaX, currentPos.y + deltaY));
                if (this->IsMaximized()) {
                    windowSwitching();
                }
            }
...
void MainFrame::OnLeftDown(wxMouseEvent &event)
{
    ...
    wxPoint mouse_pos = ClientToScreen(m_startPos);
...
void MainFrame::OnMouseLost(wxMouseCaptureLostEvent &event)
{
    m_offset = wxPoint(-1, -1);
    if (HasCapture())
        ReleaseMouse();
...
void MainFrame::OnDoubleClick(wxMouseEvent &event) { windowSwitching(); }

void MainFrame::OnMinimizeButtonClick(wxMouseEvent &event)
{
    Iconize(); // 最小化窗口
}

void MainFrame::OnRestoreButtonClick(wxMouseEvent &event)
{
    windowSwitching();
    m_restoreBtn->SetBackgroundColour(Slic3r::GUI::wxGetApp().m_color_top_bg);
}

void MainFrame::OnCloseButtonClick(wxMouseEvent &event)
{
    // m_plater->close_search();
    Close(); // 关闭窗口
    /*if (m_is_close) {
        exit(0);
    }*/
...
void MainFrame::onItemLeftButton(wxMouseEvent &event)
{
    auto id = event.GetId();
    // auto pos = event.GetPosition(); //此种方式 pos 是响应鼠标控件(Item)内的坐标系
    // PopupMenu 使用的是控件的局部坐标系
    auto globalPos = wxGetMousePosition();
    auto pos       = m_title_bar->ScreenToClient(globalPos);
    pos.y          = 30;
    switch (id) {
    case BITTONBARID + 1: wxGetApp().is_gcode_viewer() ? PopupMenu(fileMenuGcodeviewer, pos) : PopupMenu(fileMenu, pos); break;
    case BITTONBARID + 2: PopupMenu(editMenu, pos); break;
    case BITTONBARID + 3: PopupMenu(configurationMenu, pos); break;
    case BITTONBARID + 4: wxGetApp().is_gcode_viewer() ? PopupMenu(helpMenuGcodeviewer, pos) : PopupMenu(helpMenu, pos); break;
    default: break;
    }
}

void MainFrame::windowSwitching()
{
    wxDisplay display;
    wxSize    screenSize = display.GetClientArea().GetSize();
    if (this->IsMaximized()) {
        Restore(); // 恢复窗口到最小化前的状态
        m_restoreBtn->SetBitmap(*get_bmp_bundle("square", 20));
        // 窗口处于最大化状态
    } else {
        this->Maximize(true);
        m_restoreBtn->SetBitmap(*get_bmp_bundle("reduction", 20));
        // this->SetSize(wxSize(screenSize.x+10, screenSize.y+15));
        // 窗口不处于最大化状态
    }
}
...
SettingsDialog::SettingsDialog(MainFrame *mainframe)
    : DPIFrame(NULL,
               ...
               wxDEFAULT_FRAME_STYLE,
               "settings_dialog",
               mainframe->normal_font().GetPointSize())
    ,
...
#else
    SetIcon(wxIcon(var("FlsunSlicer_128px.png"), wxBITMAP_TYPE_PNG));
#endif // _WIN32
...
#if _WIN32
void MyPrintDialog::OnCharHook(wxKeyEvent &event)
{
    int keyCode = event.GetKeyCode();
    if (event.ControlDown() && keyCode == 'F') {
