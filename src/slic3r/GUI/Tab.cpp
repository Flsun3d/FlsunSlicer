...
Tab::Tab(wxDialog* parent, const wxString& title, Preset::Type type) :
...
//    m_mode_sizer = new ModeSizer(panel, int (0.5*em_unit(this)));
   ...
   // m_h_buttons_sizer->Add(m_btn_hide_incompatible_presets, 0, wxALIGN_CENTER_VERTICAL);
    ...
    m_treectrl->Bind(wxEVT_TREE_SEL_CHANGED, [this](wxTreeEvent&) {
        ...
        // see https://github.com/prusa3d/FlsunSlicer/issues/5720
...
//    wxWindowUpdateLocker noUpdates(this);
    ...
    if (m_presets_choice->IsShown())
        ...
        // (see https://github.com/prusa3d/FlsunSlicer/issues/10746)
...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (/*opt_key == "fill_density" || */opt_key == "pad_enable" || opt_key == "brim_type")
    {
    ...
    //if (opt_key == "brim_width" || opt_key == "brim_type")
    //{
    //    bool have_brim = m_config->opt_enum<BrimType>("brim_type") != btNoBrim;
    //    bool val       = (m_config->opt_float("brim_width")) && have_brim /*> 0.0 ? true : false*/;
    ...
    if (m_postpone_update_ui) {
        ...
        // see https://github.com/prusa3d/FlsunSlicer/issues/7146
        return;
    }
...
void Tab::build_preset_description_line(ConfigOptionsGroup* optgroup)
{
    ...
    auto detach_preset_btn = [this](wxWindow* parent) {
        ...
        btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent&)
        {
            ...
            RichMessageDialog dialog(parent, msg_text, _(L("Detach preset")), wxICON_WARNING | wxYES_NO | wxCANCEL);
...
void Tab::update_frequently_changed_parameters()
{
    ...
    og_freq_chng_params->set_value("support", support_combo_value_for_config(*m_config, is_fff));
        ...
        boost::any val1 = og_freq_chng_params->get_config_value(*m_config, "brim_type");
    og_freq_chng_params->set_value("brim_type", val1);
 
   // og_freq_chng_params->set_value("Brim", support_combo_value_for_config(*m_config, is_fff));
...
void TabPrint::build()
{
    ...
    page = add_options_page(L("Infill"), "infill");
        ...
        optgroup->append_single_option_line("top_level_single_layer_wall");
    ...
    page = add_options_page(L("Speed"), "time");
        ...
        optgroup->append_single_option_line("small_boundary_radius");
        optgroup->append_single_option_line("small_perimeter_speed");
        optgroup->append_single_option_line("external_perimeter_speed");
        optgroup->append_single_option_line("infill_speed");
        optgroup->append_single_option_line("switch_layer_infill_speed");
        ...
        optgroup->append_single_option_line("enable_inner_dynamic_overhang_speeds");
        ...
        optgroup->append_single_option_line("first_wall_velocity");
        optgroup->append_single_option_line("travel_speed_first_layer");
        ...
        optgroup = page->new_optgroup(L("Deceleration control (advanced)"));
        optgroup->append_single_option_line("accel_to_decel_enable");
        optgroup->append_single_option_line("external_perimeter_deceleration");
        optgroup->append_single_option_line("perimeter_deceleration");
        optgroup->append_single_option_line("top_solid_infill_deceleration");
        optgroup->append_single_option_line("solid_infill_deceleration");
        optgroup->append_single_option_line("infill_deceleration");
        optgroup->append_single_option_line("bridge_deceleration");
        optgroup->append_single_option_line("first_layer_deceleration");
        optgroup->append_single_option_line("first_layer_deceleration_over_raft");
        optgroup->append_single_option_line("travel_deceleration");
      
        optgroup = page->new_optgroup(L("Corner velocity"));
        optgroup->append_single_option_line("perimeters_corner_velocity");
        optgroup->append_single_option_line("external_perimeters_corner_velocity");
        optgroup->append_single_option_line("solid_infill_corner_velocity");
        optgroup->append_single_option_line("bridge_corner_velocity");
        optgroup->append_single_option_line("infill_corner_velocity");
        optgroup->append_single_option_line("top_solid_infill_corner_velocity");
    ...
    page = add_options_page(L("Advanced"), "wrench");
        ...
        optgroup->append_single_option_line("adaptive_linewidth");
...
void TabPrint::update()
{
    ...
    // see https://github.com/prusa3d/FlsunSlicer/issues/6814
...
std::vector<std::pair<std::string, std::vector<std::string>>> filament_overrides_option_keys{
    {L("Travel lift"),
     {"filament_estimated_empty_distance",
        "filament_retract_lift",
        "filament_travel_ramping_lift", "filament_travel_spiral_lift",
        "filament_travel_max_lift",
        /*"filament_travel_slope",*///隐藏抬升角度设置
...
void TabFilament::update_filament_overrides_page()
{
    ...
    const bool uses_spiral_lift  = (m_config->option("filament_travel_spiral_lift")->is_nil() ||
                                    m_config->opt_bool("filament_travel_spiral_lift", extruder_idx));
    ...
    for (const auto &[title, keys] : filament_overrides_option_keys) {
        ...
        for (const std::string& opt_key : keys) {
            ...
            if (
                title == "Travel lift"
                && uses_ramping_lift
                && (opt_key == "filament_retract_lift" || opt_key == "filament_travel_spiral_lift")
            ...
            if (title == "Travel lift" 
                && uses_spiral_lift 
                && opt_key == "filament_travel_ramping_lift"
                &&!m_config->option("filament_travel_spiral_lift")->is_nil() 
                &&m_config->opt_bool("filament_travel_spiral_lift", extruder_idx)) {
                is_checked = false;
            }
            if (title == "Travel lift"
                ...
                && opt_key != "filament_travel_spiral_lift"
                && opt_key != "filament_estimated_empty_distance"
                && opt_key != "filament_retract_lift_above"
                && opt_key != "filament_retract_lift_below"
            ) {
                is_checked = false;
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament"), "spool");
        ...
        optgroup = page->new_optgroup(L("Flow"));
        optgroup->append_single_option_line("inner_wall_flow");
        optgroup->append_single_option_line("external_wall_flow");
        optgroup->append_single_option_line("fill_flow");
        optgroup->append_single_option_line("top_flow");
        optgroup->append_single_option_line("bottom_solid_fill_flow");
        optgroup->append_single_option_line("first_flow");
...
void TabPrinter::build_print_host_upload_group(Page* page)
{
    ...
    wxString description_line_text = _L(""
        ...
        "into FlsunSlicer/physical_printer directory.");
...
void TabPrinter::build_extruder_pages(size_t n_before_extruders)
{
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        optgroup = page->new_optgroup(L("lift"));
        optgroup->append_single_option_line("estimated_empty_distance", "", extruder_idx);
        optgroup->append_single_option_line("retract_lift", "", extruder_idx);
        optgroup->append_single_option_line("travel_ramping_lift", "", extruder_idx);
        optgroup->append_single_option_line("travel_spiral_lift", "", extruder_idx);
        optgroup->append_single_option_line("travel_max_lift", "", extruder_idx);
        //optgroup->append_single_option_line("travel_slope", "", extruder_idx);//隐藏抬升角度设置
...
void TabPrinter::toggle_options()
{
        ...
        val > 0 && (size_t)val <= m_extruders_count)
    {
        ...
        const bool spiral_lift         = m_config->opt_bool("travel_spiral_lift", i);
        ...
        toggle_option("travel_spiral_lift", !ramping_lift, i);
        toggle_option("travel_ramping_lift", !spiral_lift, i);
...
bool Tab::select_preset(std::string preset_name, bool delete_current /*=false*/, const std::string& last_selected_ph_printer_name/* =""*/)
{
    ...
    } else if (filament_tab) {
        m_dependent_tabs.emplace_back(Preset::Type::TYPE_PRINT);
    }
    ...
    if (canceled) {
        ...
        if (! is_selected && printer_tab)
        {
            ...
            if (printer_technology == ptFFF && m_dependent_tabs.front() != Preset::Type::TYPE_PRINT)
                m_dependent_tabs = {Preset::Type::TYPE_FILAMENT, Preset::Type::TYPE_PRINT};
...
{
    // There is a bug related to Ubuntu overlay scrollbars, see https://github.com/prusa3d/FlsunSlicer/issues/898 and https://github.com/prusa3d/FlsunSlicer/issues/952.
...
#ifdef _WIN32
    RichMessageDialog dlg(parent(), msg, title, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
    dlg.SetYesNoLabels(_L("Yes"), _L("No"));

#else
    MessageDialog dlg(parent(), msg, title, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
#endif
    if (current_preset.is_default ||
        //wxID_YES != wxMessageDialog(parent(), msg, title, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION).ShowModal())
        wxID_YES != dlg.ShowModal())
        return;
...
wxSizer* TabPrinter::create_bed_shape_widget(wxWindow* parent)
{
    ...
    btn->SetFont(wxGetApp().normal_font());
    btn->SetSize(btn->GetBestSize());
    btn->SetForegroundColour(wxColor(0, 0, 0));
...
void Tab::fill_icon_descriptions()
{
    ...
    m_icon_descriptions.emplace_back(&m_bmp_value_revert, L("BACK ARROW"),
        ...
        L("indicates that the settings were changed and are not equal to Is the default preset for the current option group.\n"
...
void Tab::set_tooltips_text()
{
    ...
    m_ttg_value_revert =	_(L("BACK ARROW icon indicates that the settings were changed and are not equal to "
                                "Is the default preset for the current option group.\n"
                                "Click to reset all Settings for the current option group to the default  preset."));
    ...
    m_tt_value_revert =		_(L("The Back arrow icon indicates that the value has changed and is not equal to the default default value.\n"
                                "Click to reset the current value to the default default value."));
}
...
std::vector<std::pair<std::string, std::vector<std::string>>>
    material_overrides_option_keys{{"Support head", {"support_head_front_diameter", "support_head_penetration", "support_head_width"}},
                                   {"Support pillar",
                                    {
                                        "support_pillar_diameter",
                                    }},
                                   {"Automatic generation", {"support_points_density_relative"}},
                                   {"Corrections", {"relative_correction", "elefant_foot_compensation"}}};
