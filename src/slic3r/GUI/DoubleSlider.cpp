    ...
    m_extra_style(style == wxSL_VERTICAL ? wxSL_AUTOTICKS | wxSL_VALUE_LABEL : 0)
{
    ...
    m_selection = ssUndef;
    m_ticks.set_pause_print_msg("Printing paused");
    ...
    DARK_ORANGE_PEN  = wxPen(wxColour(15, 169, 93));
    ORANGE_PEN       = wxPen(wxColour(15, 169, 93));
    LIGHT_ORANGE_PEN = wxPen(wxColour(15, 169, 93));
...
wxString Control::get_tooltip(int tick/*=-1*/)
{
    ...
    if (tick_code_it == m_ticks.ticks.end() && m_focus == fiActionIcon)    // tick doesn't exist
    {
        ...
        tooltip += (m_mode == SingleExtruder ? _L("SingleExtruder") : m_mode == MultiAsSingle ? _L("MultiAsSingle") : _L("MultiExtruder"));
