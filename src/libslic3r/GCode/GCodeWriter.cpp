...
std::string GCodeWriter::set_acceleration_internal(Acceleration type, unsigned int acceleration, unsigned int deceleration)
{
    if (type == Acceleration::Print && m_max_acceleration > 0 && acceleration > m_max_acceleration)
        acceleration = m_max_acceleration;
    if (type == Acceleration::Travel && m_max_travel_acceleration > 0 && acceleration > m_max_travel_acceleration)
    ...
    gcode << "\n";
    gcode << "SET_VELOCITY_LIMIT ACCEL_TO_DECEL=" << deceleration;
    gcode << "\n";
    return gcode.str();
}
...
{
    double speed = new_layer == 0 ? this->config.travel_speed_first_layer.value : this->config.travel_speed.value;
...
std::string GCodeWriter::travel_to_xy(const double &travelsped, const Vec2d &point, const std::string &comment) 
{ 
    m_pos.x() = point.x();
    m_pos.y() = point.y();
    ...
    GCodeG1Formatter w;
    w.emit_xy(point);
    w.emit_f(travelsped * 60.0);
    w.emit_comment(this->config.gcode_comments, comment);
    return w.string();
}

std::string GCodeWriter::_travel_to_z(double travel, double z, const std::string &comment) 
{
    m_pos.z() = z;
    ...
    if (speed == 0.) 
    {
       speed = new_layer == 0 ? this->config.travel_speed_first_layer.value : this->config.travel_speed.value;
    }

    GCodeG1Formatter w;
    w.emit_z(z);
    w.emit_f(travel * 60.0);
    w.emit_comment(this->config.gcode_comments, comment);
    return w.string();
}
...
std::string GCodeWriter::travel_to_xyz(const Vec3d &from, const Vec3d &to, const std::string_view comment, int new_layer)
{
    if (std::abs(to.x() - m_pos.x()) < EPSILON && std::abs(to.y() - m_pos.y()) < EPSILON) {
        
       
        Vec3d source     = {m_pos.x(), m_pos.y(), m_pos.z()};
        Vec3d target     = {to(0), to(1), to(2)};
        Vec3d delta      = target - source;
        Vec2d delta_no_z = {delta(0), delta(1)};
        if (config.travel_spiral_lift.get_at(0)  && delta(2) > 0) {

            return this->_spiral_travel_to_z(source, to, to.z(), "spiral");
...
std::string GCodeWriter::_spiral_travel_to_z(const Vec3d &from, const Vec3d &to, double z, const std::string &comment)
{
    Vec3d       source     = {from(0), from(1), from(2)};
    Vec3d       target     = {to(0), to(1), to(2)};
    Vec3d       delta      = target - source;
    Vec2d       delta_no_z = {delta(0), delta(1)};
    m_pos             = to;
    ...
    if (delta(2) > 0) {
        ...
        Vec2d  ij_offset = radius * delta_no_z.normalized()/2;
        ...
        if (ij_offset.x() <-5 || ij_offset.y()<-5) {
            ...
            w.emit_z(z);
            w.emit_f(60 * 60.0);
            w.emit_comment(this->config.gcode_comments,";lift");
            return w.string();

        } else if (ij_offset.x() == 0 ||ij_offset.y() == 0) {
            ...
            w.emit_ij(ij_offset);
            w.emit_f(60* 60.0);
            w.emit_comment(this->config.gcode_comments,";spiral");
            std::string gs = w.string();
            boost::replace_all(gs, "G3", "G2");
            if ((int) gs.find("I0") > 0) {
                GCodeG1Formatter w;
                w.emit_z(z);
                w.emit_f(60* 60.0);
...
std::string GCodeWriter::spiral_travel_to_z(const Vec3d& from, const Vec3d& to,double z,  const std::string &comment)
{
    ...
    if (delta(2) > 0 && delta_no_z.norm() != 0.0f) {
       ...
       if (ij_offset.x() == 0 || ij_offset.y() == 0) {
            ...
            w.emit_comment(this->config.gcode_comments, comment);
            return w.string();
       } else if (ij_offset.x() <-5 || ij_offset.y()<-5) {
...
std::string GCodeWriter::get_travel_to_xyz_gcode(const Vec3d &from, const Vec3d &to, const std::string_view comment) const {
    ...
    const double time_xy = distance_xy / speed;
    const double factor = time_z > 0 ? time_xy / time_z : 1;
    if (factor < 1) {
        w.emit_f((speed * factor + (1 - factor) * speed_z) * 60.0);
    } else {
        w.emit_f(speed * 60.0);
    }
