...
{
    auto time1 = trapezoid.acceleration_time(feedrate_profile.entry, acceleration);
    auto time2 = trapezoid.cruise_time();
    auto time3 = trapezoid.deceleration_time(distance, acceleration);
...
void GCodeProcessor::TimeMachine::calculate_time(size_t keep_last_n_blocks, float additional_time)
{
    ...
    for (size_t i = 0; i < n_blocks_process; ++i) {
        ...
        time_vec.push_back(block.trapezoid.cruise_feedrate);
        if (i == 0)
        ...
        layer_num = block.layer_id - 1;
    }
    /*if (layer_num == 0) 
    {
        layers_time[layer_num] += 60;
    }*/
...
void GCodeProcessor::post_process()
{
        ...
        void write(FilePtr& out, float backtrace_time, GCodeProcessorResult& result, const std::string& out_path) {
            ...
            if (m_binarizer.is_enabled()) {
                if (m_binarizer.append_gcode(out_string) != bgcode::core::EResult::Success)
                    throw Slic3r::RuntimeError(_u8L("Error while sending gcode to the binarizer."));
            }
            else {
            ...
            if (!out_string.empty()) {
                if (!m_binarizer.is_enabled()) {
                    ...
                    if (ferror(out.f)) {
                        ...
                        throw Slic3r::RuntimeError(_u8L("GCode processor post process export failed.\nIs the disk full?"));
                    }
                }
            }
        }
    ...
    if (m_binarizer.is_enabled()) {
        if (m_binarizer.finalize() != bgcode::core::EResult::Success)
            throw Slic3r::RuntimeError(_u8L("Error while finalizing the gcode binarizer."));
    }
