...
VoronoiUtils::get_source_segment(const VoronoiDiagram::cell_type &cell, const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    if (!cell.contains_segment())
        throw Slic3r::InvalidArgument(_u8L("Voronoi cell doesn't contain a source segment!"));
...
VoronoiUtils::get_source_point(const VoronoiDiagram::cell_type &cell, const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    ...
    if (!cell.contains_point())
        throw Slic3r::InvalidArgument(_u8L("Voronoi cell doesn't contain a source point!"));
    ...
    if (cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) {
        ...
        throw Slic3r::RuntimeError(_u8L("Voronoi diagram is always constructed using segments, so cell.source_category() shouldn't be SOURCE_CATEGORY_SINGLE_POINT!"));
    } else {
        throw Slic3r::InvalidArgument(_u8L("Function get_source_point() should only be called on point cells!"));
    }
}
...
VoronoiUtils::get_source_point_index(const VD::cell_type &cell, const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    ...
    if (cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) {
        ...
        throw Slic3r::InvalidArgument(_u8L("Function get_source_point_index() should only be called on point cells!"));
    }
}
