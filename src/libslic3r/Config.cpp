...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by FlsunSlicer.
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before FlsunSlicer 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by FlsunSlicer ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or FlsunSlicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char prusaslicer_gcode_header[] = "; generated by FlsunSlicer ";
        std::string header;
        ...
        if (! header_found)
            throw Slic3r::RuntimeError("Not a FlsunSlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters)
    {
        // FlsunSlicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with 
        // ; flsunslicer_config = begin
        // ...
        // ; flsunslicer_config = end
        ...
        while (reader.getline(line))
            if (line == "; flsunslicer_config = end") {
                end_found = true;
                break;
            }
        if (! end_found) 
            throw Slic3r::RuntimeError(format("Configuration block closing tag \"; flsunslicer_config = end\" not found when reading %1%", filename));
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; flsunslicer_config = begin") {
                begin_found = true;
                break;
            }
        ...
        if (! begin_found) 
            throw Slic3r::RuntimeError(format(L("Configuration block opening tag \"; flsunslicer_config = begin\" not found when reading %1%"), filename));
    }
    else
    {
        // Slic3r or FlsunSlicer older than 2.4.0-alpha0 do not emit any delimiter.
