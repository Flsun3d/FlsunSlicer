    ...
    std::vector<ExPolygonRange>                            &fill_expolygons_ranges)
{
    ...
    if (this->layer()->object()->config().perimeter_generator.value == PerimeterGeneratorType::Arachne && !spiral_vase)
        g.process_arachne(perimeter_and_gapfill_ranges, fill_expolygons_ranges);
    else
        g.process_classic(perimeter_and_gapfill_ranges, fill_expolygons_ranges);
    ...
    //params.upper_slices            = upper_slices;
