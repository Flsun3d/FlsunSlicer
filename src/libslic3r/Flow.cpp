...
FlowErrorNegativeSpacing::FlowErrorNegativeSpacing() : 
	FlowError(L("Flow::spacing() produced negative spacing. Did you set some extrusion width too small?")) {}

FlowErrorNegativeFlow::FlowErrorNegativeFlow() :
    FlowError(L("Flow::mm3_per_mm() produced negative flow. Did you set some extrusion width too small?")) {}
...
static inline FlowRole opt_key_to_flow_role(const std::string &opt_key)
{
 	if (opt_key == "perimeter_extrusion_width" || 
    	...
    	throw Slic3r::RuntimeError(_u8L("opt_key_to_flow_role: invalid argument"));
};
...
{
    if (height <= 0)
        throw Slic3r::InvalidArgument(_u8L("Invalid flow height supplied to new_from_config_width()"));
...
Flow Flow::with_spacing(float new_spacing) const
{
    ...
    if (m_bridge) {
        ...
        if (out.m_width < out.m_height)
            throw Slic3r::InvalidArgument(_u8L("Invalid spacing supplied to Flow::with_spacing()"));
    }
