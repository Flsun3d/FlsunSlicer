...
static ExtrusionEntityCollection traverse_loops_classic(const PerimeterGenerator &params, const Polygons &lower_slices_polygons_cache, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls)
{
...
static ExtrusionEntityCollection traverse_extrusions(const PerimeterGenerator &params, const Polygons &lower_slices_polygons_cache, std::vector<PerimeterGeneratorArachneExtrusion> &pg_extrusions)
{
...
void PerimeterGenerator::process_arachne(std::vector<std::pair<ExtrusionRange, ExtrusionRange>> &perimeter_and_gapfill_ranges,
    ...
    //ExPolygons                 &out_fill_expolygons)
{
    ...
    if (this->config.overhangs && lower_slices != nullptr && lower_slices_polygons_cache.empty()) {
    ...
    for (const Surface &surface : this->slices->surfaces) {
        ...
        auto perimeters_begin      = uint32_t(out_loops.size());
        auto gap_fills_begin       = uint32_t(out_gap_fill.size());
        auto fill_expolygons_begin = uint32_t(out_fill_expolygons.size());
        ...
        if (loop_number > 0 && this->config.top_level_single_layer_wall.value && this->upper_slices == nullptr) {
        ...
        ExPolygons last        = offset_ex(surface.expolygon.simplify_p(this->scaled_resolution),
        ...
        if (!surface.is_bridge() && !is_topmost_layer) {
            ...
            if (top_fills_tmp.empty()) {
                ...
                if (loop_number > 0 && config.top_level_single_layer_wall.value) {
                   ...
                   Arachne::WallToolPaths wallToolPaths(last_p, ext_perimeter_spacing, perimeter_spacing, coord_t(1), 0,
                                                         this->layer_height, this->object_config, this->print_config);
        ...
        Arachne::WallToolPaths wallToolPaths(last_p, ext_perimeter_spacing, perimeter_spacing, coord_t(loop_number + 1), 0,
                                             this->layer_height, this->object_config, this->print_config);
        ...
        Arachne::WallToolPaths::ExtrusionLineSet extrusions_constrains =
            Arachne::WallToolPaths::getRegionOrder(all_extrusions, this->config.external_perimeters_first);
        ...
        if (this->layer_id > 0 && this->config.fuzzy_skin != FuzzySkinType::None) {
            ...
            if (this->config.fuzzy_skin == FuzzySkinType::External) {
                ...
                for (const auto &cl_extrusion : closed_loop_extrusions) {
                    ...
                    for (auto junction_it = cl_extrusion->extrusion->junctions.begin();
                         junction_it != std::prev(cl_extrusion->extrusion->junctions.end()); ++junction_it)
                ...
                for (const ClipperLib_Z::PolyNode *child_node : loops_polytree.Childs) {
                    ...
                    bool    has_same_idx = std::all_of(child_node->Contour.begin(), child_node->Contour.end(),
                                                    [&polygon_idx](const ClipperLib_Z::IntPoint &point) -> bool {
                                                        return polygon_idx == point.z();
                                                    });
                    if (has_same_idx)
        ...
        if (ExtrusionEntityCollection extrusion_coll = traverse_extrusions(*this, lower_slices_polygons_cache, ordered_extrusions);
        ...
        for (ExPolygon &ex : infill_contour)
            ex.simplify_p(this->scaled_resolution, &pp);
        ...
        if (lower_slices != nullptr && this->config.overhangs && this->config.extra_perimeters_on_overhangs &&
            ...
            auto [extra_perimeters, filled_area] = generate_extra_perimeters_over_overhangs(infill_areas, lower_slices_polygons_cache,
                                                                                            loop_number + 1, this->overhang_flow,
                                                                                            this->scaled_resolution, this->object_config,
                                                                                            this->print_config);
        ...
        perimeter_and_gapfill_ranges.emplace_back(ExtrusionRange{perimeters_begin, uint32_t(out_loops.size())},
                                                  ExtrusionRange{gap_fills_begin, uint32_t(out_gap_fill.size())});
        fill_expolygons_ranges.emplace_back(ExtrusionRange{fill_expolygons_begin, uint32_t(out_fill_expolygons.size())});
    }
   
}

void PerimeterGenerator::process_classic(std::vector<std::pair<ExtrusionRange, ExtrusionRange>> &perimeter_and_gapfill_ranges,
    ...
    //ExPolygons                 &out_fill_expolygons)
{
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        // Set the topmost layer to be one wall
        if (loop_number > 0 && this->config.top_level_single_layer_wall.value && upper_slices == nullptr) {
            loop_number = 0;
        }

        ExPolygons last = union_ex(surface.expolygon.simplify_p(this->scaled_resolution));
        ExPolygons gaps;
        ...
        if (loop_number >= 0) {
            ...
            for (int i = 0;; ++i) { // outer loop is 0
                ...
                if (i == 0) {
                    ...
                    offsets = this->config.thin_walls ? offset2_ex(last, -float(ext_perimeter_width / 2. + ext_min_spacing / 2. - 1),
                    ...
                    if (this->spiral_vase && offsets.size() > 1) {
                ...
                if (i == 0 && i != loop_number && this->config.top_level_single_layer_wall.value && !surface.is_bridge() &&
                    this->upper_slices != NULL) {
                    split_top_surfaces(last, top_fills, last, fill_clip);
                }
            ...
            ExtrusionEntityCollection entities = traverse_loops_classic(*this, lower_slices_polygons_cache, contours.front(), thin_walls);
            // if brim will be printed, reverse the order of perimeters so that
            // we continue inwards after having finished the brim
            // TODO: add test for perimeter order
            if (this->config.external_perimeters_first || (this->layer_id == 0 && this->object_config.brim_width.value > 0))
                entities.reverse();
        ...
        if (!gaps.empty()) {
            ...
            if (!polylines.empty()) {
                ...
                variable_width_classic(polylines, ExtrusionRole::GapFill, this->solid_infill_flow, gap_fill.entities);
        ...
        if (inset > 0) {
            infill_peri_overlap = coord_t(
                scale_(this->config.infill_overlap.get_abs_value(unscale<double>(inset + solid_infill_spacing / 2))));
                                            ...
                                            ExPolygons &      fill_clip)
{
    ...
    coord_t offset_top_surface = scale_(
        1.5 * (this->config.perimeters.value == 0 ?
                   0. :
                   unscaled(double(ext_perimeter_width + perimeter_spacing * int(int(this->config.perimeters.value) - int(1))))));
    // if possible, try to not push the extra perimeters inside the sparse infill
    if (offset_top_surface > 0.9 * (this->config.perimeters.value <= 1 ? 0. : (perimeter_spacing * (this->config.perimeters.value - 1))))
        offset_top_surface -= coord_t(0.9 * (this->config.perimeters.value <= 1 ? 0. : (perimeter_spacing * (this->config.perimeters.value - 1))));
    else
    ...
    double min_width_top_surface = std::max(double(ext_perimeter_spacing / 2 + 10),
                                            scale_(unscale_(perimeter_width)));
    ...
    auto       nozzle_diameter = this->print_config.nozzle_diameter.get_at(0);
    // Check whether surface be bridge or not
    if (this->lower_slices != NULL) {

        ExPolygons cslice = *this->lower_slices;

        // BBS: get the Polygons below the polygon this layer
       Polygons lower_polygons_series_clipped;
       //Polygons lower_polygons_series_clipped = ClipperUtils::clip_clipper_polygons_with_subject_bbox(*params.lower_slices, last_box);
       for (size_t i = 0; i < (*this->lower_slices).size(); i++) {

           Polygons lower_polygons_series_clippedd = ClipperUtils::clip_clipper_polygons_with_subject_bbox((*this->lower_slices)[i],
                                                                                                           last_box);
       
           lower_polygons_series_clipped.insert(lower_polygons_series_clipped.end(), lower_polygons_series_clippedd.begin(),
                                                lower_polygons_series_clippedd.end());
       }
