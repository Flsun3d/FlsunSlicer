...
static const std::string VERSION_CHECK_URL = "https://flsun-hk-slicer.oss-cn-hongkong.aliyuncs.com/version";
// Url to index archive zip that contains latest indicies
static const std::string INDEX_ARCHIVE_URL = "http://192.168.100.190:8090/SourceType?sourceType=FlsunSlicer&fileName=vendor_indices.zip";

// Url to folder with vendor profile files. Used when downloading new profiles that are not in resources folder.
static const std::string PROFILE_FOLDER_URL = "https://192.168.100.190:8090/PrusaSlicer-settings-master/live/";
...
std::string AppConfig::load(const std::string &path)
{
    ...
    if (ini_ver) {
        ...
        boost::filesystem::path vendor_path(boost::filesystem::path(Slic3r::data_dir()) / "vendor");
        auto                    bundle_path = vendor_path / ("FLSun.ini");
        const std::string       path        = resources_dir() + "/profiles/";

        boost::filesystem::path vendor_path_resources(path + "FLSun.ini");
        if (!boost::filesystem::exists(bundle_path)) {
            m_legacy_datadir = true;
        } else {
            VendorProfile vp           = VendorProfile::from_ini(bundle_path, false);
            VendorProfile vp_resources = VendorProfile::from_ini(vendor_path_resources, false);

            // m_legacy_datadir = ini_ver < Semver(1, 0, 0);
            m_legacy_datadir = vp_resources.config_version > m_orig_version;
        }
    }
