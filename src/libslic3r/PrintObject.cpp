            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, m_layers.size()), [this, &curled_lines, &unscaled_polygons_lines,
                ...
                for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                    ...
                    for (LayerRegion *layer_region : l->regions()) {
                        ...
                        prev_layer_id = l->lower_layer ? l->lower_layer->id() : size_t(-1);
                        //if (layer_region->region().config().enable_inner_dynamic_overhang_speeds.value) {

                        //    prev_layer_id = (l->lower_layer && l->lower_layer->lower_layer) ? l->lower_layer->lower_layer->id() :
                        //                                                                             size_t(-1);
                        //} else {
                        //    prev_layer_id = l->lower_layer ? l->lower_layer->id() : size_t(-1);
                        //}
    ...
    const ConfigOptionResolver &old_config, const ConfigOptionResolver &new_config, const std::vector<t_config_option_key> &opt_keys)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys) {
        if (   opt_key == "brim_width"
            ...
            || opt_key == "adaptive_linewidth" || opt_key == "enable_inner_dynamic_overhang_speeds" 
            ...
            || opt_key == "small_perimeter_speed" || opt_key == "small_boundary_radius"
            ...
            || opt_key == "switch_layer_infill_speed"
...
void PrintObject::discover_horizontal_shells()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++region_id) {
        for (size_t i = 0; i < m_layers.size(); ++i) {
            ...
            if (m_layers[i]->lower_layer){         
                for (Surface &surfacet : m_layers[i]->lower_layer->regions()[region_id]->m_fill_surfaces.surfaces) {                 
                    if (surfacet.surface_type == stTop) {        
                        for (Surface &surfacef : layerm->m_fill_surfaces.surfaces) {                    
                            if (surfacef.surface_type == stInternal) {
                                Polygons pxv = intersection(surfacet.expolygon, surfacef.expolygon);                               
                                if (!pxv.empty()) {
                                    if (surfacef.area() < surfacet.expolygon.area()) {                                     
                                        surfacef.surface_type = stInternalSolid;  
                                    }
                                }
                            }
                        }                          
