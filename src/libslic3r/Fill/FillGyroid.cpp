...
static inline double fco(double x, double z_sin, double z_cos, bool vertical, bool flip)
{
    if (vertical) {
        double phase_offset = (z_cos < 0 ? M_PI : 0) + M_PI;
        double a            = sin(x + phase_offset);
...
static inline Polyline make_waveco(
    const std::vector<Vec2d>& one_period, double width, double height, double offset, double scaleFactor,
    double z_cos, double z_sin, bool vertical, bool flip)
{
    std::vector<Vec2d> points = one_period;
    ...
    if (width != period) // do not extend if already truncated
    {
        ...
        points.emplace_back(Vec2d(width, fco(width, z_sin, z_cos, vertical, flip)));
    }
...
static std::vector<Vec2d> make_one_periodco(
    double width, double scaleFactor, double z_cos, double z_sin, bool vertical, bool flip, double tolerance)
{
    ...
    for (double x = 0.; x < limit - EPSILON; x += dx) {
        points.emplace_back(Vec2d(x, fco(x, z_sin, z_cos, vertical, flip)));
    }
    points.emplace_back(Vec2d(limit, fco(limit, z_sin, z_cos, vertical, flip)));
    ...
    for (;;) {
        ...
        for (unsigned int i = 1; i < size; ++i) {
            ...
            auto & rp = points[i];     // right point
            double x  = lp(0) + (rp(0) - lp(0)) / 2;
            double y  = fco(x, z_sin, z_cos, vertical, flip);
            Vec2d  ip = {x, y};
            if (std::abs(cross2(Vec2d(ip - lp), Vec2d(ip - rp))) > sqr(tolerance)) {
                points.emplace_back(std::move(ip));
            }
...
static Polylines make_gyroid_wavesco(double gridZ, double density_adjusted, double line_spacing, double width, double height)
{
    const double scaleFactor = scale_(line_spacing) / density_adjusted;

    // tolerance in scaled units. clamp the maximum tolerance as there's
    // no processing-speed benefit to do so beyond a certain point
    const double tolerance = std::min(line_spacing / 2, FillCorrugated::PatternTolerance1) / unscale<double>(scaleFactor);
    ...
    std::vector<Vec2d> one_period_odd =
        make_one_periodco(width, scaleFactor, z_cos, z_sin, vertical, flip,
                        tolerance);             // creates one period of the waves, so it doesn't have to be recalculated all the time
    flip                               = !flip; // even polylines are a bit shifted
    std::vector<Vec2d> one_period_even = make_one_periodco(width, scaleFactor, z_cos, z_sin, vertical, flip, tolerance);
    Polylines          result;

    for (double y0 = lower_bound; y0 < upper_bound + EPSILON; y0 += M_PI) {
        // creates odd polylines
        result.emplace_back(make_waveco(one_period_odd, width, height, y0, scaleFactor, z_cos, z_sin, vertical, flip));
        // creates even polylines
        y0 += M_PI;
        if (y0 < upper_bound + EPSILON) {
            result.emplace_back(make_waveco(one_period_even, width, height, y0, scaleFactor, z_cos, z_sin, vertical, flip));
        }
    }
...
void FillCorrugated::_fill_surface_single(const FillParams &             params,
                                      unsigned int                   thickness_layers,
                                      const std::pair<float, Point> &direction,
                                      ExPolygon                      expolygon,
                                      Polylines &                    polylines_out)
{
    auto infill_angle = float(this->angle + (CorrectionAngle1 * 2 * M_PI) / 360.);
    if (std::abs(infill_angle) >= EPSILON)
        expolygon.rotate(-infill_angle);

    BoundingBox bb = expolygon.contour.bounding_box();
    // Density adjusted to have a good %of weight.
    double density_adjusted = std::max(0., params.density * DensityAdjust1);
    // Distance between the gyroid waves in scaled coordinates.
    coord_t distance = coord_t(scale_(this->spacing) / density_adjusted);
    ...
    // generate pattern
    Polylines polylines = make_gyroid_wavesco(scale_(this->z), density_adjusted, this->spacing, ceil(bb.size()(0) / distance) + 1.,
                                            ceil(bb.size()(1) / distance) + 1.);
