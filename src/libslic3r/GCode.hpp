...
private:
    ...
    // get perimeter list from layer region.
    ExtrusionPaths get_perimeters_from_layer_region(const LayerRegion *_region);

    // get target perimeters from layer.
    ExtrusionPaths get_perimeters_from_layer(const ObjectLayerToPrint &_layer_to_print, const ExtrusionRole &_tgt_role);

    // This function will segment infill and modify the width of infill segment.
    void process_layer_infill_width(const ObjectLayerToPrint &layer_to_print);
    
    // process overhang speed of internal perimeters.
    void process_internal_perimeter_overhang_speed(const ObjectLayerToPrint &layer_to_print);
