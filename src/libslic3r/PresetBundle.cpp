...
const char *PresetBundle::PRUSA_BUNDLE = "FLSun";
...
PresetBundle::PresetBundle() :
    ...
    printers(Preset::TYPE_PRINTER, Preset::printer_options(), static_cast<const PrintRegionConfig&>(FullPrintConfig::defaults()), "- FLSun S1 -"),
...
void PresetBundle::update_multi_material_filament_presets()
{
    ...
    //auto bnm = this->printers.get_edited_preset().config.opt_string("default_filament_profile");
    if (this->extruders_filaments.size() > num_extruders)
        ...
        for (size_t id = extruders_filaments.size(); id < num_extruders; id++)
            extruders_filaments.emplace_back(ExtruderFilaments(&filaments, id,
                                                               id == 0 ? /*filaments.first_visible().name*/ "Generic PLA_S1 @FLSUN" :
