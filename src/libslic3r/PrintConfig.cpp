///|/ Copyright (c) Prusa Research 2016 - 2023 Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena, Lukáš Hejl @hejllukas, Tomáš Mészáros
///@tamasmeszaros, Oleksandra Iushchenko @YuSanka, Pavel Mikuš @Godrak, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Filip Sykala
...
///|/
///|/ FlsunSlicer is released under the terms of the AGPLv3 or higher
///|/
...
static const t_config_enum_values s_keys_map_PrintHostType{{"prusalink", htPrusaLink},
...
static const t_config_enum_values s_keys_map_DraftShield = {{"disabled", dsDisabled}, {"limited", dsLimited}, {"enabled", dsEnabled}};
...
void PrintConfigDef::init_common_params()
{
    ...
    def->set_default_value(
        new ConfigOptionPoints{Vec2d(96.5926, 25.8819),   Vec2d(93.9693, 34.202),    Vec2d(90.6308, 42.2618),   Vec2d(86.6025, 50),
                               Vec2d(81.9152, 57.3576),   Vec2d(76.6044, 64.2788),   Vec2d(70.7107, 70.7107),   Vec2d(64.2788, 76.6044),
                               Vec2d(57.3576, 81.9152),   Vec2d(50, 86.6025),        Vec2d(42.2618, 90.6308),   Vec2d(34.202, 93.9693),
                               Vec2d(25.8819, 96.5926),   Vec2d(17.3648, 98.4808),   Vec2d(8.71557, 99.6195),   Vec2d(6.12323e-15, 100),
                               Vec2d(-8.71557, 99.6195),  Vec2d(-17.3648, 98.4808),  Vec2d(-25.8819, 96.5926),  Vec2d(-34.202, 93.9693),
                               Vec2d(-42.2618, 90.6308),  Vec2d(-50, 86.6025),       Vec2d(-57.3576, 81.9152),  Vec2d(-64.2788, 76.6044),
                               Vec2d(-70.7107, 70.7107),  Vec2d(-76.6044, 64.2788),  Vec2d(-81.9152, 57.3576),  Vec2d(-86.6025, 50),
                               Vec2d(-90.6308, 42.2618),  Vec2d(-93.9693, 34.202),   Vec2d(-96.5926, 25.8819),  Vec2d(-98.4808, 17.3648),
                               Vec2d(-99.6195, 8.71557),  Vec2d(-100, 1.22465e-14),  Vec2d(-99.6195, -8.71557), Vec2d(-98.4808, -17.3648),
                               Vec2d(-96.5926, -25.8819), Vec2d(-93.9693, -34.202),  Vec2d(-90.6308, -42.2618), Vec2d(-86.6025, -50),
                               Vec2d(-81.9152, -57.3576), Vec2d(-76.6044, -64.2788), Vec2d(-70.7107, -70.7107), Vec2d(-64.2788, -76.6044),
                               Vec2d(-57.3576, -81.9152), Vec2d(-50, -86.6025),      Vec2d(-42.2618, -90.6308), Vec2d(-34.202, -93.9693),
                               Vec2d(-25.8819, -96.5926), Vec2d(-17.3648, -98.4808), Vec2d(-8.71557, -99.6195), Vec2d(-1.83697e-14, -100),
                               Vec2d(8.71557, -99.6195),  Vec2d(17.3648, -98.4808),  Vec2d(25.8819, -96.5926),  Vec2d(34.202, -93.9693),
                               Vec2d(42.2618, -90.6308),  Vec2d(50, -86.6025),       Vec2d(57.3576, -81.9152),  Vec2d(64.2788, -76.6044),
                               Vec2d(70.7107, -70.7107),  Vec2d(76.6044, -64.2788),  Vec2d(81.9152, -57.3576),  Vec2d(86.6025, -50),
                               Vec2d(90.6308, -42.2618),  Vec2d(93.9693, -34.202),   Vec2d(96.5926, -25.8819),  Vec2d(98.4808, -17.3648),
                               Vec2d(99.6195, -8.71557),  Vec2d(100, -2.44929e-14)});
    // def->set_default_value(new ConfigOptionPoints{200,Vec2d(0, 0)});
    ...
    def->tooltip = L(
        "Picture sizes to be stored into a .gcode and .sl1 / .sl1s files, in the following format: \"XxY/EXT, XxY/EXT, ...\"\n"
    ...
    def->max      = 1200;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(430.0));
    ...
    //    def->tooltip = L("");
    def->set_enum<AuthorizationType>({{"key", L("API key")}, {"user", L("HTTP digest")}});
    def->mode = comAdvanced;
    def->cli  = ConfigOptionDef::nocli;
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->set_enum<ArcFittingType>({{"disabled", "Disabled"}, {"emit_center", "Enabled: G2/3 I J"}});
    def->mode = comAdvanced;
    ...
    def->tooltip    = L("This code is inserted between objects when using sequential printing. By default extruder and bed temperature are "
                     "reset using non-wait command; however if M104, M109, M140 or M190 are detected in this custom code, Slic3r will not "
                     "add temperature commands. Note that you can use placeholder variables for all Slic3r settings, so you can put a "
    ...
    def           = this->add("bridge_deceleration", coFloat);
    def->label    = L("Bridge");
    def->tooltip  = L("This is the deceleration your printer will use for bridges. "
                     "Set zero to disable acceleration control for bridges.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def           = this->add("enable_inner_dynamic_overhang_speeds", coBool);
    def->label    = L("Enable the inner profile dynamic overhang speed");
    def->category = L("Speed");
    def->tooltip  = L("This setting enables dynamic speed control of the overhang's inner profile.");
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionBool(false));

    // TRN PrintSettings : "Dynamic overhang speed"
    auto overhang_speed_setting_description = L(
        "overlap size is expressed as a percentage of overlap of the extrusion with the previous layer: "
    ...
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloatOrPercent(240, false));
    ...
    def->max      = 100;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionInts{30});
    ...
    def->max      = 100;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionInts{70});
    ...
    def->max      = 100;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionInts{90});
    ...
    def->mode = comSimple;
    def->set_default_value(new ConfigOptionEnum<BrimType>(btNoBrim));
    ...
    def->tooltip    = L("This end procedure is inserted at the end of the output file. "
                     "Note that you can use placeholder variables for all FlsunSlicer settings.");
    def->multiline  = true;
    def->full_width = true;
    def->height     = 12;
    def->mode       = comExpert;
    ...
    def->set_enum<InfillPattern>({{"rectilinear", L("Rectilinear")},
    ...
    def->set_default_value(new ConfigOptionEnum<InfillPattern>(ipMonotonicLines));
    ...
    def          = this->add("inner_wall_flow", coFloat);
    def->label   = L("Inner wall flow");
    def->tooltip = L("Used to control inner wall flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});

    def          = this->add("external_wall_flow", coFloat);
    def->label   = L("External wall flow");
    def->tooltip = L("Used to control external wall flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});

    def          = this->add("fill_flow", coFloat);
    def->label   = L("Fill flow"); 
    def->tooltip = L("Used to control fill flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});

   def          = this->add("top_flow", coFloat);
    def->label   = L("Top flow");
    def->tooltip = L("Used to control top flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});

    def          = this->add("bottom_solid_fill_flow", coFloat);
    def->label   = L("Bottom solid fill flow");
    def->tooltip = L("Used to control bottom solid fill flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});

    def          = this->add("first_flow", coFloat);
    def->label   = L("First-floor flow");
    def->tooltip = L("Used to control First-floo flow.");
    def->max     = 2;
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionFloat{1.0});
    ...
    def->tooltip  = L("Time for the printer firmware (or the Multi Material Unit 2.0) to load a new filament during a tool change (when "
    ...
    def->tooltip  = L("Time for the printer firmware (or the Multi Material Unit 2.0) to unload a filament during a tool change (when "
                     "executing the T code). This time is added to the total print time by the G-code time estimator.");
    def->sidetext = L("s");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def->sidetext = L("g/cm³");
    def->min      = 0;
    def->set_default_value(new ConfigOptionFloats{1.26});
    ...
    def->min       = 0;
    def->max       = 100;
    def->set_enum_values(ConfigOptionDef::GUIType::f_enum_open, {{"0", "0%"},
                                                                 {"5", "5%"},
                                                                 {"10", "10%"},
                                                                 {"15", "15%"},
                                                                 {"20", "20%"},
    ...
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(12000));

    def           = this->add("first_layer_deceleration", coFloat);
    def->label    = L("First layer");
    def->tooltip  = L("This is the deceleration your printer will use for first layer. Set zero "
                     "to disable deceleration control for first layer.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(12000));
    ...
    def->set_default_value(new ConfigOptionFloat(0));
     ...
     def           = this->add("first_layer_deceleration_over_raft", coFloat);
    def->label    = L("First object layer over raft interface");
    def->tooltip  = L("This is the deceleration your printer will use for first layer of object above raft interface. Set zero "
                     "to disable deceleration control for first layer of object above raft interface.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(0));
    ...
    def->max        = 0;
    def->max        = 300;
    def->set_default_value(new ConfigOptionInts{65});
    ...
    def->label       = L("First layer filling speed");
    ...
    def              = this->add("first_wall_velocity", coFloatOrPercent);
    def->label       = L("First wall velocity");
    def->tooltip     = L("If expressed as absolute value in mm/s, this speed will be applied to all the print moves "
                     "of the first layer, regardless of their type. If expressed as a percentage "
                     "(for example: 40%) it will scale the default speeds.");
    def->sidetext    = L("mm/s or %");
    ...
    def->min        = 0;
    def->max        = max_temp;
    def->set_default_value(new ConfigOptionInts{230});
    ...
    def->max     = 1000;
    def->mode    = comExpert;
    def->set_default_value(new ConfigOptionInts{2});

    def           = this->add("fuzzy_skin", coEnum);
    def->label    = L("Fuzzy Skin");
    def->category = L("Fuzzy Skin");
    def->tooltip  = L("Fuzzy skin type.");
    def->set_enum<FuzzySkinType>({{"none", L("None")}, {"external", L("Outside walls")}, {"all", L("All walls")}});
    def->mode = comSimple;
    ...
    def->tooltip = L("Some G/M-code commands, including temperature control and others, are not universal. "
                     ...
                     "The \"No extrusion\" flavor prevents FlsunSlicer from exporting any extrusion value at all.");
    def->set_enum<GCodeFlavor>({{"reprap", "RepRap/Sprinter"},
    ...
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(35000));
    ...
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(20000));

    def           = this->add("infill_deceleration", coFloat);
    def->label    = L("Infill");
    def->tooltip  = L("This is the deceleration your printer will use for infill. Set zero to disable "
                     "acceleration control for infill.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def           = this->add("solid_infill_deceleration", coFloat);
    def->label    = L("Solid infill");
    def->tooltip  = L("This is the deceleration your printer will use for solid infill. Set zero to use "
                     "the value for infill.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def           = this->add("perimeters_corner_velocity", coFloat);
    ...
    def        = this->add("external_perimeters_corner_velocity", coFloat);
    def->label = L("External perimeters");
    // def->category   = L("Speed");
    ...
    def           = this->add("solid_infill_corner_velocity", coFloat);
    def->label    = L("Solid infill");
    def->tooltip  = L("This is the acceleration your printer will use for solid infill. Set zero to use "
                     "the value for infill.");
    def->sidetext = L("mm/s");
    ...
    def           = this->add("infill_corner_velocity", coFloat);
    def->label    = L("infill");
    def->tooltip  = L("This is the speed at which your printer will be used for corner filling. Set 0 to use the populated value.");
    def->sidetext = L("mm/s");
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(800));

    def           = this->add("bridge_corner_velocity", coFloat);
    def->label    = L("Bridge");
    def->tooltip  = L("This is the acceleration your printer will use for bridges. "
                     "Set zero to disable acceleration control for bridges.");
    def->sidetext = L("mm/s");
    ...
    def           = this->add("top_solid_infill_corner_velocity", coFloat);
    ...
    def->label    = L("Top solid infill");
    def->tooltip  = L("This is the speed at which your printer will use the top solid fill. Set 0 to use The populated value.");
    def->sidetext = L("mm/s");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(38000));

    def           = this->add("top_solid_infill_deceleration", coFloat);
    def->label    = L("Top solid infill");
    def->tooltip  = L("This is the speed at which your printer will use the top solid fill. Set 0 to use The populated value.");
    def->sidetext = L("mm/s");
    def->min      = 0;
    ...
    def           = this->add("travel_deceleration", coFloat);
    def->label    = L("Travel");
    def->tooltip  = L("This is the deceleration your printer will use for travel moves. Set zero to disable "
                     "deceleration control for travel.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(38000));

    def           = this->add("set_deceleration_acceleration", coFloat);
    def->label    = L("Klipper's acceleration reduction");
    def->tooltip  = L("Set the deceleration acceleration of the Klipper.");
    def->sidetext = L("mm/s²");
    def->min      = 0;
    def->mode     = comExpert;
    ...
    def->tooltip                     = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "FlsunSlicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->max_literal                 = 1000;
    def->set_enum_values(ConfigOptionDef::GUIType::f_enum_open, {{"0", L("0 (no open anchors)")},
                                                                 {"1", L("1 mm")},
                                                                 {"2", L("2 mm")},
                                                                 {"5", L("5 mm")},
                                                                 {"10", L("10 mm")},
    ...
    def->set_enum_values(ConfigOptionDef::GUIType::f_enum_open, {{"0", L("0 (not anchored)")},
                                                                 {"1", L("1 mm")},
                                                                 {"2", L("2 mm")},
                                                                 {"5", L("5 mm")},
                                                                 {"10", L("10 mm")},
    ...
    def->max_literal = 50;
    def->mode        = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0.48, false));
    ...
    def          = this->add("top_level_single_layer_wall", coBool);
    def->label   = L("Top level single-layer wall setting");
    def->tooltip = L("This option controls whether the top fill is single-layer");
    def->mode    = comExpert;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->set_default_value(new ConfigOptionFloatOrPercent(25, true));
     ...
     def = this->add("adaptive_linewidth", coBool);
     def->label = L("Adaptive linewidth");
     def->category = L("Infill");
     def->tooltip = L("Used to control automatic adaptation of line width,It is recommended that the overlap is greater than 30%."
                        "Slicing after selection can take a lot of time." );
     def->mode = comAdvanced;
     def->set_default_value(new ConfigOptionBool(false));

     def           = this->add("accel_to_decel_enable", coBool);
     def->label    = L("Applied brake speed");
     def->tooltip  = L("Klipper's deceleration acceleration has been modified.");
     def->mode     = comExpert;
     def->set_default_value(new ConfigOptionBool(false));

     def           = this->add("accel_to_decel_factor", coPercent);
     def->label    = L("braking speed");
     def->tooltip  = L("The Klipper's deceleration acceleration has been modified as a percentage of acceleration.");
     def->sidetext = L("%");
     def->min      = 0;
     def->max      = 100;
     def->mode     = comExpert;
    ...
    def           = this->add("switch_layer_infill_speed", coFloat);
    def->label    = L("Switch layer filling speed");
    def->category = L("Speed");
    def->tooltip  = L("Filling velocity of the first cross-layer.");
    def->sidetext = L("mm/s");
    def->min      = 0;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(800));
    ...
    def->set_enum<IroningType>({{"top", L("All top surfaces")}, {"topmost", L("Topmost surface only")}, {"solid", L("All solid surfaces")}});
    def->mode = comAdvanced;
    ...
    def->set_enum<MachineLimitsUsage>(
        {{"emit_to_gcode", L("Emit to G-code")}, {"time_estimate_only", L("Use for time estimate")}, {"ignore", L("Ignore")}});
    def->mode = comAdvanced;
    ...
    def->min        = 0;
    def->mode       = comAdvanced;
    def->set_default_value(new ConfigOptionFloats{100000., 1250.});
    ...
    def->tooltip  = L("Generate no less than the number of skirt loops required to consume "
                     "the specified amount of filament on the bottom layer.");
    def->sidetext = L("mm");
    def->min      = 0;
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(15));
    ...
    def->set_enum<PrintHostType>({{"prusalink", "PrusaLink"},
    ...
    def           = this->add("external_perimeter_deceleration", coFloat);
    def->label    = L("External perimeters");
    def->tooltip  = L("This is the deceleration your printer will use for external perimeters. "
                     "Set zero to use the value for perimeters.");
    def->sidetext = L("mm/s²");
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(10000));

    def           = this->add("perimeter_deceleration", coFloat);
    def->label    = L("Perimeters");
    def->tooltip  = L("This is the deceleration your printer will use for perimeters. "
                     "Set zero to disable deceleration control for perimeters.");
    def->sidetext = L("mm/s²");
    def->mode     = comExpert;
    def->set_default_value(new ConfigOptionFloat(10000));
    ...
    def->max_literal = 50;
    def->mode        = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0.43, false));
    ...
    def          = this->add("travel_spiral_lift", coBools);
    def->label   = L("Use spiral lift");
    def->tooltip = L("Used with screw lift.");
    def->mode    = comAdvanced;
    def->set_default_value(new ConfigOptionBools{false});
    ...
    def->tooltip = L("If enabled, FlsunSlicer detects obstacles along the travel path and makes the slope steeper "
    ...
    def              = this->add("estimated_empty_distance", coFloats);
    def->label       = L("Estimated empty distance");
    def->tooltip     = L("Used to set estimates of empty driving distance. When the empty driving distance is greater than this value, it will first lift in the empty driving, otherwise only empty driving.");
    def->sidetext    = L("mm");
    def->min         = 0;
    def->max_literal = 1000;
    def->mode        = comSimple;
    ...
    def->sidetext = L("mm");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloats{-0.1});
    ...
    def->set_enum<SeamPosition>({{"random", L("Random")}, {"nearest", L("Nearest")}, {"aligned", L("Aligned")}, {"rear", L("Rear")}});
    def->mode = comSimple;
...
//    def->gui_type = ConfigOptionDef::GUIType::slider;
    ...
    def->set_enum<DraftShield>({{"disabled", L("Disabled")}, {"limited", L("Limited")}, {"enabled", L("Enabled")}});
    def->mode = comAdvanced;
    ...
    def->category   = L("Speed");
    def->tooltip    = L("This separate setting will affect the speed of perimeters having radius <= Small boundary radius "
    ...
    def           = this->add("small_boundary_radius", coFloat);
    def->label    = L("Small boundary radius");
    def->category = L("Speed");
    def->tooltip  = L("Set a small boundary radius.");
    def->sidetext = L("mm");
    def->min      = 0;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(1));
    ...
    def->max_literal = 50;
    def->mode        = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0.46, false));
    ...
    def->min        = 0;
    def->mode       = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(500, false));
    ...
    def->tooltip = L("When enabled, FlsunSlicer will check whether your Custom Start G-Code contains M104 or M190. "
                     "If so, the temperatures will not be emitted automatically so you're free to customize "
                     "the order of heating commands and other custom actions. Note that you can use "
                     "placeholder variables for all FlsunSlicer settings, so you can put "
                     "a \"M109 S[first_layer_temperature]\" command wherever you want.\n"
                     "If your Custom Start G-Code does NOT contain M104 or M190, "
                     "FlsunSlicer will execute the Start G-Code after bed reached its target temperature "
                     "and extruder just started heating.\n\n"
                     "When disabled, FlsunSlicer will NOT emit commands to heat up extruder and bed, "
    ...
    def->tooltip    = L("This start procedure is inserted at the beginning, after any printer start gcode (and "
                     ...
                     "This is used to override settings for a specific filament. If FlsunSlicer detects "
                     ...
                     "for all FlsunSlicer settings, so you can put a \"M109 S[first_layer_temperature]\" command "
    ...
    def->set_enum<SlicingMode>({{"regular", L("Regular")}, {"even_odd", L("Even-odd")}, {"close_holes", L("Close holes")}});
    def->mode = comAdvanced;
    ...
    def->set_enum_values(ConfigOptionDef::GUIType::f_enum_open,
                         {{"0", L("0 (soluble)")}, {"0.1", L("0.1 (detachable)")}, {"0.2", L("0.2 (detachable)")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(0.2));
    ...
    def->max_literal = 50;
    def->mode        = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(0.44, false));
    ...
    def->set_enum_values(ConfigOptionDef::GUIType::i_enum_open,
                         {{"0", L("0 (off)")}, {"1", L("1 (light)")}, {"2", L("2 (default)")}, {"3", L("3 (heavy)")}});
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(3));
    ...
    def->tooltip  = L("Style and shape of the support towers. Projecting the supports into a regular grid "
                     ...
                     "object scarring.");
    def->set_enum<SupportMaterialStyle>({{"grid", L("Grid")}, {"snug", L("Snug")}, {"organic", L("Organic")}});
    def->mode = comAdvanced;
    ...
    def->tooltip = L(
        "This custom code is inserted before every toolchange. Placeholder variables for all FlsunSlicer settings "
        "as well as {toolchange_z}, {previous_extruder} and {next_extruder} can be used. When a tool-changing command "
        "which changes to the correct extruder is included (such as T{next_extruder}), FlsunSlicer will emit no other such command. "
    ...
    def->min        = 0;
    def->mode       = comAdvanced;
    def->set_default_value(new ConfigOptionFloatOrPercent(400, false));
    ...
    def           = this->add("travel_speed_first_layer", coFloat);
    ...
    def->set_default_value(new ConfigOptionFloats{0.,   140., 140., 140., 140., 140., 0.,   140., 140., 140., 140., 140., 0.,
                                                  140., 140., 140., 140., 140., 0.,   140., 140., 140., 140., 140., 0.});
    ...
    def->set_default_value(new ConfigOptionFloat(0));
    //设置热床分区大小
    def           = this->add("hot_bed_zoning", coFloat);
    def->label    = L("Hot bed zoning");
    def->tooltip  = L("Offset of brim from the printed object. The offset is applied after the elephant foot compensation.");
    def->sidetext = L("mm");
    def->min      = 0;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(110.f));
    ...
    def->set_enum<PerimeterGeneratorType>({{"classic", L("Classic")}, {"arachne", L("Arachne")}});
    def->mode = comAdvanced;
    ...
    for (const char *opt_key :
         {// floats
                                "retract_length", "retract_lift", "retract_lift_above", "estimated_empty_distance",
                                "retract_lift_below",
                                "retract_speed",
                                "travel_max_lift", "deretract_speed", "retract_restart_extra", "retract_before_travel",
                                "retract_length_toolchange", "retract_restart_extra_toolchange",
                                // bools
                                "retract_layer_change", "wipe", "travel_lift_before_obstacle", "travel_ramping_lift", "travel_spiral_lift",
                                // percents
...
void PrintConfigDef::init_extruder_option_keys()
{
    ...
    m_extruder_option_keys = {"nozzle_diameter",
                              "min_layer_height",
                              "max_layer_height",
                              "extruder_offset", "estimated_empty_distance",
                              "retract_length",
                              "retract_lift",
                              "retract_lift_above", 
                              "retract_lift_below",
...
void PrintConfigDef::init_sla_params()
{
    ...
    def->set_enum<SLADisplayOrientation>({{"landscape", L("Landscape")}, {"portrait", L("Portrait")}});
    def->mode = comExpert;
    ...
    def->set_enum<SLAMaterialSpeed>({{"slow", L("Slow")}, {"fast", L("Fast")}, {"high_viscosity", L("High viscosity")}});
    def->mode = comAdvanced;
    ...
    for (const char *opt_key : {// float
                                "support_head_front_diameter", "branchingsupport_head_front_diameter", "support_head_penetration",
                                "branchingsupport_head_penetration", "support_head_width", "branchingsupport_head_width",
                                "support_pillar_diameter", "branchingsupport_pillar_diameter", "relative_correction_x",
                                "relative_correction_y", "relative_correction_z", "elefant_foot_compensation",
                                // int
...
static std::set<std::string> PrintConfigDef_ignore = {
    ...
    "serial_port",
    "serial_speed",
    // Introduced in some FlsunSlicer 2.3.1 alpha, later renamed or removed.
    "fuzzy_skin_perimeter_mode",
    "fuzzy_skin_shape",
    // Introduced in FlsunSlicer 2.3.0-alpha2, later replaced by automatic calculation based on extrusion width.
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    if (opt_key == "extrusion_width_ratio" || opt_key == "bottom_layer_speed_ratio" || opt_key == "first_layer_height_ratio") {
        ...
        if (value == "makerbot")
            ...
            // the "new" marlin firmware flavor used to be called "marlinfirmware" for some time during FlsunSlicer 2.4.0-alpha development.
            value = "marlin2";
        ...
        // FlsunSlicer no more accepts skirt_height == -1 to print a draft shield to the top of the highest object.
        // A new "draft_shield" enum config value is used instead.
        opt_key = "draft_shield";
        value   = "enabled";
    } else if (opt_key == "draft_shield" && (value == "1" || value == "0")) {
        // draft_shield used to be a bool, it was turned into an enum in FlsunSlicer 2.4.0.
        ...
        // and enum since FlsunSlicer 2.6.2.
    ...
    // In FlsunSlicer 2.3.0-alpha0 the "monotonous" infill was introduced, which was later renamed to "monotonic".
...
std::string validate(const FullPrintConfig &cfg)
{
    ...
    if (cfg.use_firmware_retraction.value && cfg.gcode_flavor.value != gcfSmoothie && cfg.gcode_flavor.value != gcfRepRapSprinter &&
        cfg.gcode_flavor.value != gcfRepRapFirmware && cfg.gcode_flavor.value != gcfMarlinLegacy &&
        cfg.gcode_flavor.value != gcfMarlinFirmware && cfg.gcode_flavor.value != gcfMachinekit && cfg.gcode_flavor.value != gcfRepetier &&
    ...
    // The following test was commented out after 482841b, see also https://github.com/prusa3d/FlsunSlicer/pull/6743.
    ...
    // if ((cfg.perimeter_acceleration != 0. || cfg.infill_acceleration != 0. || cfg.bridge_acceleration != 0. ||
    // cfg.first_layer_acceleration != 0.) &&
...
CLITransformConfigDef::CLITransformConfigDef()
{
    ...
    */
    ...
    def->label   = L("Repair");
    def->tooltip = L("Try to repair any non-manifold meshes (this option is implicitly added whenever we need to slice the model to "
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of FlsunSlicer may not understand configurations produced by the newest FlsunSlicer versions. "
                     "For example, newer FlsunSlicer may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI FlsunSlicer, "
                     "or an existing FlsunSlicer window is activated. "
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so FlsunSlicer knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so FlsunSlicer deretracts correctly when it gets control back.");
...
CustomGcodeSpecificConfigDef::CustomGcodeSpecificConfigDef()
{
    ...
    // TRN: This is a label in custom g-code editor dialog, belonging to color_change_extruder. Denoted index of the extruder for which
...
Points get_bed_shape(const PrintConfig &cfg) { return to_points(cfg.bed_shape.values); }
