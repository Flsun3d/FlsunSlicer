...
#include "format.hpp"
#include "XAlgorithm/FillWidth.cpp"
...
//        print_object_instances_ordering = sort_object_instances_by_max_z(print);
    ...
    if (export_to_binary_gcode) {
        ...
        // Append full config, delimited by two 'phony' configuration keys flsunslicer_config = begin and flsunslicer_config = end.
        // The delimiters are structured as configuration key / value pairs to be parsable by older versions of PrusaSlicer G-code viewer.
        {
            file.write("\n; flsunlicer_config = begin\n");
            std::string full_config;
            append_full_config(*m_print, full_config);
            if (!full_config.empty())
                file.write(full_config);
            file.write("; flsunlicer_config = end\n");
        }
    }
    print.throw_if_canceled();
}
    ...
    const DynamicConfig *config_override)
{
    ...
    try {
        ...
        if (m_config.printer_model.value == "S1" && name == "start_gcode") {
                      std::string shape =
                "109.581x9.58713,108.329x19.1013,106.252x28.4701,103.366x37.6222,99.6939x46.488,95.2628x55,90.1067x63.0934,84.2649x70.7066,"
                "77.7817x77.7817,70.7066x84.2649,63.0934x90.1067,55x95.2628,46.488x99.6939,37.6222x103.366,28.4701x106.252,19.1013x108.329,"
                "9.58713x109.581,6.73556e-15x110,-9.58713x109.581,-19.1013x108.329,-28.4701x106.252,-37.6222x103.366,-46.488x99.6939,-"
                "55x95.2628,-63.0934x90.1067,-70.7066x84.2649,-77.7817x77.7817,-84.2649x70.7066,-90.1067x63.0934,-95.2628x55,-99.6939x46."
                "488,-103.366x37.6222,-106.252x28.4701,-108.329x19.1013,-109.581x9.58713,-110x1.34711e-14,-109.581x-9.58713,-108.329x-19."
                "1013,-106.252x-28.4701,-103.366x-37.6222,-99.6939x-46.488,-95.2628x-55,-90.1067x-63.0934,-84.2649x-70.7066,-77.7817x-77."
                "7817,-70.7066x-84.2649,-63.0934x-90.1067,-55x-95.2628,-46.488x-99.6939,-37.6222x-103.366,-28.4701x-106.252,-19.1013x-108."
                "329,-9.58713x-109.581,-2.02067e-14x-110,9.58713x-109.581,19.1013x-108.329,28.4701x-106.252,37.6222x-103.366,46.488x-99."
                "6939,55x-95.2628,63.0934x-90.1067,70.7066x-84.2649,77.7817x-77.7817,84.2649x-70.7066,90.1067x-63.0934,95.2628x-55,99."
                "6939x-46.488,103.366x-37.6222,106.252x-28.4701,108.329x-19.1013,109.581x-9.58713,110x-2.69422e-14";

            Polygon                  pCircle;
            std::vector<std::string> string_split;
            boost::split(string_split, shape, boost::is_any_of(","), boost::token_compress_on);
            for (std::string sx : string_split) {
                std::vector<std::string> strVecx;
                boost::split(strVecx, sx, boost::is_any_of("x"), boost::token_compress_on);
                Point p(scale_(std::stod(strVecx[0])), scale_(std::stod(strVecx[1])));
                pCircle.append(p);
            }
            ...
            bool isIn  = false;
            bool isOut = false;
            for (auto obj : m_print->m_objects) {
                if (!obj->layers().empty()) {
                    if (!obj->layers()[0]->lslices[0].holes.empty()) {
                        for (auto hole : obj->layers()[0]->lslices[0].holes) {
                            isOut          = false;
                            Polygons itsec = diff(pCircle, hole);
                            if (itsec.empty()) {
                                isOut = true;
                                break;
                            }
...
std::string GCodeGenerator::get_layer_change_gcode(const Vec3d& from, const Vec3d& to, const unsigned extruder_id) {
    ...
    if (sqrt(pow(to.x() - from.x(), 2) + pow(to.y() - from.y(), 2)) * 1e-6 <=
        m_config.estimated_empty_distance.get_at(extruder_id)) {
        //不抬升
    ...
    const size_t                     		 single_object_instance_idx)
{
    ...
    // lmCommon, lmSlant, lmSpiral
    bool bl = m_config.travel_spiral_lift.get_at(*m_layer_change_extruder_id);
    if (first_layer) {
        layer_change_gcode = "; move to next layer (1)\n"; // Explicit for readability.
    
    } else if (do_ramping_layer_change) {
        if (m_config.printer_model.value == "S1" && layer.id() == 1) {
           
            layer_change_gcode += "SET_TMC_CURRENT STEPPER=extruder CURRENT=1.2\n";
        }
        std::string layer_change = "move to next  layer (" + std::to_string(layer.id()) + ")";
    ...
    } else if (m_config.travel_spiral_lift.get_at(*m_layer_change_extruder_id)) {
      ...
      const Vec3d sta = *m_previous_layer_last_position;
      const Vec3d end = *m_current_layer_first_position;
      if (sqrt(pow(end.x() - sta.x(), 2) + pow(end.y() - sta.y(), 2)) * 1e-6 <=
          m_config.estimated_empty_distance.get_at(*m_layer_change_extruder_id)) {
          std::string layer_change = "Simple Lift Z move to next  layer (" + std::to_string(layer.id() + 1) + ")";
          //不抬升
          layer_change_gcode += this->writer().get_travel_to_z_gcode(print_z, layer_change.c_str());
      } else {
          std::string layer_change = "Spiral Lift Z move to next  layer (" + std::to_string(layer.id() + 1) + ")";
          //抬升
           layer_change_gcode = this->writer().spiral_travel_to_z(*m_previous_layer_last_position, *m_current_layer_first_position,
                                                                 print_z + m_config.retract_lift.get_at(*m_layer_change_extruder_id),
                                                                layer_change.c_str());
      }
        ...
        std::string layer_change = "move to next layer (" + std::to_string(layer.id()+1) + ")";
        
        const Vec3d sta = *m_previous_layer_last_position;
        const Vec3d end = *m_current_layer_first_position;
        double      dis = sqrt(pow(end.x() - sta.x(), 2) + pow(end.y() - sta.y(), 2)) * 1e-6;
        if (sqrt(pow(end.x() - sta.x(),2) + pow(end.y() - sta.y(), 2)) <=
            m_config.estimated_empty_distance.get_at(*m_layer_change_extruder_id)) {
            //不抬升          
            layer_change_gcode += this->writer().get_travel_to_z_gcode(print_z,layer_change.c_str());
        } else {
            //抬升
            layer_change_gcode += this->writer().get_travel_to_z_gcode(print_z + (this->config().retract_lift.get_at( *m_layer_change_extruder_id) - m_layer->height),layer_change.c_str());    
        }        
    }
...
// get perimeters from layer region.
ExtrusionPaths GCodeGenerator::get_perimeters_from_layer_region(const LayerRegion *_region)
{
    auto _perimeters = _region->perimeters();

    ExtrusionPaths _perimeter_paths;

    ExtrusionEntitiesPtr _perimeter_collections = _perimeters.entities;
    for (auto _perimeter_collection_it = _perimeter_collections.begin(); _perimeter_collection_it != _perimeter_collections.end();
         _perimeter_collection_it++) {
        const ExtrusionEntityCollection *_perimeter_collection = dynamic_cast<const ExtrusionEntityCollection *>(*_perimeter_collection_it);
        auto                             _perimeter_entities   = _perimeter_collection->entities;

        for (auto _perimeter_it = _perimeter_entities.begin(); _perimeter_it != _perimeter_entities.end(); _perimeter_it++) {
            auto _perimeter = *_perimeter_it;

            if (const ExtrusionPath *_path = dynamic_cast<const ExtrusionPath *>(_perimeter)) {
                _perimeter_paths.push_back(*_path);
            } else if (const ExtrusionMultiPath *_perimeter_multipath = dynamic_cast<const ExtrusionMultiPath *>(_perimeter)) {
                auto _multi_paths = _perimeter_multipath->paths;
                for (auto _path_it = _multi_paths.begin(); _path_it < _multi_paths.end(); _path_it++) {
                    _perimeter_paths.push_back(*_path_it);
                }
            } else if (const ExtrusionLoop *_perimeter_loop = dynamic_cast<const ExtrusionLoop *>(_perimeter)) {
                auto _loop_paths = _perimeter_loop->paths;
                for (auto _path_it = _loop_paths.begin(); _path_it != _loop_paths.end(); _path_it++) {
                    _perimeter_paths.push_back(*_path_it);
                }
            }
        }
    ...
    return _perimeter_paths;
}


// get target perimeters from layer.
ExtrusionPaths GCodeGenerator::get_perimeters_from_layer(const ObjectLayerToPrint &_layer_to_print, const ExtrusionRole &_tgt_role)
{
    ExtrusionPaths _perimeter_paths;

    m_layer        = _layer_to_print.layer();
    auto _layer_id = m_layer->id();

    auto _layer_regions = m_layer->regions();

    for (auto _region_it = _layer_regions.begin(); _region_it != _layer_regions.end(); _region_it++) {
        auto _perimeters = (*_region_it)->perimeters();
        ExtrusionEntitiesPtr _perimeter_collections = _perimeters.entities;

        for (auto _perimeter_collection_it = _perimeter_collections.begin(); _perimeter_collection_it != _perimeter_collections.end();
             _perimeter_collection_it++) {
            const ExtrusionEntityCollection *_perimeter_collection = dynamic_cast<const ExtrusionEntityCollection *>(
                *_perimeter_collection_it);
            auto _perimeter_entities = _perimeter_collection->entities;
            ...
            for (auto _perimeter_it = _perimeter_entities.begin(); _perimeter_it != _perimeter_entities.end(); _perimeter_it++) {
                ...
                if (const ExtrusionPath *_path = dynamic_cast<const ExtrusionPath *>(_perimeter)) {
                    if (_path->role() == _tgt_role) {
                        _perimeter_paths.push_back(*_path);
                    }
                } else if (const ExtrusionMultiPath *_perimeter_multipath = dynamic_cast<const ExtrusionMultiPath *>(_perimeter)) {
                    auto _multi_paths = _perimeter_multipath->paths;
                    for (auto _path_it = _multi_paths.begin(); _path_it < _multi_paths.end(); _path_it++) {
                        if (_path_it->role() == _tgt_role) {
                            _perimeter_paths.push_back(*_path_it);
                        }
                    }
                } else if (const ExtrusionLoop *_perimeter_loop = dynamic_cast<const ExtrusionLoop *>(_perimeter)) {
...
// This function will segment infill and modify the width of infill segment.
void GCodeGenerator::process_layer_infill_width(const ObjectLayerToPrint &layer_to_print)
{
    m_layer        = layer_to_print.layer();
    auto _layer_id = m_layer->id();

    if (_layer_id < 5 || _layer_id > m_layer_count - 5) {
        return;
    }
    ...
    for (auto _region_it = _layer_regions.begin(); _region_it != _layer_regions.end(); _region_it++) {
        // get perimeters from region.
        ExtrusionPaths _perimeters = get_perimeters_from_layer_region(*_region_it);

        auto _perimeter_bounding_boxes = get_bounding_boxes_from_extrusion_paths(_perimeters);
        auto _perimeter_rtree = get_rtree_from_bounding_boxes(_perimeter_bounding_boxes);

        auto &               _fills            = (*_region_it)->fills();
        ExtrusionEntitiesPtr _fill_collections = _fills.entities;

        for (auto _fill_collection_it = _fill_collections.begin(); _fill_collection_it != _fill_collections.end(); _fill_collection_it++) {
            ExtrusionEntityCollection *_fill_collection = dynamic_cast<ExtrusionEntityCollection *>(*_fill_collection_it);
            auto &                     _fill_entities   = _fill_collection->entities;
            for (auto _fill_it = _fill_entities.begin(); _fill_it != _fill_entities.end(); _fill_it++) {
                auto &_fill = *_fill_it;

                if (ExtrusionPath *_fill_path = dynamic_cast<ExtrusionPath *>(_fill)) {
                    std::vector<int> _near_perimeter_indices;

                    //bool _is_fill_near_perimeters = is_infill_near_perimeters(_fill_path, _perimeters, _perimeter_bounding_boxes, _near_perimeter_indices);
                    bool _is_fill_near_perimeters = is_infill_near_perimeters(_fill_path, _perimeters, _perimeter_rtree, _near_perimeter_indices);

                    if (_is_fill_near_perimeters) {
                        ExtrusionPaths _near_perimeters;
                        for (auto _near_idx : _near_perimeter_indices) {
                            _near_perimeters.push_back(_perimeters[_near_idx]);
                        }

                        ExtrusionPaths _infill_segments = segment_infill_path(_fill_path, _near_perimeters);

                        if (_infill_segments.size() > 0) {
                            // std::shared_ptr<ExtrusionMultiPath> _infill_multipath_ptr = std::make_shared<ExtrusionMultiPath>(_infill_segments);
                            ExtrusionMultiPath *_infill_multipath_ptr = new ExtrusionMultiPath(_infill_segments);
                            _fill                                     = _infill_multipath_ptr;
                        }
                    }
                } else if (ExtrusionMultiPath *_multipath = dynamic_cast<ExtrusionMultiPath *>(_fill)) {
                    ExtrusionPaths _paths;
                    for (auto _fill_path : _multipath->paths) {
                        std::vector<int> _near_perimeter_indices;
                        
                        //bool _is_fill_near_perimeters = is_infill_near_perimeters(&_fill_path, _perimeters, _perimeter_bounding_boxes, _near_perimeter_indices);
                        bool _is_fill_near_perimeters = is_infill_near_perimeters(&_fill_path, _perimeters, _perimeter_rtree, _near_perimeter_indices);
                        //bool _is_fill_near_perimeters = is_infill_near_perimeters(&_fill_path, _perimeters, _perimeter_meshes,
                        //                                                          _perimeter_bounding_boxes, _near_perimeter_indices);
                        ...
                        if (_is_fill_near_perimeters) {
                            ...
                            ExtrusionPaths _infill_segments = segment_infill_path(&_fill_path, _near_perimeters);
                            if (_infill_segments.size() > 0) {
                                for (auto _segment : _infill_segments) {
                                    _paths.push_back(_segment);
                                }
                            } else {
                                _paths.push_back(_fill_path);
                            }
                        } else {
                            _paths.push_back(_fill_path);
                        }
                    ...
                    if (_paths.size() > 0) {
                        ExtrusionMultiPath *_modified_multipath = new ExtrusionMultiPath(_paths);
                        _fill                                   = _modified_multipath;
                    }
                } else if (ExtrusionLoop *_loop = dynamic_cast<ExtrusionLoop *>(_fill)) {
                    ExtrusionPaths _paths;
                    for (auto _fill_path : _loop->paths) {
                        std::vector<int> _near_perimeter_indices;
                    ...
                    if (_paths.size() > 0) {
                        ExtrusionLoop *_modified_loop = new ExtrusionLoop(_paths);
                        _fill                         = _modified_loop;
                    }
                }
            }
        }
...
// process overhang speed of internal perimeters.
void GCodeGenerator::process_internal_perimeter_overhang_speed(const ObjectLayerToPrint &layer_to_print) {
    // get external perimeters from layer.
    auto _external_perimeters = get_perimeters_from_layer(layer_to_print, ExtrusionRole::ExternalPerimeter);

    // get bounding boxes from _external_perimeters.
    auto _external_perimeter_bboxes = get_bounding_boxes_from_extrusion_paths(_external_perimeters);
    // get rtree from _external_perimeter_bboxes.
    auto _external_perimeter_rtree  = get_rtree_from_bounding_boxes(_external_perimeter_bboxes);

    m_layer        = layer_to_print.layer();
    auto _layer_regions = m_layer->regions();

    double _near_dist_threshold = 0;
    if (m_config.perimeters.value > 1 + EPSILON) {
        _near_dist_threshold = scale_(m_config.external_perimeter_extrusion_width.value * 0.5 +
                                      m_config.perimeter_extrusion_width * (m_config.perimeters.value - 1.5));
    }
    ...
    for (auto _region_it = _layer_regions.begin(); _region_it != _layer_regions.end(); _region_it++) {
        ...
        for (auto _perimeter_collection_it = _perimeter_collections.begin(); _perimeter_collection_it != _perimeter_collections.end(); _perimeter_collection_it++) {
            ExtrusionEntityCollection *_perimeter_collection = dynamic_cast<ExtrusionEntityCollection *>(*_perimeter_collection_it);
            auto & _perimeter_entities = _perimeter_collection->entities;

            for (auto _perimeter_it = _perimeter_entities.begin(); _perimeter_it != _perimeter_entities.end(); _perimeter_it++) {
                auto & _perimeter = *_perimeter_it;

                if (ExtrusionPath *_perimeter_path = dynamic_cast<ExtrusionPath *>(_perimeter)) {
                    if (_perimeter_path->role() == ExtrusionRole::Perimeter) {
                        std::vector<int> _near_external_perimeter_indices;
                        bool _is_near_external_perimeters = is_near_perimeters(_perimeter_path, _external_perimeters,
                                                                               _external_perimeter_rtree, _near_dist_threshold,
                                                                               _near_external_perimeter_indices);

                        if (_is_near_external_perimeters) {
                            ExtrusionPaths _near_external_perimeters;
                            for (auto _near_idx : _near_external_perimeter_indices) {
                                _near_external_perimeters.push_back(_external_perimeters[_near_idx]);
                            }

                            ExtrusionPaths _internal_perimeter_segments = segment_internal_perimeter_path(_perimeter_path, _near_external_perimeters);
                            if (_internal_perimeter_segments.size() > 0) {
                                ExtrusionMultiPath *_internal_perimeter_multipath_ptr = new ExtrusionMultiPath(_internal_perimeter_segments);
                                _perimeter = _internal_perimeter_multipath_ptr;
                            }
                        }

                    }
                } else if (const ExtrusionMultiPath *_multipath = dynamic_cast<const ExtrusionMultiPath *>(_perimeter)) {
                    ExtrusionPaths _paths;
                    for (auto _perimeter_path : _multipath->paths) {
                        if (_perimeter_path.role() != ExtrusionRole::Perimeter) {
                            _paths.push_back(_perimeter_path);
                            continue;
                        }

                        std::vector<int> _near_external_perimeter_indices;
                        bool _is_near_external_perimeters = is_near_perimeters(&_perimeter_path, _external_perimeters,
                                                                               _external_perimeter_rtree,
                                                                               _near_dist_threshold, _near_external_perimeter_indices);
                        ...
                        if (_is_near_external_perimeters) {
                            ...
                            ExtrusionPaths _internal_perimeter_segments = segment_internal_perimeter_path(&_perimeter_path,
                                                                                                          _near_external_perimeters);
                            if (_internal_perimeter_segments.size() > 0) {
                                for (auto _segment : _internal_perimeter_segments) {
                                    _paths.push_back(_segment);
                                }
                            } else {
                                _paths.push_back(_perimeter_path);
                    ...
                    if (_paths.size() > 0) {
                        ExtrusionMultiPath *_modified_multipath = new ExtrusionMultiPath(_paths);
                        _perimeter                              = _modified_multipath;
                    }
                } else if (const ExtrusionLoop *_loop = dynamic_cast<const ExtrusionLoop *>(_perimeter)) {
                    ExtrusionPaths _paths;
                    for (auto _perimeter_path : _loop->paths) {
                        if (_perimeter_path.role() != ExtrusionRole::Perimeter) {
                            _paths.push_back(_perimeter_path);
                            continue;
                        }
    ...
    const bool                print_wipe_extrusions)
{
    ...
    if (m_config.adaptive_linewidth.value) {
        // modify the width of infill segment, which is part of infill and near the layer perimeters.
        process_layer_infill_width(layer_to_print);
    }
    if (m_config.enable_inner_dynamic_overhang_speeds.value) {
        // process overhang speed of internal perimeters.
        process_internal_perimeter_overhang_speed(layer_to_print);
    }
...
static double temporary_travel = 1;
...
std::string GCodeGenerator::extrude_loop(const ExtrusionLoop &loop_src, const GCode::SmoothPathCache &smooth_path_cache, const std::string_view description, double speed)
{
    ...
    auto small_rediu = (m_config.small_boundary_radius / SCALING_FACTOR) * 2 * PI;
    // Apply the small perimeter speed.
    if (loop_src.paths.front().role().is_perimeter() && loop_src.length() <= small_rediu && speed == -1)
...
std::string GCodeGenerator::extrude_entity(const ExtrusionEntityReference &entity, const GCode::SmoothPathCache &smooth_path_cache, const std::string_view description, double speed)
{
    if (const ExtrusionPath *path = dynamic_cast<const ExtrusionPath*>(&entity.extrusion_entity()))
        ...
        throw Slic3r::InvalidArgument(_u8L("Invalid argument supplied to extrude()"));
    return {};
}
...
std::string GCodeGenerator::extrude_path(const ExtrusionPath &path, bool reverse, const GCode::SmoothPathCache &smooth_path_cache, std::string_view description, double speed)
{
    ...
    if (m_config.fill_pattern == ipGrid) {
        smooth_path = smooth_path_cache.resolve_or_fit_infill(path, reverse, m_scaled_resolution);
    } else {
        smooth_path = smooth_path_cache.resolve_or_fit(path, reverse, m_scaled_resolution);
    }
...
std::string GCodeGenerator::travel_to_first_position(const Vec3crd &point, const double from_z)
{
    ...
    if (EXTRUDER_CONFIG(retract_length) > 0 && (!this->last_position || (!EXTRUDER_CONFIG(travel_ramping_lift)))) {
        if (!this->last_position || EXTRUDER_CONFIG(retract_before_travel) < (this->point_to_gcode(*this->last_position) - gcode_point.head<2>()).norm()) {
            ...
            this->writer().new_layer = m_layer_index;
            //gcode += this->writer().get_travel_to_z_gcode(from_z + lift, "lift");
            if (m_previous_layer_last_position.has_value()) 
            {
                const Vec3d sta = *m_previous_layer_last_position;
                if (sqrt(pow(gcode_point.x() - sta.x(), 2) + pow(gcode_point.y() - sta.y(), 2)) <=
                    m_config.estimated_empty_distance.get_at(*m_layer_change_extruder_id)) {
                    //不抬升
                    gcode += this->writer().get_travel_to_z_gcode(from_z, "lift");
    ...
    double                           speed)
{
    ...
    if (m_config.accel_to_decel_enable.value) {
        if (this->on_first_layer() && m_config.first_layer_deceleration.value > 0) {
            deceleration = m_config.first_layer_deceleration.value;
        } else if (this->object_layer_over_raft() && m_config.first_layer_deceleration_over_raft.value > 0) {
            deceleration = m_config.first_layer_deceleration_over_raft.value;
        } else if (m_config.bridge_deceleration.value > 0 && path_attr.role.is_bridge()) {
            deceleration = m_config.bridge_deceleration.value;
        } else if (m_config.top_solid_infill_deceleration > 0 && path_attr.role == ExtrusionRole::TopSolidInfill) {
            deceleration = m_config.top_solid_infill_deceleration.value;
        } else if (m_config.solid_infill_deceleration > 0 && path_attr.role.is_solid_infill()) {
            deceleration = m_config.solid_infill_deceleration.value;
        } else if (m_config.infill_deceleration.value > 0 && path_attr.role.is_infill()) {
            deceleration = m_config.infill_deceleration.value;
        } else if (m_config.external_perimeter_deceleration > 0 && path_attr.role.is_external_perimeter()) {
            deceleration = m_config.external_perimeter_deceleration.value;
        } else if (m_config.perimeter_deceleration.value > 0 && path_attr.role.is_perimeter()) {
            deceleration = m_config.perimeter_deceleration.value;
        } 
    }
    ...
    if (m_config.default_acceleration.value > 0) {
        ...
        gcode +=  deceleration > 0 ?
                     m_writer.set_print_acceleration((unsigned int) floor(acceleration + 0.5), (unsigned int) floor(deceleration + 0.5)) :
                     m_writer.set_print_acceleration((unsigned int) floor(acceleration + 0.5));
    }

    // calculate extrusion length per distance unit
    double e_per_mm = m_writer.extruder()->e_per_mm3() * path_attr.mm3_per_mm;
    if (m_layer_index == 0) {
        e_per_mm *= m_config.first_flow;
    }
    if (m_layer_index != 0 && m_layer_index < m_config.bottom_solid_layers.value  && path_attr.role == ExtrusionRole::SolidInfill) {
        e_per_mm *= m_config.bottom_solid_fill_flow;
    }
    if (path_attr.role == ExtrusionRole::TopSolidInfill) {
        e_per_mm *= m_config.top_flow;
    }
    if (path_attr.role == ExtrusionRole::InternalInfill) {
        e_per_mm *= m_config.fill_flow;
    }
    if (m_layer_index > 0 && path_attr.role == ExtrusionRole::ExternalPerimeter) {
        e_per_mm *= m_config.external_wall_flow;
    }
    if (m_layer_index > 0 && path_attr.role == ExtrusionRole::Perimeter) {
        e_per_mm *= m_config.inner_wall_flow;
    }
    ...
    if (speed == -1) {
        if (path_attr.role == ExtrusionRole::Perimeter) {
            ...
            throw Slic3r::InvalidArgument(_u8L("Invalid speed"));
        }
    }
    ...
    if (this->on_first_layer()) {
        if (path_attr.role == ExtrusionRole::Perimeter || path_attr.role == ExtrusionRole::ExternalPerimeter) {
            speed = m_config.get_abs_value("first_wall_velocity", speed);
        } else {
    ...
    if (path_attr.is_infill_near_perimeter) {
        double _max_near_perimeter_infill_speed = 200.0;
        if (path_attr.role == ExtrusionRole::InternalInfill) {
            double _tgt_infill_speed = m_config.get_abs_value("infill_speed");
            speed = std::min({speed, _tgt_infill_speed / 2.0, _max_near_perimeter_infill_speed});
        } else if (path_attr.role == ExtrusionRole::SolidInfill) {
            double _tgt_solid_infill_speed = m_config.get_abs_value("solid_infill_speed");
            speed = std::min({speed, _tgt_solid_infill_speed / 2.0, _max_near_perimeter_infill_speed});
        }
    }
    ...
    if (GCodeExtrusionRole role = extrusion_role_to_gcode_extrusion_role(path_attr.role); role != m_last_processor_extrusion_role) {
        ...
        std::string m_205;
        m_205 = buf;
        if (m_205 == ";TYPE:Perimeter\n") {
            gcode += "M205 X" + std::to_string((unsigned int) (m_config.perimeters_corner_velocity.value + 0.5));
            gcode += "\n";
        } else if (";TYPE:External perimeter\n" == m_205) {
            gcode += "M205 X" + std::to_string((unsigned int) (m_config.external_perimeters_corner_velocity.value + 0.5));
            gcode += "\n";
        } else if (";TYPE:Solid infill\n" == m_205) {
            gcode += "M205 X" + std::to_string((unsigned int) (m_config.solid_infill_corner_velocity.value + 0.5));
            gcode += "\n";
        } else if (";TYPE:Internal infill\n" == m_205) {
            gcode += "M205 X" + std::to_string((unsigned int) (m_config.infill_corner_velocity.value + 0.5));
            gcode += "\n";
        }
        else if (";TYPE:Top solid infill\n" == m_205) {
            gcode += "M205 X" + std::to_string((unsigned int) (m_config.top_solid_infill_corner_velocity.value + 0.5));
            gcode += "\n";
        }
    }
    ...
    bool Islow = false;
    for (int i = 0; i < m_layer->lslices.size(); i++) {
        if (m_layer->lslices[i].area() >=1.6e15 ) {
           
            Islow = true;
        }
    }
    // F is mm per minute.
    if (m_config.fill_pattern == ipRectilinear) {

        if (path_attr.role == ExtrusionRole::InternalInfill) {

            if (m_config.bottom_solid_layers.value == m_layer_index) {
                
                gcode += m_writer.set_speed(6000, "", cooling_marker_setspeed_comments);

            } else {

                if (m_layer_index > m_config.bottom_solid_layers.value && m_layer_index % 15 == 0) {
                   
                    if (Islow) {

                        gcode += m_writer.set_speed(m_config.switch_layer_infill_speed.value*60, "", cooling_marker_setspeed_comments);
    ...
    for (++ it; it != end; ++ it) {
        ...
        if (p != prev) {
            ...
            if (radius == 0) {
                ...
                {
                    if (comment == "Bridge infill") {
                        brigr_gcode += m_writer.extrude_to_xy(p, e_per_mm * line_length, comment);
                    }
...
std::string GCodeGenerator::generate_travel_gcode(
    ...
    const unsigned deceleration = (unsigned) (m_config.travel_deceleration.value + 0.5);
    if (travel.empty()) {
        return "";
    }
    // generate G-code for the travel move
    // use G1 because we rely on paths being straight (G0 may make round paths)
    if (m_config.accel_to_decel_enable.value && deceleration > 0) 
    {
        gcode += this->m_writer.set_travel_acceleration(acceleration, deceleration);
    ...
    for (const Vec3crd& point : travel) {
        ...
        const Vec3d gcode_point{this->point_to_gcode(point)};
        gcode += this->m_writer.travel_to_xyz(previous_point, gcode_point, comment, m_layer_index);
        this->last_position = point.head<2>();
        previous_point = gcode_point;
    }
...
std::string GCodeGenerator::travel_to(
   ...
   // double b1 = sqrt(pow(end_point.x() - start_point.x(), 2) + pow(end_point.y() - start_point.y(), 2)) * 1e-6;
    if (sqrt(pow(end_point.x() - start_point.x(), 2) + pow(end_point.y() - start_point.y(), 2)) * 1e-6 <= m_config.estimated_empty_distance.get_at(extruder_id)) {
    ...
    std::string   s1     = generate_travel_gcode(travel, comment);
    return wipe_retract_gcode + s1;
}
