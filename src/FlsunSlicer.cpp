...
int CLI::run(int argc, char **argv)
{
    ...
    for (const std::string& s : { "opengl-version", "opengl-compatibility", "opengl-debug", "gcodeviewer" }) {
        ...
        if (it != m_actions.end()) {
            boost::nowide::cerr << "Parameter '" << s << "' is ignored, this FlsunSlicer build is CLI only." << std::endl;
            m_actions.erase(it);
        }
    }
#endif
    ...
    if (!start_as_gcodeviewer) {
        for (const std::string& file : m_input_files) {
            if (boost::starts_with(file, "flsunslicer://")) {
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into FlsunSlicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a FlsunSlicer process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the FlsunSlicer process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"FlsunSlicer is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using FlsunSlicer.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes FlsunSlicer "
                L"to crash on a secondary monitor, see FlsunSlicer github issue #5573";
...
#endif /* SLIC3R_GUI */
        << std::endl
        << "https://github.com/prusa3d/FlsunSlicer" << std::endl << std::endl
...
extern "C" {
    // Enable printing stacktrace by default. It can be disabled by running FlsunSlicer with "UBSAN_OPTIONS=print_stacktrace=0".
