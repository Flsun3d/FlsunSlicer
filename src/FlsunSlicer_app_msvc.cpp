...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    if (load_mesa) {
        ...
        if (!res) {
            MessageBox(nullptr, L"FlsunSlicer was unable to automatically switch to MESA OpenGL library\nPlease, try to run the application using the '--sw-renderer' option.\n",
                L"FlsunSlicer Warning", MB_OK);
            return -1;
        }
        else {
    ...
    wcscat(path_to_slic3r, L"FlsunSlicer.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("FlsunSlicer.dll was not loaded\n");
        return -1;
    }
    ...
    if (slic3r_main == nullptr) {
        printf("could not locate the function slic3r_main in FlsunSlicer.dll\n");
        return -1;
    }
